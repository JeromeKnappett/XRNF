...
# --- PARAMETERS ---
data_dir = '/user/home/ptypy-0.5.0/jk/experiments/SOLEIL_telePtycho_91/'
# '/user/home/opt/xl/xl/experiments/SOLEIL_telePtycho_92/' 
scan_name = 'Image_20250928_001standardized'
# '13x15_20um_1600_pad448'
# '13x15_20um_2048_pad224'
# '13x15_20um_2048'
h5_file_path = f"{data_dir}{scan_name}.h5"#f"{data_dir}SOLEIL_{scan_name}.h5"   # 🔁 Change to your actual HDF5 file path
define_roi = None  # Set to (x_min, x_max, y_min, y_max) in meters, or None for full dataset
asize = 128#2048#1600#2048#512 #256  # Size of the overlay box in plot (in pixels)
pad = None#448

# --- LOAD HDF5 FILE ---
with h5py.File(h5_file_path, 'r') as f:
    data_stack = f['entry/data'][:]  # Shape: (N, H, W)
    pixel_size = f['entry/pixel_m'][()]#f['det_pixelsize_m'][()]  # [dx, dy]
...
xmR = np.nanmean(MX)
ymR = np.nanmean(MY)
xmxR = np.nanmedian(MX)
ymxR = np.nanmedian(MY)
xm = np.nanmean(MXc)
ym = np.nanmean(MYc)
xmx = np.nanmedian(MXc)
ymx = np.nanmedian(MYc)
...
plt.scatter(xm,ym,label='Mean (outliers removed)')
plt.scatter(xmR,ymR, label='Mean (raw)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.imshow(np.log(np.sum(data_stack,0)))#[500:-500,500:-500])
plt.text(xm,ym,'X',color='red')
plt.text(1023.5,1023,'x',color='white')
plt.xlim(xm-asize//8,xm+asize//8)
plt.ylim(ym-asize//8,ym+asize//8)
plt.show()
