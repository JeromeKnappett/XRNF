...
class edge_roughness:
    ...
    #analyse an image
    def LER_analysis(self, image, res, thresh,maxI=None,show=False,extra=0): #origional image, width in meters of image (ie the scale), threshold for outliers
        
        if maxI == None:
            maxI = np.max(image)
    
        bin_im = self.binarization(image,maxI,show=False) #binarize the image
        
        for e,i in enumerate(bin_im.T):
            if i[0] != 0:
                try:
                    k = i.index(0)
                except:
        ...
        #         except:
        #             k = 10
        #         # np.flipup(bin_im.T)[e][]
        #         i[k:-1] = 255
        
        # plt.imshow(bin_im,aspect='auto')
        # plt.show()                
    ...
    def binarization(self, image, maxI, show):
        ...
        ret1,th1 = cv2.threshold(blur,maxI*0.6,255,cv2.THRESH_BINARY_INV) #manual threshold value # Changed to 0.6 of max - JK 05/03/25
        
        # if th1[0,0] !=0:
        #     ret1,th1 = cv2.threshold(blur,maxI*0.6,255,cv2.THRESH_BINARY_INV)
        
        if show:
            plt.imshow(th1,aspect='auto')
            plt.colorbar()
            plt.show()
