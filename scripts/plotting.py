...
def resample(a, fx, fy, midX=None, midY=None, debug=False, show=False):
    ...
    aNew = a[
        int(midY - fy * midY) : int(midY + fy * midY),
        int(midX - fx * midX) : int(midX + fx * midX),
    ]
    if debug:
...
def plotTwoD(
    ...
    plt.yticks(
        [int((ny - 1.0) * (b / (numYticks - 1.0))) for b in range(0, numYticks)],
        [
            round_sig(ny * dy * (a / (numYticks - 1.0)) * sF)
            for a in range(-int((numYticks - 1.0) / 2.0), int((numYticks / 2.0 + 1.0)))
        ],
        fontsize=fSize,
    )
    plt.xticks(
        [int((nx - 1.0) * (b / (numXticks - 1.0))) for b in range(0, numXticks)],
        [
            round_sig(nx * dx * (a / (numXticks - 1.0)) * sF)
            for a in range(-int((numXticks - 1.0) / 2.0), int((numXticks / 2.0 + 1.0)))
        ],
        fontsize=fSize,
    )
    if cbar:
...
def plotMultiTwoD(
    ...
    for i, t in enumerate(T):
        if dims[0] > 1 and dims[1] > 1:
            if i <= dims[1] - 1:
                im = ax[0, i].imshow(
                    t, vmin=ran[i][0], vmax=ran[i][1], cmap=colour, aspect=aspct
                )
                if title[i] != None:
                    ax[0, i].set_title(title[i])
                ax[0, i].set_yticks(
                    [
                        int((ny[i] - 1.0) * (b / (numYticks - 1.0)))
                        for b in range(0, numYticks)
                    ]
                )
                ax[0, i].set_xticks(
                    [
                        int((nx[i] - 1.0) * (b / (numXticks - 1.0)))
                        for b in range(0, numXticks)
                    ]
                )
                if onlyEdgeLabels:
                    ...
                    if i == 0:
                        ...
                        ax[0, i].set_yticklabels(
                            [
                                round_sig(ny[i] * dy[i] * (a / (numYticks - 1.0)) * sF)
                                for a in range(
                                    -int((numYticks - 1.0) / 2),
                                    int((numYticks / 2 + 1)),
                                )
                    ...
                    ax[0, i].set_yticklabels(
                        [
                            round_sig(ny[i] * dy[i] * (a / (numYticks - 1.0)) * sF)
                            for a in range(
                                -int((numYticks - 1.0) / 2), int((numYticks / 2 + 1))
                            )
                        ],
                        fontsize=fSize,
                    )
                    ...
                    ax[0, i].set_xticklabels(
                        [
                            round_sig(nx[i] * dx[i] * (a / (numXticks - 1.0)) * sF)
                            for a in range(
                                -int((numXticks - 1.0) / 2), int((numXticks / 2 + 1))
                            )
                        ],
                        fontsize=fSize,
                    )
                ...
                im2 = ax[1, abs(i - dims[1])].imshow(
                    t, vmin=ran[i][0], vmax=ran[i][1], cmap=colour, aspect=aspct
                )
                if title[i] != None:
                    ax[1, abs(i - dims[1])].set_title(title[i])
                ax[1, abs(i - dims[1])].set_yticks(
                    [
                        int((ny[i] - 1.0) * (b / (numYticks - 1.0)))
                        for b in range(0, numYticks)
                    ]
                )
                ax[1, abs(i - dims[1])].set_xticks(
                    [
                        int((nx[i] - 1.0) * (b / (numXticks - 1.0)))
                        for b in range(0, numXticks)
                    ]
                ...
                im3 = ax[2, abs(i - 2 * dims[1])].imshow(
                    t, vmin=ran[i][0], vmax=ran[i][1], cmap=colour, aspect=aspct
                )
                if title[i] != None:
                    ax[2, abs(i - 2 * dims[1])].set_title(title[i])
                ax[2, abs(i - 2 * dims[1])].set_yticks(
                    [
                        int((ny[i] - 1.0) * (b / (numYticks - 1.0)))
                        for b in range(0, numYticks)
                    ]
                )
                ax[2, abs(i - 2 * dims[1])].set_xticks(
                    [
                        int((nx[i] - 1.0) * (b / (numXticks - 1.0)))
                        for b in range(0, numXticks)
                    ]
            ...
            if onlyEdgeLabels:
                if dims[0] * dims[1] - dims[1] <= i <= dims[0] * dims[1] - 1:
                    ...
                    ax[dims[0] - 1, abs(i - dims[0] * dims[1]) - 1].set_xticks(
                        [
                            int((nx[i] - 1.0) * (b / (numXticks - 1.0)))
                            for b in range(0, numXticks)
                        ]
                    )
                    ax[dims[0] - 1, abs(i - dims[0] * dims[1]) - 1].set_xticklabels(
                        [
                            round_sig(nx[i] * dx[i] * (a / (numXticks - 1.0)) * sF)
                            for a in range(
                                -int((numXticks - 1.0) / 2), int((numXticks / 2 + 1))
    ...
    if cBar == "side":
        if multiCBar == False:
            ...
            try:
                fig.colorbar(im, cax=cbar_ax, label=cbarLabel).ax.tick_params(
                    labelsize=10
                )
...
def plotOneD(
    ...
    if split != None:
        ...
        if split != "dual":
            for i, l in enumerate(p):
                if customX != None:
                    ...
                    ax[i].set_xticklabels(
                        [round_sig(x * d[i] * (a / 6) * sF) for a in range(-3, 4)],
                        fontsize=fSize,
                    )
                ...
                ax[i].legend(shadow=True,frameon=True)#,facecolor='gray')

    else:
        for i, l in enumerate(p):
            # print("HERE I AM")
            # print(customX[i])
            if customX != None or any(customX[i]) != None:
                print("custom x-axis....")
                ...
                plt.xticks(
                    [int(x * (b / 8)) for b in range(0, 9)],
                    [round_sig(x * d[i] * (a / 8) * sF) for a in range(-4, 5)],
                    fontsize=fSize,
                )
        ...
        plt.legend(shadow=True,frameon=True)#,facecolor='gray')
    plt.tight_layout()
    if savePath != None:
        print("Saving : ", savePath)
        plt.savefig(savePath)
        ...
        if i == 0:
            ...
            if split:
                for j in l:
                    ...
                    try:
                        ax[0, i].plot(
                            X, j, linestyle=lineStyle, marker=pointStyle
                        )  # , label=legend[i])
                ...
                try:
                    ax[0, i].plot(
                        X, l, linestyle=lineStyle, marker=pointStyle
                    )  # , label=legend[i])
                    ax[0, i].set_ylabel(yLabel[i], fontsize=fSize)
                except IndexError:
                    ax[i].plot(
            ...
            if split:
                for j in l:
                    try:
                        ax[2, abs(i - 2 * dims[1])].plot(
                            X, j, linestyle=lineStyle, marker=pointStyle
                        )
                        ax[2, abs(i - 2 * dims[1])].set_ylabel(
                            yLabel[i], fontsize=fSize
                        )
                    except IndexError:
                        ax[i].plot(X, j, linestyle=lineStyle, marker=pointStyle)
                        ax[i].set_ylabel(yLabel[i], fontsize=fSize)
...
def plotCorrelationMap(
    ...
    dF = pd.DataFrame(np.array(dataStructure).T, columns=labels)
    ...
    res = sns.heatmap(
        abs(correlations).round(2), cmap=colours, vmin=ran[0], vmax=ran[1], annot=annot
    )
    res.set_xticklabels(
        res.get_xmajorticklabels(), fontsize=labelSize[0], rotation=labelRot[0]
    )
    res.set_yticklabels(
        res.get_ymajorticklabels(), fontsize=labelSize[1], rotation=labelRot[1]
    )
    if savePath != None:
...
def plotComplex2D(
    ...
    plt.yticks(
        [int((ny - 1) * (b / (numYticks - 1))) for b in range(0, numYticks)],
        [
            round_sig(ny * dy * (a / (numYticks - 1)) * sF)
            for a in range(-int((numYticks - 1) / 2), int((numYticks / 2 + 1)))
        ],
        fontsize=fSize,
    )
    plt.xticks(
        [int((nx - 1) * (b / (numXticks - 1))) for b in range(0, numXticks)],
        [
            round_sig(nx * dx * (a / (numXticks - 1)) * sF)
            for a in range(-int((numXticks - 1) / 2), int((numXticks / 2 + 1)))
        ],
        fontsize=fSize,
    )
    plt.colorbar(label=cbarLabel).ax.tick_params(labelsize=10)
...
def testPlot2D():
    ...
    Tnew = [
        resample(t, fx=1, fy=0.25) for t in T
    ]  # [resample(t, fx=0.024,fy=0.14) for t in T]
...
def testMultiTwoD():
    ...
    Tnew = [
        np.array(resample(t, fx=0.00415, fy=0.085, midX=mid[1], midY=mid[0])).T
        for t in T
    ]
...
def testPlot1D():
    ...
    res = [
        2.5011882651601634e-09,
        2.5011882651601634e-09,
    ]  # coherenceAE[2.4986984256410265e-09, 2.5011882651601634e-09, 2.5053583133791704e-09] #[3.3452615124385716e-07]
...
def testMultiOneD():
    # rcParams['figure.dpi']=300
    dirPath = '/home/jerome/dev/data/coherenceEffects/'
#    "/home/jerome/dev/data/LER/"
    ...
    # MULTIPLE
    files =[ 'contrastMetrics50.pkl', 'contrastMetrics350.pkl','contrastMetrics50_2d.pkl']
...
#        "$LWR$",
    ...
    #    ['$NILS_{mean}$', '$C_{Composite}$', '$NILS_{\sigma_n}$', '$LWR [nm]$']
    legend =['$\Delta SSA_x = 50 \mu m$','$\Delta SSA_x = 350 \mu m$'] 
#    ["$6.7 \ nm$", "$13.5 \ nm$"]
    ...
    savePath = None
#    "/home/jerome/Documents/OneDrive/Documents/PhD/Figures/forPapers/AImetricsLER.pdf"
...
#    X = X[0]  # [1::]
    ...
    #    print(X)
    ##
    plt.plot(X[0], A[0][0], "x:",label='michelson')
    plt.plot(X[2], A[2][0], "x:")
#    plt.plot(X[2], A[2][1], "x:")
    plt.title('Michelson')
    plt.show()
    
    plt.plot(X[0], A[0][1], "x:")
    plt.plot(X[2], A[2][1], "x:")
#    plt.plot(X[2], A[2][1], "x:")
    plt.title('RMS')
    plt.show()
    
    plt.plot(X[0], [a/0.6 for a in A[0][2]], "x:")
    plt.plot(X[2], [a[0]/0.6 for a in A[2][2]], "x:")
#    plt.plot(X[2], A[2][1], "x:")
    plt.title('Composite')
    plt.show()
    
    plt.plot(X[0], [a/np.pi for a in A[0][3]], "x:")
    plt.plot(X[2], [a/np.pi for a in A[2][3]], "x:")
#    plt.plot(X[2], A[2][1], "x:")
    plt.title('NILS rms')
    plt.show()
    
    plt.plot(X[0],[1 - a for a in A[0][4]], "x:")
    plt.plot(X[2],[1 - a for a in A[2][4]], "x:")
#    plt.plot(X[2], A[2][1], "x:")
    plt.title('Fidelity')
    plt.show()
    
    plt.plot(X[0], [a/np.pi for a in A[0][5]], "x:")
    plt.plot(X[2], [a/np.pi for a in A[2][5]], "x:")
#    plt.plot(X[2], A[2][1], "x:")
    plt.title('NILS')
    plt.show()
    #
    plt.plot(X[0], A[0][0], "x:",label='michelson')
    plt.plot(X[0], A[0][1], "x:",label='RMS')
    plt.plot(X[0], [a/0.6 for a in A[0][2]], "x:",label='composite')
    plt.plot(X[0], [a/np.pi for a in A[0][5]], "x:",label='NILS')
    plt.legend()
    plt.show()
    
    plt.plot(X[1], A[1][0], "x:",label='michelson')
    plt.plot(X[1], A[1][1], "x:",label='RMS')
    plt.plot(X[1], [a/0.6 for a in A[1][2]], "x:",label='composite')
    plt.plot(X[1], [a/np.pi for a in A[1][5]], "x:",label='NILS')
    plt.legend()
    plt.show()
    #    # print(A)
    
    plt.plot(X[2],[p*1e9 for p in picks[2][lwr][1:8]], "x:")
    plt.title("LWR [nm]")
    plt.show()
...
def testComplex():
    ...
    files = ['maskExitEfieldEfieldsNEW.pkl']
...
#    ["sx100sy100Efields.pkl"]
    ...
    savepathImage = dirPath + "maskExit.eps" #dirPath + "atMask100x100Complex.eps"
    ...
    savepathCbar = None # dirPath + 'aerialImageCbar.eps'
    ...
    for i, e in enumerate(E):
        ...
        ny = np.shape(cIm)[0]
        x,y = 240,25
#        cIm = cIm[ny//2 - y:ny//2 + y, nx//2 - x:nx//2 + x]
#        cBar, di, dP, dA = c2image.complexCbar(e, 1000)
        ...
        plotTwoD(
            ...
            cbar=False,
            aspct='auto', #40.0,  #'auto',
            savePath=savepathImage,
        )  # [i])
...
#        plt.show()
#        
#        plotMultiTwoD([cIm[180:210,995:1120],cIm[180:210,12387:12512],cIm[180:210,23895:24020]],
#                      dims=(1,3),
#                      dx=[res[i][0],res[i][0],res[i][0]],
#                      dy=[res[i][1],res[i][1],res[i][1]],
#                      sF=1e9,
#                      numXticks=5,
#                      fSize = 6,
#                      title=['-1','aerial image','+1'],
#                      xLabel="x-position [nm]",  #'x-position [nm]', #'x-position $[\mu m]$',
#                      yLabel="y-position [nm]",
#                      cBar = None,
#                      aspct=4,
#                      onlyEdgeLabels=True)
#        
#        plotMultiTwoD([np.abs(e)[188:208,996:1126],np.abs(e)[188:208,12384:12514],np.abs(e)[188:208,23892:24022],
#                       np.angle(e)[188:208,996:1126],np.angle(e)[188:208,12384:12514],np.angle(e)[188:208,23892:24022],
#                       cIm[188:208,996:1126],cIm[188:208,12384:12514],cIm[188:208,23892:24022]],
#                      dims=(3,3),
#                      dx=[res[i][0],res[i][0],res[i][0],
#                          res[i][0],res[i][0],res[i][0],
#                          res[i][0],res[i][0],res[i][0]],
#                      dy=[res[i][1],res[i][1],res[i][1],
...
#                      onlyEdgeLabels=True)
#        
#        plotMultiTwoD([np.abs(e)[188:208,6629:6759],np.angle(e)[188:208,6629:6759],cIm[188:208,6629:6759]],
#                       dims=(1,3),
#                       dx=[res[i][0],res[i][0],res[i][0]],
#                       dy=[res[i][1],res[i][1],res[i][1]],
#                       sF=1e9,
...
#                       aspct=6,
#                       onlyEdgeLabels=True)
##        plt.imshow(cIm[170:220,0:3000], aspect="auto")
##        plt.colorbar()
#        # plt.savefig(dirPath + 'atMask100x100Intensity.eps')
##        plt.show()

def testSplitPlot1D():
    dirPath = "/home/jerome/dev/data/CvsI/"
    files = ["cXlength.pkl", "totIx.pkl"]
    labels = None  # ['TE','TM'] # ['$100$ $\mu m$ $SSA$','$200$ $\mu m $ $SSA$','$300$ $\mu m$ $SSA$'] #['normalised intensity (y=0)', 'horizontal coherence']
    res = [1,1]
    resnmidIncluded = False
    ...
    ps[0] = [p*1e3 for p in ps[0][0:len(ps[0])-1]]
    ps[1] = [p for p in ps[1][0:len(ps[1])-1]]
    
    I = ps[1]
    C = ps[0]
    print(ps[1])
    x = [27.5,50,75,100,125,150,200,250,300,350]
    ...
    plotOneD(
        ...
        title=None,
        labels=labels,
        xLabel=["Horizontal SSA Size [$\mu$m]"],  # ['Position [nm]'],
        yLabel=["Intensity [ph/s/0.1$\%$bw/cm$^2$]","Coherence Length [mm]"],  # ['Intensity [ph/s/.1\%bw/mm²]']],
        lStyle=[":",":"],#[":", ":"],
        lWidth=1,
        pStyle=".",  #'',
        fSize=10,
