...
for i, bd in enumerate(Bdif_0):
    plt.text(
        x=x0 - 12,
        y=(B0 / 4) * (i + 12),
        s="$B_0 (m=$" + str(M[i + 1]) + ") = " + str(round_sig(bd)) + "$B_0 (m=1)$",
        fontsize=6,
    )
...
plt.text(
    x=x0 + 1.1, y=0.0, s="E=" + str(round_sig(E0 + xp, 5)), fontsize=6, color="gray"
)
for i, bd in enumerate(Bdif_p):
    plt.text(
        x=x0 - 12,
        y=(B0 / 4) * (i + 7),
        s="$B_p (m=$" + str(M[i + 1]) + ") = " + str(round_sig(bd)) + "$B_p (m=1)$",
        fontsize=6,
        color="gray",
    )
...
for i, bd in enumerate(Bdif_best):
    plt.text(
        x=x0 - 12,
        y=(B0 / 4) * (i + 2),
        s="$B(m=$" + str(M[i + 1]) + ") = " + str(round_sig(bd)) + "$B(m=1)$",
        fontsize=6,
        color="blue",
    )
