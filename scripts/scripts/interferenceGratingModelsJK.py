...
def LWR(I, px, dim=1):
    ...
    med = np.mean(stripes)
    lower = int(max(0, 0.4 * med))
    upper = int(min(255, 0.6 * med))
    ...
    # sobel = cv2.Sobel(stripes,cv2.CV_64F,dx,dy,ksize=3)
    ...
    plt.show()
    plt.imshow(edges, cmap="Greys", aspect=0.1)
...
def test():
    ...
    files = [
        "beamCoherence2/data/fullRes_4000e/fullRes_4000eintensity.tif",
        "beamCoherence2/data/fullRes_5000e/fullRes_5000eintensity.tif",
    ]
    ...
    pickledFiles = [
        "maskRoughness/data/1/10.pkl",
        "maskRoughness/data/2/20.pkl",
        "maskRoughness/data/3/30.pkl",
        "maskRoughness/data/4/40.pkl",
    ]
    ...
    fourierC = [
        gratingContrastFourier(a, profileV1 * 1e6, show=False) for a in aerialImagesV
    ...
    fidel = [
        fidelity(a, IV) for a in aerialImagesV
    ...
    compositeC5 = [
        meanDynamicRange(a) for a in aerialImagesRMS5nm
    ]  # , mdrC, imbalanceC
    nils15 = [NILS(a, profileV1, d / 4, show=False) for a in aerialImagesRMS5nm]
    fourierC5 = [
        gratingContrastFourier(a, profileV1 * 1e6, show=False)
        for a in aerialImagesRMS5nm
    ]  # Cf,  Am, Fr, peakFr - Still unsure but seems good
...
def testx():
    ...
    profileFile = (
        "/user/home/opt/xl/xl/experiments/beamPolarisation17ME/data/t0/profile.txt"
    )
    ...
    d = 65.0e-9  # expected half-width of pattern, i.e width of line
...
def testTETMContrast():
    ...
    m = 1  # order of diffracted beams from each grating
    pitch = [
        round_sig(n, sig=5) for n in np.linspace(10e-9, 100e-9, 10000)
    ]  # 24e-9 #24e-9 #100e-9 # grating spacing
    #    print(pitch)
    ...
    n = 200000
    x = np.linspace(
        -xstep * int(n / 2), xstep * int(n / 2), n + 1
    )  # [round_sig(n,sig=5) for n in np.linspace(-xstep*int(n/2),xstep*int(n/2),n)]  #np.linspace(-90e-9,90e-9,10000)
    # print(x)
    ...
    """ generate interference intensity"""
    I_TM = [
        interferenceIntensity(x, k, t, A=A, polarisationModes=("TM", "TM"))
        for t in theta
    ]
    I_TE = [
        interferenceIntensity(x, k, t, A=A, polarisationModes=("TE", "TE"))
        for t in theta
    ]
    ...
    fourierC = [
        gratingContrastFourier(a, x, show=False) for a in I_TM
    ]  # Cf,  Am, Fr, peakFr - Still unsure but seems good
    ...
    fourierCte = [
        gratingContrastFourier(a, x, show=False) for a in I_TE
    ]  # Cf,  Am, Fr, peakFr - Still unsure but seems good
    ...
    ax1.set_xlabel("Grating Pitch [nm]")
    ax1.set_xticklabels(
        [1e9 * np.max(pitch) * a for a in [0, 1 / 5, 2 / 5, 3 / 5, 4 / 5, 1]]
    )
    if save:
    ...
    axs[0, 0].set_xticklabels(
        [1e9 * np.max(pitch) * a for a in [0, 1 / 4, 1 / 2, 3 / 4, 1]]
    )
