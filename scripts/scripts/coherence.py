...
def testCoherenceProfile():
    ...
    res = [(3.3452615124385716e-07, 6.074644954894064e-06)]
    ...
    # Converting image files into complex electric fields
    ...
    for ce, ch, cv, s, r in zip(cE, Eh, Ev, sX, res):
        ...
        #        J_H = np.array([ex.conjugate()*a for a in ex]) # Horizontal coherence profile - Eq. 2.3.1.3
        #        print("Shape of each Mutual Intensity profile:")
        #        print("Horizontal profile: {}".format(np.shape(J_H)))
        #        j_H = np.sqrt(J_H**2) #abs(J_H) #(J_H[0]**2 + J_H[1]**2)**(1/2)
        #        jHd = np.diagonal(np.squeeze(J_H))        # Diagonal of matrix J_H
    ...
    # ax3 = ax2.twinx()
    # for coherence
    ax0.plot(sX, [c * 1e6 for c in cLx], ":x")
...
def testSumEfields():
    ...
    # resolution of each electric field
    res = [
        (3.364751715568435e-07, 2.7893918273477274e-06),
    ...
    Eh = [ExR + ExI * 1j for ExR, ExI in zip(EhR, EhI)]
    Ev = [EyR + EyI * 1j for EyR, EyI in zip(EvR, EvI)]
    cE = [Ex + Ey for Ex, Ey in zip(Eh, Ev)]
    ...
    for ce, ch, cv, r in zip(cE, Eh, Ev, res):
        I = ce.conjugate() * ce
        ex = ce[int(np.shape(ce)[0] / 2), :]
        ey = ce[:, int(np.shape(ce)[1] / 2)]
        Ix = I[int(np.shape(I)[0] / 2), :]
        ...
        #
        #        axs[0].plot(ch)
        #        axs[0].set_title('$E_x$ - Complex')
        #        axs[1].plot(cv)
        #        axs[1].set_title('$E_y$ - Complex')
        #        plt.tight_layout()
        #        plt.show()
        ...
        axs[0].plot(ey.real)
        axs[0].set_title("$E_y$ - Real")
        axs[1].plot(ey.imag)
        axs[1].set_title("$E_y$ - Imaginary")
        axs[2].plot(ey)
        axs[2].set_title("$E_y$ - Complex")
        axs[3].plot(abs(ey))
        axs[3].set_title("$I_y$")
        plt.tight_layout()
        plt.show()
    ...
    sumY_1 = iY[0] + iY[1] + 2 * A * np.sqrt(iY[0] * iY[1] * (cY[1].imag - cY[0].imag))
    sumY_2 = np.sqrt(sumY_1)

    sumY_3 = (
        iY[0] + iY[1] + 2 * A * np.sqrt(iY[0] * iY[1]) * np.cos(cY[1].imag - cY[0].imag)
    )
    sumY_4 = np.sqrt(sumY_3)

    sumY_test1 = iY[0] + iY[1] + 2 * A * np.sqrt(iY[0] * iY[1] * (phaseDiff))
    sumY_test2 = np.sqrt(sumY_test1)

    #    plt.clf()
    #    plt.close()
    #    fig, axs = plt.subplots(2,2)
    #    axs[0,0].plot(sumY_1)
    ##    axs[0,0].set_title('$summed fields #1$')
    #    axs[0,1].plot(sumY_2)
    ##    axs[0,1].set_title('$summed fields #2$')
    #    axs[1,0].plot(sumY_3)
    ##    axs[1,0].set_title('$summed fields #3$')
    #    axs[1,1].plot(sumY_4)
    ##    axs[1,1].set_title('$summed fields #4$')
    #    plt.tight_layout()
    #    plt.show()
...
def sumWaves(E_1, a_1, E_2, a_2):
    E_0 = np.sqrt(E_1**2 + E_2**2 + 2 * E_1 * E_2 * np.cos(a_2 - a_1))

    a_0 = np.arctan(
        (E_1 * np.sin(a_1) + E_2 * np.sin(a_2))
        / (E_1 * np.cos(a_1) + E_2 * np.cos(a_2))
    )
...
def testSumWaves():
    ...
    wf = getComplex(wf0, polarization="total")

    I_i = wf0.get_intensity()
    ...
    mask = 255 - imageio.imread("/home/jerome/dev/masks/mask_p10.tif")
    ...
    exitWf = w * mask
    print(np.shape(exitWf))

    I_ex = abs(exitWf.conjugate() * exitWf)

    plt.clf()
    plt.close()
    plt.imshow(I_ex)
    plt.title("Exit Surface Wave")
    plt.colorbar()
    plt.show()
