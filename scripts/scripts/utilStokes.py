...
def getPolarisationCharacteristics(S=None, Sparam=None, d=2):
    ...
    if d == 1:
        ...
        print(
            "... eccentricity, inclination and chirality anslysis disabled for 1D cuts"
        )
        ...
        e = np.sqrt(
            s2 * np.sqrt(s1**2 + s2**2) / (1 + np.sqrt(s1**2 + s2**2))
        )  # added sqrt - jerome
...
def getStokesParamFromStokes(stk, d=2):
    ...
    if stk.mutual > 0:
        ...
        if d == 1:
            ...
            s0x = np.reshape(s[0, :, :], (stk.mesh.ny, stk.mesh.nx))[
                int(stk.mesh.ny / 2), :
            ]
            s1x = np.reshape(s[1, :, :], (stk.mesh.ny, stk.mesh.nx))[
                int(stk.mesh.ny / 2), :
            ]
            s2x = np.reshape(s[2, :, :], (stk.mesh.ny, stk.mesh.nx))[
                int(stk.mesh.ny / 2), :
            ]
            s3x = np.reshape(s[3, :, :], (stk.mesh.ny, stk.mesh.nx))[
                int(stk.mesh.ny / 2), :
            ]

            s0y = np.reshape(s[0, :, :], (stk.mesh.ny, stk.mesh.nx))[
                :, int(stk.mesh.nx / 2)
            ]
            s1y = np.reshape(s[1, :, :], (stk.mesh.ny, stk.mesh.nx))[
                :, int(stk.mesh.nx / 2)
            ]
            s2y = np.reshape(s[2, :, :], (stk.mesh.ny, stk.mesh.nx))[
                :, int(stk.mesh.nx / 2)
            ]
            s3y = np.reshape(s[3, :, :], (stk.mesh.ny, stk.mesh.nx))[
                :, int(stk.mesh.nx / 2)
            ]
...
def getStokes(w, mutual=0, Fx=1, Fy=1):
    ...
    if mutual == 0:
        stk.allocate(
            w.mesh.ne, w.mesh.nx, w.mesh.ny, _mutual=mutual
        )  # numbers of points vs photon energy, horizontal and vertical positions
        stk.mesh.zStart = (
            w.mesh.zStart
        )  # 30. #longitudinal position [m] at which UR has to be calculated
        stk.mesh.eStart = w.mesh.eStart  # initial photon energy [eV]
        stk.mesh.eFin = w.mesh.eFin  # 20000. #final photon energy [eV]
        stk.mesh.xStart = (
            w.mesh.xStart
        )  # initial horizontal position of the collection aperture [m]
        stk.mesh.xFin = (
            w.mesh.xFin
        )  # final horizontal position of the collection aperture [m]
        stk.mesh.yStart = (
            w.mesh.yStart
        )  # initial vertical position of the collection aperture [m]
        stk.mesh.yFin = (
            w.mesh.yFin
        )  # final vertical position of the collection aperture [m]
        ...
        stk.allocate(
            w.mesh.ne, int(Fx * (w.mesh.nx)), int(Fy * (w.mesh.ny)), _mutual=mutual
        )  # numbers of points vs photon energy, horizontal and vertical positions
        stk.mesh.zStart = (
            w.mesh.zStart
        )  # 30. #longitudinal position [m] at which UR has to be calculated
...
def normaliseStoke(S, d=2, outputArray=False):
    ...
    if d == 1:
        s0x, s1x, s2x, s3x, s0y, s1y, s2y, s3y = (
            S[0],
            S[1],
            S[2],
            S[3],
...
def plotStokes(s, dx=1, dy=1, savePath=None, compact=False):
    ...
    if compact:
        ...
        plt.setp(axs, xticks=tickX, xticklabels=labX, yticks=tickY, yticklabels=labY)
        im = axs[0].imshow(
            s[0], cmap="RdYlBu_r", vmin=smin, vmax=smax, aspect=1
        )  # )#, aspect=0.045)#1)#0.045)#'auto')#1)#
        axs[0].set_title("$S_0$")
        axs[1].imshow(
            s[1], cmap="RdYlBu_r", vmin=smin, vmax=smax, aspect=1
        )  # )#, aspect=0.045)#1)#0.045)#'auto')#1)
        axs[1].set_title("$S_1$")
        axs[2].imshow(
            s[2], cmap="RdYlBu_r", vmin=smin, vmax=smax, aspect=1
        )  # )#, aspect=0.045)#1)#0.045)#'auto')#1)
        axs[2].set_title("$S_2$")
        axs[3].imshow(
            s[3], cmap="RdYlBu_r", vmin=smin, vmax=smax, aspect=1
        )  # )#, aspect=0.045)#1)#0.045)#'auto')#1)
        axs[3].set_title("$S_3$")
        ...
        plt.setp(axs, xticks=tickX, xticklabels=labX, yticks=tickY, yticklabels=labY)
        im = axs[0, 0].imshow(
            s[0], cmap="RdYlBu_r", vmin=smin, vmax=smax, aspect="auto"
        )
    ...
    plt.setp(axs, xticks=tickX, xticklabels=labX, yticks=tickY, yticklabels=labY)
    im1 = axs[0].imshow(
        sPol, cmap="cividis", aspect=1
    )  # )#, aspect=0.045)#)#, vmin=pMin, vmax=pMax)
    axs[0].set_title("Polarised component", fontsize=20)
    im2 = axs[1].imshow(
        sUnPol, cmap="cividis", aspect=1, vmin=0, vmax=0.000001
    )  # )#, aspect=0.045)#)#, vmin=pMin, vmax=pMax)
    axs[1].set_title("Unpolarised component", fontsize=20)
    im3 = axs[2].imshow(
        D, cmap="cividis", aspect=1, vmin=0, vmax=1
    )  # )#, aspect=0.045)#, aspect='auto')
    ...
    if savePath != None:
        print(
            "Saving ellipticity figure to path: {}".format(savePath + "ellipticity.png")
        )
    ...
    if savePath != None:
        print(
            "Saving Inclination figure to path: {}".format(savePath + "inclination.png")
        )
...
def sumWaves(
    wavefronts, sampleFraction=1, stokes=None, norm=None, plots=None, savePath=None
):
    from wpg.wavefront import Wavefront
    from wpg.srwlib import SRWLStokes, SRWLWfr
    ...
    for i, f in enumerate(wavefronts):
        ...
        Ex = _Ex[
            int(midEhx - midEhx * sampleFraction) : int(
                midEhx + midEhx * sampleFraction
            ),
            int(midEhy - midEhy * sampleFraction) : int(
                midEhy + midEhy * sampleFraction
            ),
            :,
        ].flatten()
        Ey = _Ey[
            int(midEvx - midEvx * sampleFraction) : int(
                midEvx + midEvx * sampleFraction
            ),
            int(midEvy - midEvy * sampleFraction) : int(
                midEvy + midEvy * sampleFraction
            ),
            :,
        ].flatten()
        ...
        if stokes != None:
            ...
            if stokes == 0:
                ...
                char = getPolarisationCharacteristics(
                    Sparam=stot, d=1
                )  # Dx, Dy, DavgX, DavgY
                snT = normaliseStoke(stot, d=1)
                normChar = getPolarisationCharacteristics(
                    Sparam=stot / np.max(stot[0]), d=1
                )  # Dx, Dy, DavgX, DavgY
    ...
    if stokes != None:
        ...
        if stokes == 0:
            ...
            getPolarisationCharacteristics(
                Sparam=sFin / np.max(sFin[0]), d=1
            )  # D, Davg, e, i, c
            if plots != None:
        ...
        if plots != None:
            if stokes == 0:
                ...
                plt.savefig(plots + "degPol.png")
                print(
                    "Degree of Polarisation of final wavefield saved to: "
                    + plots
                    + "degPol.png"
                )
                plt.show()
                ...
                print(
                    "Stokes parameter variation (x-cut) of wavefield saved to: "
                    + plots
                    + "stokeEvolutionX.png"
                )
                plt.show()
                ...
                print(
                    "Stokes parameter variation (y-cut) of wavefield saved to: "
                    + plots
                    + "stokeEvolutionY.png"
                )
                plt.show()
                ...
                dp2x = ((stoke1x**2 + stoke2x**2 + stoke3x**2) ** (1 / 2)) / (
                    stoke0x
                )
                dp2y = ((stoke1y**2 + stoke2y**2 + stoke3y**2) ** (1 / 2)) / (
                    stoke0y
                )
                ...
                print(
                    "Degree of Polarisation variation of wavefield saved to: "
                    + plots
                    + "degPolEvolution.png"
                )
                plt.show()
                ...
                print(
                    "Stokes parameter variation of wavefield saved to: "
                    + plots
                    + "stokeEvolution.png"
                )
                plt.show()
...
def sumStokes(
    ...
    for i, f in enumerate(wavefronts):
        ...
        if i == 0:
            if dimensions == 2:
                stokes.append(s)
                stokesN.append(sn[1])
                Pchars = getPolarisationCharacteristics(
                    Sparam=np.squeeze(stokes), d=2
                )  # D, Davg, e, i, c
                P2.append(Pchars[1])
            elif dimensions == 1:
                ...
                Pchars = getPolarisationCharacteristics(
                    Sparam=np.squeeze(stokesCut), d=1
                )  # Dx, Dy, DavgX, DavgY
    ...
    if dimensions == 2:
        ...
        D_pol = (np.sqrt(stokes[1] ** 2 + stokes[2] ** 2 + stokes[3] ** 2)) / (
            stokes[0]
        )
        D_polN = (np.sqrt(totNS[1][1] ** 2 + totNS[1][2] ** 2 + totNS[1][3] ** 2)) / (
            totNS[1][0]
        )
        ...
        if display == True:
            try:
                plotStokes(
                    ...
                    dx=(xMax - xMin) / nx,
                    dy=(yMax - yMin) / ny,
                    savePath=plotsPath + "finalStokes",
                )
                plotStokes(
                    totNS[1],
                    dx=(xMax - xMin) / nx,
                    dy=(yMax - yMin) / ny,
                    savePath=plotsPath + "finalStokesNorm",
                )
                plotStokes(
                    totNN[1],
                    dx=(xMax - xMin) / nx,
                    dy=(yMax - yMin) / ny,
                    savePath=plotsPath + "finalStokesTwiceNorm",
                )
            except TypeError:
            ...
            plt.close()
            fig, axs = plt.subplots(1, 2)
            plt.setp(axs, xticks=Xtix, xticklabels=Xlab, yticks=Ytix, yticklabels=Ylab)
            ...
            if plotsPath != None:
                ...
                print(
                    "Polarisation Components of final wavefield saved to: "
                    + plotsPath
                    + "PolComponents.png"
                )
            plt.show()
            ...
            fig, axs = plt.subplots(1, 3)
            # figsize(15,8))
            plt.setp(axs[0:2], xticks=Xtix, xticklabels=Xlab)
            plt.setp(axs[1], yticks=Ytix, yticklabels=Ylab)
            ...
            if plotsPath != None:
                ...
                print(
                    "Normalised Degree of Polarisation of final wavefield saved to: "
                    + plotsPath
                    + "degPolNorm.png"
                )
            plt.show()
            ...
            if plotsPath != None:
                ...
                print(
                    "Degree of Polarisation evolution saved to: "
                    + plotsPath
                    + "degPolEvolution.png"
                )
            plt.show()
        ...
        D_polx = (np.sqrt(totScuts[1] ** 2 + totScuts[2] ** 2 + totScuts[3] ** 2)) / (
            totScuts[0]
        )
        D_poly = (np.sqrt(totScuts[5] ** 2 + totScuts[6] ** 2 + totScuts[7] ** 2)) / (
            totScuts[4]
        )
        D_polxN = (
            np.sqrt(totNcuts[2][1] ** 2 + totNcuts[2][2] ** 2 + totNcuts[2][3] ** 2)
        ) / (totNcuts[2][0])
        D_polyN = (
            np.sqrt(totNcuts[2][5] ** 2 + totNcuts[2][6] ** 2 + totNcuts[2][7] ** 2)
        ) / (totNcuts[2][4])
        ...
        if display == True:
            ...
            plt.close()
            fig, axs = plt.subplots(2, 2)
            plt.setp(axs, xticks=Xtix, xticklabels=Xlab)
            ...
            sPolxN = np.sqrt(
                totNcuts[2][1] ** 2 + totNcuts[2][2] ** 2 + totNcuts[2][3] ** 2
            )
            sPolyN = np.sqrt(
                totNcuts[2][5] ** 2 + totNcuts[2][6] ** 2 + totNcuts[2][7] ** 2
            )
            ...
            if plotsPath != None:
                ...
                print(
                    "Degree of Polarisation cuts of final wavefield saved to: "
                    + plotsPath
                    + "degPolCuts.png"
                )
            plt.show()
            ...
            if plotsPath != None:
                ...
                print(
                    "Normalised Degree of Polarisation cuts of final wavefield saved to: "
                    + plotsPath
                    + "degPolCutsNorm.png"
                )
            plt.show()
...
def testMultiFunc():
    ...
    wfr_files = [
        dirPath + f + "wf_final.hdf" for f in folders
    ]  # ['wf_tcl.hdf', 'wf_tcr.hdf','wf_tld.hdf','wf_te.hdf'] #,'wf_tlv.hdf', 'wf_tlh.hdf', 'wf_tclOP.hdf', 'wf_tcrOP.hdf','wf_tldOP.hdf','wf_teOP.hdf','wf_tlvOP.hdf', 'wf_tlhOP.hdf']
    ...
    sumWaves(
        ...
        norm="E",
        plots=dirPath + "plots/",
    )  # ,savePath=dirPath + 'TESTwfSum.hdf')
...
def sumStokesFromTif():
    ...
    stokesTifs = [
        [
            tifffile.imread(dirPath + "i" + str(r) + "stokes0.tif"),
            tifffile.imread(dirPath + "i" + str(r) + "stokes1.tif"),
            tifffile.imread(dirPath + "i" + str(r) + "stokes2.tif"),
            tifffile.imread(dirPath + "i" + str(r) + "stokes3.tif"),
        ]
        for r in ran
    ]
    ...
    resampleStokes = [
        [
            s[0][
                np.shape(s[0])[0] // 2
                - (numPix // 2) : np.shape(s[0])[0] // 2
                + (numPix // 2),
                np.shape(s[0])[1] // 2
                - (numPix // 2) : np.shape(s[0])[1] // 2
                + (numPix // 2),
            ]
            for s in stokesTifs
        ],
    ...
    print("... ... summing")
    finalStokes = [
        np.sum(resampleStokes[0], axis=0),
...
if __name__ == "__main__":
    ...
    stokesTifs = [
        tifffile.imread("/home/jerome/dev/data/sourceNdBeam/maskPlane/100s0.tif"),
