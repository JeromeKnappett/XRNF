...
dirPath = "/home/jerome/dev/data/flux/"
#'/home/jerome/dev/experiments/beamPolarisation13/data/' #'/home/jerome/dev/data/sourceNdBeam/'

files = ["res_brilliance(1).dat"]
# ['res_trjHOR.dat','res_trjVER.dat','res_trj45.dat','res_trjCIR.dat'] #
...
plotType = "flux"  #'meSpectrum' #'eTraj' # 'divergence' #'eTraj'
...
for i, f in enumerate(files):
    ...
    if plotType == "meSpectrum":
        ...
        if i == 0:
            ...
            axs[0].axvline(
                x=HM - fwhm / 2, linestyle=":", color=colours[1], label="FWHM"
            )
            ...
            axs[0].text(
                20000, h1.max() / 2, "FWHM: {} eV".format(fwhm * dE), color=colours[1]
            )
            axs[0].legend()
        elif i == 1:
            ...
            axs[1].text(
                2000, h1.max() / 2, "FWHM: {} eV".format(fwhm * dE), color=colours[1]
            )
    ...
    if plotType == "eTraj":
        T = np.loadtxt(
            dirPath + f, dtype=str, comments=None, skiprows=1
        )  # , max_rows=1, usecols=(0)))
        ...
        ax.plot(
            ...
            tZ,
            1e6 * tY,
            label="Electron Trajectory [\u03bcm]",
            color=colours[1],
        )
        # ax.plot(10*bX,1e6*bZ,10*bY, ':', label='magnetic field')
        ax.plot(
            1e1 * bX, tZ, 1e1 * bY, ":", label=r"$B \times 10^1$ [T]", color=colours[3]
        )
        ...
        if save:
            plt.savefig(
                savePath + "magField_{}.pdf".format(str(f[7 : len(str(f)) - 4]))
            )
            plt.savefig(
                savePath + "magField_{}.png".format(str(f[7 : len(str(f)) - 4])),
                dpi=2000,
            )
        plt.show()
    ...
    if plotType == "seIntensity":
        ...
        I = np.reshape(
            np.loadtxt(dirPath + f + sePR, skiprows=10), (int(ny), int(nx))
    ...
    if plotType == "flux":
        D = np.loadtxt(dirPath + f, dtype=str, comments=None, skiprows=1)
        f1 = np.array(D[:, 1], dtype="float")
        f3 = np.array(D[:, 3], dtype="float")
        f5 = np.array(D[:, 5], dtype="float")
        e1 = np.array(D[:, 0], dtype="float")
        e3 = np.array(D[:, 2], dtype="float")
        e5 = np.array(D[:, 4], dtype="float")

        plt.plot(e1 * 1e3, f1, label="1")
        plt.plot(e3 * 1e3, f3, label="3")
        plt.plot(e5 * 1e3, f5, label="5")
        plt.legend()
        plt.show()
        print(np.min([e1, e3, e5]) * 1e3)
...
if save:
    plt.savefig(savePath + "spectrum_{}.pdf".format(str(f[7 : len(str(f)) - 4])))
    plt.savefig(
        savePath + "spectrum_{}.png".format(str(f[7 : len(str(f)) - 4])), dpi=2000
    )
plt.show()
