...
from srwl_bl import srwl_uti_merge_options, srwl_uti_parse_options, srwl_uti_ext_options
...
def csvParams(filename):
    ...
    csv.register_dialect ("parameters",skipinitialspace=True)
    with open(filename, "r") as infile:
       reader = csv.reader(infile,dialect="parameters")
...
#    
    ...
    def mergeOptions(self,options):
        if self.options:
            ...
            self.varParam = srwl_uti_ext_options(self.varParam)      # added - JK 06/11/23
...
#            print(wf.arEx)
    ...
    def processOutput(self,wf,v):
        ...
        v['si_res'] = None  # added by JK - 06/11/23
        # print('\n HERE')
        # print(len(v))
        self.store(v, resp, self.trialCount)
        
    def save_stokes(self,stk,path,filestem):
        
        if stk is not None: 
            try:
                for i,component in enumerate(stk):
                    p = os.path.join(path,filestem + str(i)) + '.pkl'
                    with open(p,'wb') as f:
                        pickle.dump(component, f)
                        print('Wrote file ',p)
            except:
                pass  # later need to log error.
...
#
        ...
        #  for special case of multielectron propagation we need to save each stokes component outside the dictionary of results
        self.save_stokes(cv['res_ipm'],cv['fdir'],cv['stokes_fname'])
        
        #strip the stokes components 
        cv['res_ipm']= None
