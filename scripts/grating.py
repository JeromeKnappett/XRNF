...
class grating:
    ...
    def rotate(self, A, angle, pivot):
        ...
        imgR = scipy.ndimage.rotate(
            imgP, angle, order=1, reshape=False
        )  # order = 1 should avoid overshoot
        return imgR
    ...
    def pattern(self):
        if self.cell is not None:
            ...
            if self.symmetry > 1:
                ...
                mdim = 2 * np.sqrt(
                    np.square(pad + self.xdim / 2)
                    + np.square(self.radius + self.ydim / 2)
                )

                C0 = self.height * np.ones(
                    (int(mdim / self.xres), int(mdim / self.yres))
                )
                block0 = self.blockHeight * np.ones(
                    (int(mdim / self.xres), int(mdim / self.yres))
                )
                ...
                shifty = centre[0] + int(
                    (self.radius - (self.xdim / 2)) / self.xres
                )  # + int((self.radius)/self.xres)  #int((mdim-self.xdim)/self.xres)
                shiftx = centre[1] - int((self.ydim / 2) / self.yres)

                C0[shiftx : shiftx + A.shape[0], shifty : shifty + A.shape[1]] = A
                block0[
                    shiftx : shiftx + A.shape[0], shifty : shifty + A.shape[1]
                ] = np.zeros_like(A)

                if self.rotationMask != 0:
                    C0 = scipy.ndimage.rotate(
                        C0, self.rotationMask, reshape=False, order=1
                    )
                    block0 = scipy.ndimage.rotate(
                        block0, self.rotationMask, reshape=False, order=1
                    )
                ...
                for angle in np.arange(360 / self.symmetry, 360, 360 / self.symmetry):
                    ...
                    blocka = scipy.ndimage.rotate(
                        block0,
                        angle,
                        reshape=False,
                        mode="constant",
                        cval=self.blockHeight,
                        order=1,
                    )
    ...
    def generateRandomRoughness2D(self, h=100e-9, sigma=50e-9, clx=0, cly=0):
        ...
        ** TODO: check/understand relationship between correlation length input parameters
        ...
        if clx != 0 or cly != 0:
            ...
            F = np.exp(
                -(
                    np.square(x) / (np.square(clx) / 2)
                    + np.square(y) / (np.square(cly) / 2)
                )
            )
    ...
    def addRoughnessToMaskEverywhere(
        self,
    ):
    ...
    def addRoughnessToMaskLines(self, threshold=0.5):
        ...
        idx = np.where(self.mask > threshold * np.max(self.mask))
        self.mask[idx] = self.mask[idx] - np.subtract(
            self.roughness[idx], np.mean(self.roughness[idx])
        )
        self.updateMaskRange()
...
def testMetrics(savePath="", profileWidth=10, axis="y"):
    ...
    gr = grating(
        ...
        ydim=10.00e-6,
        height=72e-9
        / 1000000,  # GVR HACK TO MAKE mask and roughness map scales similar
        blockHeight=255,
        symmetry=2,
        rotationCell=0.0,
        rotationMask=90,
    ...
    with open(savePath + "metricsTest.csv", "w") as file:
        ...
        file.write(
            "index, op_Mask_thick, op_Roughness_thick, roughnessSigma, \
    ...
    for x, L, clx in zip(inB, range(numCorrLengths), c_length):
        for height in Rh:
            ...
            print(
                "Height of mask: {}, RMS roughness: {}.  Difference = {}".format(
                    maskHeight, rmsRough, abs(maskHeight - rmsRough)
                )
            )
            ...
            fileStem = (
                savePath + "{height*1e9:.5f}_{clx*1e9:.5f}_{cly*1e9:.5f}"
            )  #    str(round(((height/2)*1e9)-1)) + x

            gr.showRoughness()
            rough.plotHDF(
                gr.roughness, bins="auto", outFileName=fileStem + "_roughness.png"
            )
            ...
            line = (
                "{L},{maskHeight},{height},{sigma},{rmsRough},{clx},{cly},"
                + fileStem
                + "_cell.tif,"
                + savePath
                + "_rough.tif"
...
def testGratingSimple(savePath="./", display=True):
    ...
    if display is True:
        ...
        corrx, corry = rough.getCorrelationLength(acfx, xv), rough.getCorrelationLength(
            acfy, yv
        )
        print("Correlation lengths: lx = {} m, ly = {} m".format(corrx, corry))
...
def testGrating(savePath="./", display=True):
    ...
    print("time taken to generate mask (s): {}".format(end - start))
    ...
    if savePath is not None:
        path = "/opt/xl/xl/experiments/masks/"
        name = "mask_p200.tif"
        gr.writeMask(path + name)
...
def testMultiGrating(savePath="./", display=True):
    """
    Added by Jerome
    Function to create multiple ideal grating masks with variable geometry"""

    # specify number of gratings and define geometry
    numGratings = 1
    pitches = np.linspace(1.0e-6, 1.1e-6, numGratings)
    Nx = 100
    Ny = 100
    xDim = 10e-6
    yDim = 10e-6
    ...
    #    pitches =[10e-9, 14e-9, 18e-9, 22e-9, 26e-9, 30e-9, 34e-9, 38e-9] #np.linspace(10e-9, 36e-9, numGratings)

    for i, p in enumerate(pitches):
        print("")
        print("Generating mask #{}".format(i + 1))
        print("Pitch [nm]: {}".format(p * 1e9))
        print("...")
        import time
        ...
        if savePath is not None:
            start2 = time.time()
            path = "/home/jerome/dev/masks/"
            name = "mask_b" + str(int(p * 1e6)) + ".tif"
            gr.writeMask(path + name)
            end2 = time.time()
            print("Wrote mask to: {}".format(path + name))
            print("...time taken to save mask (s): {}".format(end2 - start2))
