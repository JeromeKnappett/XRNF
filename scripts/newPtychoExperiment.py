...
def test():
    ...
    Lx = 4.0e-6         # probe size in x
    Ly = 4.0e-6         # probe size in y
    dr  = 45.0e-9       # required resolution
    R = 1.0e-6#260.0e-9     #0.2*Lx          # Distance between probe positions
    A = 3.0e-6**2      # Area to be imaged
    E = 8360            # Photon energy (eV)
    wl = EtoWL(E)
    Np = 2048            # number of detector pixels
    px = 11.0e-6        # pixel size
    ...
    Lx = 4.0e-6         # Aperture size in x
    Ly = 4.0e-6         # Aperture size in y
    dr  = 0.2e-9       # required resolution
...
def SOLEIL():
    # Experimental setup A: Conventional ptychography of a FZP
    Lx = 20.e-6         # probe size in x
    Ly = 20.e-6         # probe size in y
    dr  = 0.2e-9       # required resolution
    R = 400.0e-9 #1.0e-6#260.0e-9     #0.2*Lx          # Distance between probe positions
    A = (36.0e-6)**2#3.0e-6**2      # Area to be imaged
    E = 91.0 # 200.0#92.0#293.0#293.0#92.0#185.0            # Photon energy (eV)
    wl = EtoWL(E)
    Np = 2048#256#1024#2048            # number of detector pixels
    px = 11.0e-6        # pixel size
    dt = 1.0#300.0e-3 # dwell time
    
    Z = 54.9e-3
    
    #   initial / coherence / other orders / magnified
    Ns = (12*3) + (6*6*3)  +  (4*3*3)#   +   (8*3*3)    # total number of scans in experiment
    print(f'Wavelength: {wl}')
    ...
    Zsd = minDistanceForSampling(wl, Lx, px, dims=2)#, overlap=O)
    sr = checkSampling(wl, Zsd, Lx, px)#, overlap=O)
    dXs = reconstructedResolution(wl, Z, Np, px)
    Ws = speckleWidth(wl, Lx, Zsd)
    ...
    print(f'Overlap:                       {O}')
    print(f'Number of positions:           {N}')
    print(f'Scan time:                     {T} min')
    print(f'Number of scans:               {Ns}')
    print(f'Total experiment time:         {Ns*T / 60} hours')
    print(f'                               {(Ns*T / 60)/8} shifts')
    print(f'                               {(Ns*T / 60)/24} days')
    print(' ')
    print(f'Minimum distance for sampling: {Zsd} m')
    print(f' -Sampling Ratio:                {sr}')
    print(f' -Resolution of Resonstruction:  {dXs} m')
    print(f' -Speckle Width:                 {Ws} m')
    print(f'Minimum distance for speckle:  {zmin} m')
    print(f' -Sampling Ratio:                {SR}')
    print(f' -Resolution of Resonstruction:  {DXS} m')
    print(f' -Speckle Width:                 {WS} m')
