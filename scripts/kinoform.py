...
class kinoform:
    ...
    def xj0(self, j):
        ...
        xval = np.sqrt(
            2 * j * (self.m * self.wl) * self.f + j**2 * (self.m * self.wl) ** 2
        )
        return xval
    ...
    def zj(self, j):
        ...
        z = (
            np.sqrt(self.xj(j) ** 2 + self.f**2)
            - np.sqrt(self.xj0(j) ** 2 + self.f**2)
        ) / self.delta
...
def compare():
    ...
    ax.plot(
        np.multiply(xx, 1.0e6), np.multiply(zz, 1e6), label="Kinoform", linewidth=0.1
    )
    ax.plot(xc, np.multiply(zc, 1.0e6), label="other source", linewidth=0.1)
    # ax2 = ax.twinx()
    ...
    fig, ax = plt.subplots()
    ax.plot(np.multiply(xp, 1.0e6), phase, label="Kinoform phase shift", linewidth=0.1)
...
def kinoform2D(
    diameter=15.0e-6, NumberOfZones=36, pixelSize=50e-9, savePath=None, display=False
):
    resolution = pixelSize
...
if __name__ == "__main__":
    ...
    kino, x = kinoform2D(
        ...
        NumberOfZones=36,
        pixelSize=50.0e-9,
        savePath="kinoformR30umPx10nm.npy",
        display=True,
    )
