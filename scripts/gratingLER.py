...
class grating():
    ...
    def __init__(self, 
        ...
        self.xdim = xdim 
        self.ydim = ydim 
        # if rotationCell == 90.0: # added by JK - 30/01/25
        #     self.xres = self.xdim/self.ny
        #     self.yres = self.ydim/self.nx
        # else:
...
#                    Cn = scipy.ndimage.rotate(Cn, self.rotationMask, reshape=True) 
    ...
    def lineEdgeRoughness(self, s, t, dim = 1):
        
        '''
        parameters:
            grating:  2d linear grating
            t (float) –  correlation length line edge error
            s (float) – standard deviation of line edge error
        ...
        Rx, Ry = self.xres*Nx, self.yres*Ny
        x = np.linspace(-Rx/2,Rx/2,Nx) 
        y = np.linspace(-Ry/2,Ry/2,Ny)
        px, py = self.xres, self.yres
        xv, yv = np.meshgrid(x, y, sparse=False, indexing='xy')
        
        mask = (255*self.mask/np.max(self.mask)).astype(np.uint8)
        self.ler = np.zeros_like(mask)
   
       # Get indices for pixels identified by Canny edge detection
        med = np.median(mask) 
        lower = int(max(0 ,0.7*med))
        upper = int(min(255,1.3*med))
        edges = cv2.Canny(mask, lower, upper) > 0
        ...
        if dim==0:
            dx, dy = 1,0
        sobel = cv2.Sobel(mask,cv2.CV_64F,dx,dy,ksize=3)

     
        edges_pos  = sobel<0
        indices_pos = np.argwhere(edges_pos)
        indices=indices_pos'''

        # generare uncorrelated, normally distributed line edge error
        np.random.seed(0) # reproducible - for testing
        #deltas_pos = np.random.normal(size = np.size(indices),  scale=s)
        deltas_neg = np.random.normal(size = np.size(indices),  scale=s/10) # changed from scale=s - JK 30/01/25
        rms = np.sqrt(np.mean(np.square(deltas_neg)))

    
        # now introduce correlation by generating a Gaussian kernel and convolving it with the deltas.
        G = np.exp(-(np.square(x)/(np.square(t)/2)))
        # deltasCorr = signal.fftconvolve(deltas_neg, G, mode='same') # commented out by JK - 30/01/25
        deltas_neg_scaled = deltas_neg * 1e10  # Scale up deltas_neg temporarily to avoid numerical issues - added by JK 30/01/25 
        deltasCorr = signal.fftconvolve(deltas_neg_scaled, G, mode='same')
        deltasCorr = deltasCorr / 1e10  # Scale down after convolution

        plt.plot(deltasCorr)
        plt.show()
        
        # normalise 
        #rms=np.sqrt(np.mean(deltasCorr**2)
        #z = h*z/rms
        
        # now replace points along edges with lines of values -1 or 1 that can be used to
        # define an erosion or dilation of edge.
        if dim==1:
            deltasCorrPix = list(map(int, deltasCorr/px))
            for a, d in zip(indices, deltasCorrPix):   
                if d < 0:      
                    self.ler[ a[0]+d:a[0]+1 ,a[1] ] = 1  #CHANGED a[0] to a[0]+1 - JK
                if d > 0:      
                    self.ler[ a[0]+1:a[0]+d ,a[1] ] = 1  #CHANGED a[0] to a[0]+1 - JK


                                                 
        if dim==0:
            deltasCorrPix = list(map(int, deltasCorr/py))
            for a, d in zip(indices, deltasCorrPix):
                if d < 0:      
                    self.ler[ a[0], a[1]+d:a[1]+1  ] = 1  #CHANGED a[1] to a[1]+1 - JK
                if d > 0:      
                    self.ler[ a[0], a[1]+1:a[1]+d ] = 1   #CHANGED a[1] to a[1]+1 - JK
    ...
    def setRoughness(self, tx=1e-6, ty=1e-6, s=100e-9):
        ...
        wraps diffrace`IO roughness method which implements method of Oglivy (see diffractio docs)
    ...
    def addLineEdgeRoughnessToMask(self,):
        '''adds LER mask amplitude'''
        
        # surely this can be simpler!
        m = np.asarray(self.mask)
        e = np.asarray(self.ler)
        hmsk = np.logical_and(m>0, e==1)
        
        lmsk = np.logical_and(m<np.max(self.mask), e==1)

        self.mask[hmsk] = 0
        self.mask[lmsk] = np.max(self.mask)

        #self.mask = np.multiply(self.mask, self.ler)
        self.updateMaskRange()


    def addRoughnessToMaskEverywhere(self,):
        '''adds roughness amplitude to mask amplitude'''
        self.mask = self.mask + np.substract(self.roughness, np.mean(self.roughness))
        self.updateMaskRange()
        ...
        if dtype == 'uint8':
            if norm == True:
                ...
                if lo < 0:
                    A = A+lo
                A = A-np.min(A)
                # 1. Replace NaNs or Inf:
                #    - "nan=0" means NaNs become 0.
                #    - "posinf=np.finfo(np.float32).max" might be used to clamp positive infinity.
                #    - "neginf=np.finfo(np.float32).min" might be used to clamp negative infinity.
                A = np.nan_to_num(A, nan=0.0, posinf=np.finfo(np.float32).max, neginf=0.0)
                
                # 2. Clamp negative values to 0, if desired:
                A[A < 0] = 0.0
                
                # 3. Compute the max. Check it.
                max_val = np.max(A)
                if max_val > 0:
                    A = A / max_val
                else:
                    pass  # do nothing or set to all zeros
                
                # 4. Scale to 8-bit range [0, 255].
                A = np.uint8(255 * A)
                ...
                imageio.imwrite(
                                path,
                                np.uint8(A),
                                bigtiff=True  # Or pass a dict to 'tifffile' specific plugin if needed
                                )
        else:
            if dtype == 'float32':
    ...
    def showLineEdgeRoughness(self):
        unit=1e6
        plt.imshow(self.ler, 
...
def testGratingSimple(savePath='./', display=True):
    ...
    # create grating object
    gr = grating (nx = 800, ny = 800,
                 xdim = 1.0e-6, ydim = 1.00e-6,
                 height=72e-9/1000000,  #GVR HACK TO MAKE mask and roughness map scales similar
                 blockHeight=255,
                 symmetry = 2,
                 rotationCell = 0.0,
    ...
    gr.lineEdgeRoughness(1e-9, 5e-9, dim=1)

    gr.showMask()
    gr.addLineEdgeRoughnessToMask()


    if display is True:
        #gr.showCell()
        gr.showMask()
        gr.showLineEdgeRoughness()
...
def generateGratingMaskWithLER( mask,
                                savePath='./test/', 
                                lerCor = [1.0e-9, 2.0e-9, 4.0e-9, 6.0e-9, 10.0e-9], 
                                lerStd = [0.1e-9, 0.2e-9, 0.4e-9, 0.6e-9, 0.8e-9], 
                                display=True):
    
   
    
    if  (mask.rotationCell + mask.rotationMask) % 90  == 0:
        dim = 1
    else:
        if (mask.rotationCell + mask.rotationMask) % 90 == 1:
            dim = 0
        else:
            raise ValueError()
     
    mask.generateMask()   # do this once only

    if savePath:
        # write ideal mask (no roughness)       
        mask.writeMask(savePath + 'vert_mask.tif')
    
        # write photon block     
        mask.writeBlock(savePath + 'vert_block.tif')
    
        # Make substrate will integrate this better later...
        mask.write(gr.idealSubstrate(),savePath  + 'vert_substrate.tif',norm=False)
       
        
        # write header to csv file
        with open(savePath + 'parameters.csv', "w") as file:
           #file.write('# Mask parameters.  Num. Heights: {}, Num. Corr. Lengths: {}, path: {}\n'.format(numHeights,numCorrLengths,savePath))
           file.write('index,op_Mask_thick,op_Mask_file_path,op_Block_file_path,lerStd,lerCor,rms\n')
       
    for lerCori in lerCor:
        for lerStdi in lerStd:
            ...
            ler,rms = gr.lineEdgeRoughness(lerStdi, lerCori, dim=dim)
            del ler
            ...
            if savePath is not None:    
                fileStem = savePath +   f'{maskHeight*1e9:.5f}_{lerStdi*1e9:.5f}_{lerCori*1e9:.5f}'  #    str(round(((height/2)*1e9)-1)) + x 
              
                gr.writeMask(fileStem +'_mask.tif')
                    
                line = f"{maskHeight}," +fileStem+"_mask.tif," + savePath + 'vert_block.tif' + f",{lerStdi},{lerCori},{rms}\n"
                print(line)
                with open(savePath + 'parameters.csv', "a") as file:
                    file.write(line)
...
if __name__ == "__main__":
    ...
    #testGrating(savePath='/opt/wpg/wpg/extensionsjk/experiments/maskRoughness/masks/',display=False)
    ...
    import time
    start = time.time()
    r = 3.0e-6 # changed by JK 24/10/24 - from 25.0e-6
    # create grating object - CHANGED TO MATCH GRATINGS USED IN MODEL
    gr = grating(
                 # single grating mask
                 # nx = 4000, ny = 10000,  # changed by JK 24/10/24 - from 8000
                 # double grating photon block
                 nx = 250, ny = 100,
                 xdim = 2.5e-6, ydim = 1.0e-6, # changed by JK 24/10/24 - from 10.0e-6
                 height= 20e-9,  
                 blockHeight=255,
                 symmetry = 2,
                 rotationCell = 0.0,
                 rotationMask= 0, # lines need to be horizontal for LER to work
                 gratingType = 'binary',
                 radius=r, #13.75e-6
                 period=0.1e-6,
                 VERBOSE =True)
    lerC = [10e-9]
    lerS = [4.0E-9]#,0.5e-9,3.5E-9,1.0e-9,3.0E-9,1.5e-9,2.5E-9,2.0e-9]#  [4.0E-9,3.5E-9,3.0E-9,2.5E-9]
    for s in lerS:
        generateGratingMaskWithLER(gr,
                                   savePath='/user/home/opt/xl/xl/experiments/maskLER_retry/masks/block_',
                                   # None,#'/user/home/opt/xl/xl/experiments/BEUVcoherenceLER1/masks_D50/',#'./test/', 
                                   lerCor = lerC, #[ 4.e-9, 8.0e-9, 12e-9, 16e-9, 20e-9], 
                                   lerStd = [s],#,2.0E-9,1.5E-9,1.0E-9,0.5E-9, 0.25E-9], #[1.0e-9, 2e-9, 4e-9, 6e-9, 8e-9], 
                                   display=True)  
    end = time.time()
    print("Time taken to generate masks with LER (s): ", (end-start))
    zEUV = opticalAxisIntercept(0.1e-6,13.5e-9,offset=2*(r))
    zBEUV = opticalAxisIntercept(0.1e-6,6.7e-9,offset=2*(r))
    print('Propagation distance to aerial image plane (wl = 13.5 nm, p =',0.1e-6,' nm): ', zEUV, 'm')
    print('Propagation distance to aerial image plane (wl = 6.7 nm, p =', 0.1e-6,' nm): ', zBEUV, 'm')
    print(" ")