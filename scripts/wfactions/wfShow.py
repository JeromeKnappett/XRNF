...
class showWavefront(Action):
    ...
    def perform_operation(self, *args, **kwargs):
        ...
        if _v['ws_pli'] in ['xy','yx','XY','YX']:
            ...
            I = list(I2D[:,:,0].ravel())  # convert to column-wise (C-aligned) 1D list
        ...
        if _v['ws_plp'] in ['xy','yx','XY','YX']:
            ...
            ''' now plot phase '''
            P2D = wfr.get_phase()
            P = list(P2D[:,:,0].ravel())  # convert to column-wise (C-aligned) 1D list
        ...
        print('nx, ny = {},{}'.format( wfr.params.Mesh.nx,  wfr.params.Mesh.ny))
        print('dx, dy = {},{}'.format( (wfr.params.Mesh.xMax-wfr.params.Mesh.xMin)/wfr.params.Mesh.nx,  (wfr.params.Mesh.yMax-wfr.params.Mesh.yMin)/wfr.params.Mesh.ny))
