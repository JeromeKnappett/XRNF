...
def maskLER(maskFile,res,pitch,shift,extra,resample,wavefront_res,midX=None,midY=None):
...
def test():
    ...
    R = [1000,1000,1000,1000,900,900,900,700,700,700,600,600,600]
    C = [140,120,100,80,70,60,40,70,60,40,70,60,40]
    maskFiles =  ['/user/home/opt_old/LineEdgeRoughness_old/masks/restest_20000020.00000_' + str(c) + '.00000_' + str(r) + '.00000_mask.tif' for c,r in zip(C,R)]

    maskImage = [tifffile.imread(m) for m in maskFiles]
    print(np.shape(maskImage))

    # PARAMETERS FOR RESTEST
    X = 5000-2
    Y = 5000-2
    res = 1.0e-9 
    midX, midY = np.shape(maskImage[0])[1]/2, 4030 #5245#(3*np.shape(maskImage)[0])/17
    pitch = 100.0e-9 / res #pixels
    hp = pitch/2
    
    numlines = 100#int(Y//hp-2)# 125
    shift = 0
    extra = 100
    
    resample = True
    wavefront_res = (9.961053965443777e-10, 1.1737763772365647e-08)

    gratingImage = [m[int(midY - (Y/2)):int(midY + (Y/2)), int(midX - (X/2)):int(midX + (X/2))] for m in maskImage]
    
    # plt.imshow(gratingImage[0])
    # plt.show()
    
    AMP, CL = [],[]    
    for m in gratingImage:
        amp, clen = maskLER(m,res,pitch,shift,extra,resample,wavefront_res)#,midX=midX,midY=midY)
