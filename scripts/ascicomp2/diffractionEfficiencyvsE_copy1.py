...
imageFolder = '/user/home/data/DetectorCom/slsMask90to290ev_50msExpose/'
darkFolder = '/user/home/data/DetectorCom/darkfield_50ms_exposure/'
savePath_dark = darkFolder + 'averaged/'
savePath_images = '/user/home/data/DetectorCom/DE_processed/90to290ev/'
...
verbose = True
show = 88
gratings = [88] #[88,120,160,200]
positions = [bl] #[tl,ml,mr,bl]
save = False
...
inums = [4000,8000,12000,16000,20000,24000,28000,32000,36000,40000,44000,48000,52000,56000,60000,64000,68000,72000,76000,80000,84000,88000,91999]
#[91999] #
...
for e,i in enumerate(inames):
    ...
    if show:
        ...
        ax["Z"].set_title(f"E = {energy_range[e]} eV")        
...
for i88,e88,nm in zip(Eff88,Er88,NM):
    if np.isnan(i88).all():
        ...
        plt.errorbar(energy_range,[_i*100 for _i in i88],yerr=[_e*100 for _e in e88],fmt='.:', 
...
for i120,e120,nm in zip(Eff120,Er120,NM):
    if np.isnan(i120).all():
        ...
        plt.errorbar(energy_range,[_i*100 for _i in i120],yerr=[_e*100 for _e in e120],fmt='.:', 
...
for i160,e160,nm in zip(Eff160,Er160,NM):
    if np.isnan(i160).all():
        ...
        plt.errorbar(energy_range,[_i*100 for _i in i160],yerr=[_e*100 for _e in e160],fmt='.:', 
...
for i200,e200,nm in zip(Eff200,Er200,NM):
    if np.isnan(i200).all():
        ...
        plt.errorbar(energy_range,[_i*100 for _i in i200],yerr=[_e*100 for _e in e200],fmt='.:', 
