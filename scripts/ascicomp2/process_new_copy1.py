...
def test():
    ...
    imageFolder = '/user/home/data/DetectorCom/Spectra_2022_12_11_corse_changed_wbs_offset/'
    darkFolder = '/user/home/data/DetectorCom/Spectra_2022_12_11_course_Darkfieldt/'
    savePath_dark = darkFolder + 'test/'
    savePath_images = '/user/home/data/DetectorCom/BeamProfile_processed/'
    ...
    eLim = (80,280)
    exposures = 20  # chenged from 100 for uncertainty analysis of fits
    exposure_time = 60e-3
    ...
    if justUseFits:
        ...
        if processDarkfield:
            ...
            sumImages(darkFolder,name='Dark_1_',sumNum=500,imgType='.tif',average=True,
                                  darkfield=None,
                                  savePath=savePath_dark,hist=False,verbose=False,show=True)
        if processImages:
            print("\n Summing/Averaging images...")
            ims,iTot,er = sumImages(imageFolder,name='Spectra_1_',sumNum=exposures,imgType=".tif",average=True,
                                    darkfield=savePath_dark + '499.tif',
                                    savePath=savePath_images,hist=False,verbose=False,show=False)
    ...
    if compareSim:
        ...
        # B3pick =  pickle.load(open(simFolder + 'BEUV/SE_cff1.4offset0_LF_sx25sy25_m3.pkl', 'rb'))
        # B3 = B3pick[0]#io.imread(simFolder + 'BEUV/SE_cff1.4offset0_m3_LF.tif')
        # m3res = (B3pick[1],B3pick[2])
        
        # pick135 = pickle.load(open(simFolder + 'SE_135_cff1.4offset0_m1_sx25sy25_highres.pkl', 'rb'))
        # pick250 = pickle.load(open(simFolder + 'SE_250_cff1.4offset0_m1_sx25sy25_highres.pkl', 'rb'))
        ...
        EUVranPix = [(11.0e-6 / EUVres[0]) * xL, (11.0e-6 / EUVres[1]) * yL]  
        BEUVranPix = [(11.0e-6 / BEUVres[0]) * xL, (11.0e-6 / BEUVres[1]) * yL]  
        EUVranPix_SE = [(11.0e-6 / EUVres[0]) * xL, (11.0e-6 / EUVres[1]) * yL]  
        BEUVranPix_SE = [(11.0e-6 / BEUVres[0]) * xL, (11.0e-6 / BEUVres[1]) * yL]  
        ...
        B3eff = 0.00017687707698421477
                #0.001725356423529401*0.1
                # 0.001725356423529401
                
        eff = [00.0837597857974174, 0.07896324231732654, 0.04451154279639219,0.03] # cff = 2, eff = 10%
        ...
        for i in range(0,len(sortedH1)):
            ...
            if includeError:
                ...
                if i % 2 == 0: # int(n) == n:
                    ...
                    # gsum1=gsum1[0:2]
                    # gsum3=gsum3[0:2]
                    # fwx1=fwx1[0:2];fwx3=fwx3[0:2];fwy1=fwy1[0:2];fwy3=fwy3[0:2]
                    # sl1=sl1[0:2];sl3=sl3[0:2]
                    # print(np.mean(isum1))
    ...
    RESULTS = pickle.load(open('/user/home/data/HarmonicContam/Detector_Comission_August_2023/ProcessedImages/BeamProfile_90to350_cff1.4_fixedslits/results.pkl','rb')) 
    ...
    if compareSim and compareFits:
        ...
        plt.errorbar(energy_range0[0:len(iTot)], iTot, yerr=er, fmt='.',label='data')#,fontsize=FSIZE)
    if compareSim:
        plt.plot(90,ItotEUV[0],'x',color='green',markerfacecolor='none')#,label='sim')
        plt.plot(185,ItotBEUV[0],'x',color='green',label='Simulated (run #1)',markerfacecolor='none')#,fontsize=FSIZE)
        plt.plot([90,185,135,250],simF[0:4],'x',color='blue', label='Simulated (run #2)',markerfacecolor='none')#,label='Simulated (n=1)')
        # plt.plot(185,simF[4],'x',color='green',label='Simulated (n=3)')
    if compareFits:
        # pass
        ...
        if includeError:
            ...
            plt.plot(energy_range0[0:20],Isum1[0:20], 'o',color='green', label='Intensity fit (run #1)',markerfacecolor='none')
            # plt.plot(energy_range0[0:21],Isum3[0:21], '^',color='green', label='Intensity fit (n = 3)',markerfacecolor='none')
            plt.errorbar(RESULTS[0][0:len(FSUM)],FSUM,yerr=FSUMer,fmt='o',color='blue',label='Intensity fit (run #2)',markerfacecolor='none')
        plt.plot(RESULTS[1],xpsF,'o',color='black',label='Photodiode',markerfacecolor='none')
    plt.xlabel("Photon Energy [eV]",fontsize=FSIZE)
    plt.ylabel("$\Phi_n$ [ph/s]",fontsize=FSIZE)
    plt.ylim(1e4,3e9)#-1e11,np.max(iTot[0:41]) + 0.25e12)#2.5e12)#np.max(iTot[0:17]))
    # plt.xlim(eLim[0],eLim[1])
    plt.yscale('log')
    ...
    if compareSim and compareFits:
        ...
        plt.plot(energy_range0[0:len(iTot)],gsums,'.',label='data')#,fontsize=FSIZE)
    if compareSim:
        plt.plot(90,IGAeuv[0],'x',color='green',markerfacecolor='none')#,label='sim')
        plt.plot(185,IGAbeuv[0],'x',color='green', label= 'Simulated (run #1)',markerfacecolor='none')#,fontsize=FSIZE)
        plt.plot([90,185,135,250],simG[0:4],'x',color='blue', label='Simulated (run #2)',markerfacecolor='none')
    ...
    if compareFits:
        if includeError:
            ...
            plt.plot(energy_range0[0:20],GSUM1[0:20], 'o', color='green', label='Intensity fit (run #1)',markerfacecolor='none')
            # plt.plot(energy_range0[0:len(Isum1)],GSUM3, '.', color='green',label='n = 3')
            plt.errorbar(RESULTS[0][0:len(GSUM)],GSUM,yerr=GSUMer,fmt='o',color='blue',label='Intensity fit (run #2)',markerfacecolor='none')
    ...
    if compareSim:
        ...
        # plt.plot(185,SB3,'x',color='green',label='Simulated (n=3)')
    ...
    if compareFits:
        ...
        if includeError:
            plt.errorbar(energy_range0[0:len(Isum1)],[f*1e3 for f in FW1x],yerr=FXer1,fmt='o',color='r',label='x (fit)',markerfacecolor='none')#,fontsize=FSIZE)
            plt.errorbar(energy_range0[0:len(Isum1)],[f*1e3 for f in FW1y],yerr=FYer1,fmt='o',color='black',label='y (fit)',markerfacecolor='none')#,fontsize=FSIZE)
        else:
    ...
    plt.xlabel('Photon Energy [eV]',fontsize=FSIZE)
    plt.ylabel('HHC',fontsize=FSIZE)#'$\Phi_{n>1} / \sum_n \Phi_n$')#'Fraction of Harmonic in Total Intensity')
    plt.legend(fontsize=LSIZE)
    plt.show()
    ...
    # elif includeError==False:
        # plt.plot(energy_range0[0:len(Isum1)]), HCtot, 'o',color='black',label='Intensity measurement fits')#,markerfacecolor='none')
        ...
        if compareSim:
            if compareFits:
                results = [energy_range0,
                             ...
                             Seuv, # changed to SE,
                             Sbeuv,
                             EUVfwhmx,EUVfwhmy, # changed to SE,
                             BEUVfwhmx,BEUVfwhmy,
