...
def test():
    ...
    imageFolder = '/user/home/data/HarmonicContam/Detector_Comission_August_2023/Beam_profile_185eV_1.4_Offset2/'
    ...
    savePath_images ='/user/home/data/HarmonicContam/Detector_Comission_August_2023/ProcessedImages/BeamProfile_185eV_offsetTesting/'
    ...
    exposures = 100
    exposure_time = 25.0e-3
    missing_end_files = 1
    energy_range = [185,185,185,185,185,185,185]
    offset = [-4,-3,-2,-1,0,1,2]
    x_axis = offset
    #[e for e in np.arange(120, 180,10)] + [185] + [e for e in np.arange(190, 355,10)]          # energy range for circular polarisation
    #[90,92] + [e for e in np.arange(100, 180,10)] + [185] + [e for e in np.arange(190, 345,10) # full energy range
    eLim = (np.min(x_axis)-np.std(x_axis),np.max(x_axis)+np.std(x_axis)) #(np.min(energy_range)-10,np.max(energy_range)+10)
    ...
    if processImages:
        ...
        imgNums = [e for e in np.arange(exposures,exposures*(len(x_axis)),exposures)] + [exposures*len(x_axis)-missing_end_files]
        # try:
        #     imgNums = [e for e in np.arange(exposures,exposures*(len(x_axis)),exposures)] + [exposures*len(x_axis)-1]
        # except Exception as e:
        #     logger.error('Error: '+ str(e))
        #     pass
        # try:
        #     imgNums = [e for e in np.arange(exposures,exposures*(len(x_axis)),exposures)] + [exposures*len(x_axis)-2]
        # except:
        #     pass
        print(imgNums)
    ...
    if findCenter:    
        ...
        for i in tqdm(ims):
            ...
            # print(energy_range)
            # print(len(cX))
        plt.errorbar(x_axis,[(c-cX[0])*11.0 for c in cX],yerr=5.5,fmt='.',label='x')
        plt.errorbar(x_axis,[(c-cY[0])*11.0 for c in cY],yerr=5.5,fmt='.',label='y')
    ...
    print(len(iTot))
    print(len(ims))
    plt.errorbar(x_axis, iTot, yerr=er, fmt='.',label='data')
    if compareSim:
        plt.plot(90,ItotEUV,'X',color='red',label='sim')
        plt.plot(185,ItotBEUV,'X',color='red')
    if compareFits:
        plt.plot(x_axis,Isum1, '.', label='H1')
        plt.plot(x_axis,Isum3, '.', label='H3')
    plt.xlabel("Photon Energy [eV]")
    ...
    plt.errorbar(x_axis, pTot, yerr=erP, fmt='.',label='data')
    if compareSim:
        plt.plot(90,PtotEUV,'X',color='red',label='sim')
        plt.plot(185,PtotBEUV,'X',color='red')
    if compareFits:
        plt.plot(x_axis,Psum1, '.', label='H1')
        plt.plot(x_axis,Psum3, '.', label='H3')
    plt.xlabel("Photon Energy [eV]")
    ...
    plt.plot(x_axis,gsums,'.',label='data')
    if compareSim:
        plt.plot(90,IGAeuv,'X',color='red',label='sim')
        plt.plot(185,IGAbeuv,'X',color='red')
    if compareFits:
        plt.plot(x_axis,GSUM1, '.', label='H1')
        plt.plot(x_axis,GSUM3, '.', label='H3')
    plt.xlabel("Photon Energy [eV]")
    ...
    plt.plot(x_axis,csums,'.',label='data')
    if compareSim:    
        plt.plot(90,Ceuv,'X',color='red',label='sim')
        plt.plot(185,Cbeuv,'X',color='red')
    if compareFits:
        plt.plot(x_axis,CSUM1, '.', label='H1')
        plt.plot(x_axis,CSUM3, '.', label='H3')
    plt.xlabel("Photon Energy [eV]")
    ...
    plt.plot(x_axis,S,'.',label='data')
    if compareSim:
        plt.plot(90,Seuv,'X',color='red',label='sim')
        plt.plot(185,Sbeuv,'X',color='red')
    if compareFits:
        plt.plot(x_axis,S1, '.', label='H1')
        plt.plot(x_axis,S3, '.', label='H3')
    plt.xlabel("Photon Energy [eV]")
    ...
    # plt.plot(x_axis,fwhmX,'.:',label='x')
    # plt.plot(x_axis,fwhmY,'.:',label='y')
    plt.plot(x_axis,[f*1e3 for f in fwXs[0:60]], '.',label='x (data)')
    plt.plot(x_axis,[f*1e3 for f in fwYs[0:60]], '.',label='y (data)')
    if compareSim:
    ...
    # plt.plot(x_axis,[f*1e3 for f in FW1x], '.', label='H1 x')
    # plt.plot(x_axis,[f*1e3 for f in FW1y], '.', label='H1 y')
    # plt.plot(x_axis,[f*1e3 for f in FW3x], '.', label='H3 x')
    # plt.plot(x_axis,[f*1e3 for f in FW3y], '.', label='H3 y')
    if compareFits:
        plt.plot(x_axis,FWHM1x, '.', label='H1 x')
        plt.plot(x_axis,FWHM1y, '.', label='H1 y')
    # plt.plot(x_axis,FWHM3x, '.', label='H3 x')
    # plt.plot(x_axis,FWHM3y, '.', label='H3 y')
    plt.xlabel("Photon Energy [eV]")
