...
def test():
    ...
    imageFolder = '/user/home/data/HarmonicContam/Detector_Comission_August_2023/Beam_profile_120_to_350eV_circ_polarisation_fixed_slits/'
    ...
    savePath_images ='/user/home/data/HarmonicContam/Detector_Comission_August_2023/ProcessedImages/BeamProfile_120to350_CircPol_fixedslits/'
    ...
    if processImages:
        ...
        ims,iTot,er = sumImages(imageFolder,name='beam_2_',sumNum=exposures,imgType=".tif",average=True,
    ...
    if compareSim:
        ...
        EUVpick = pickle.load(open(simFolder + 'EUV/atMask_cff2offset0_m1_LF_sx45sy30.pkl','rb')) # 'EUVbeamProfile/data/m1_atMask_ME/m1_atMask_ME.pkl', 'rb')) #'EUVbeamProfile/data/check2/atMask_ME/atMask_ME.pkl', 'rb'))
        EUVres = (EUVpick[1],EUVpick[2])
        EUVtiff = EUVpick[0] 
        EUVtiff_SE =  io.imread(simFolder + 'EUV/SE_cff2offset0_m1_LF_sx45sy30.tif')
        BEUVpick = pickle.load(open(simFolder + 'BEUV/atMask_cff2offset0_m1_LF_sx45sy30.pkl', 'rb')) #'BEUVbeamProfile/data/m1_atMask_ME/m1_atMask_ME.pkl', 'rb'))
        ...
        print('\n')
        print(bw)
        BEUVtiff = BEUVtiff * (bw)
        BEUVtiff_SE = BEUVtiff_SE * (bw)
        EUVtiff = EUVtiff * (bw)
        EUVtiff_SE = EUVtiff_SE * (bw)
        ...
        EUVfwhmxSE,EUVfwhmySE = getFWatValue(EUVtiff_SE,frac=0.5,dx=EUVres[0],dy=EUVres[1],cuts='x',
                                             centered=True,smoothing=None,
                                             verbose=False,show=True)
    ...
    if compareFits:
        ...
        for i in range(0,len(sortedH1)):
            ...
            i1 = photonsPs2counts(i1, energy_range[i+3], t=exposure_time, conversion=1.27)
            i3 = photonsPs2counts(i3, energy_range[i+3], t=exposure_time, conversion=1.27)
            # converting from counts to ph/s and accounting for transmission through ultralene filter
            i1 = counts2photonsPs(i1,energy_range[i+3],t=exposure_time,conversion=1.27) / T1[i]
            i3 = counts2photonsPs(i3,energy_range[i+3]*hN,t=exposure_time,conversion=1.27) / T2[i]
            ...
            pI1 = intensity2power(i1,energy_range[i+3])
            pI3 = intensity2power(i3,energy_range[i+3]*hN)
    ...
    if compareFits:
        plt.plot(energy_range[3:len(Isum1)+3],Isum1, '.', color='blue', label='n = 1')
        plt.plot(energy_range[3:len(Isum1)+3],Isum3, '.', color='green', label='n = 3')
    ...
    if compareFits:
        plt.plot(energy_range[3:len(Isum1)+3],Psum1, '.', label='n = 1')
        plt.plot(energy_range[3:len(Isum1)+3],Psum3, '.', label='n = 3')
    plt.xlabel("Photon Energy [eV]")
    ...
    if compareFits:
        plt.plot(energy_range[3:len(Isum1)+3],GSUM1, '.', color='blue',label='n = 1')
        plt.plot(energy_range[3:len(Isum1)+3],GSUM3, '.', color='green',label='n = 3')
    plt.xlabel("Photon Energy [eV]")
    ...
    if compareFits:
        plt.plot(energy_range[3:len(Isum1)+3],CSUM1, '.', label='n = 1')
        plt.plot(energy_range[3:len(Isum1)+3],CSUM3, '.', label='n = 3')
    plt.xlabel("Photon Energy [eV]")
    ...
    if compareFits:
        plt.plot(energy_range[3:len(Isum1)+3],S1, '.:',color='blue', label='n = 1')
        plt.plot(energy_range[3:len(Isum1)+3],S3, '.:',color='green', label='n = 3')
    plt.xlabel("Photon Energy [eV]")
    ...
    if compareFits:
        plt.plot(energy_range[3:len(Isum1)+3],[f*1e3 for f in FW1x], '.:',color='r', label='x (fit)')
        plt.plot(energy_range[3:len(Isum1)+3],[f*1e3 for f in FW1y], '.:',color='black', label='y (fit)')
    ...
    XPS = pickle.load(open('/user/home/data/HarmonicContam/XPS/HF (circpol).pkl', 'rb'))
    XPSe = [e for e in np.arange(130,185,20)] + [185] + [e for e in np.arange(190,345,20)]
    
    XPS_MFP = pickle.load(open('/user/home/data/HarmonicContam/XPS/HF (circpol)_MFP.pkl', 'rb'))
    ...
    plt.plot(energy_range[3:len(Isum1)+3],[g3/(g1 + g3) for g1,g3 in zip(Isum1,Isum3)],'.',color='black',label='Total I')
    plt.plot(energy_range[3:len(Isum1)+3],[g3/(g1 + g3) for g1,g3 in zip(GSUM1,GSUM3)],'.',color='blue',label='I over grating')
