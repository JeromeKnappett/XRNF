...
plt.rcParams["figure.figsize"] = (6,4) # added by GVR to make plots bigger and easier to read
...
def getVisibility(Iprofile,n, showPlot=True):
    ...
    """
    if showPlot == True:
        plt.plot(Iprofile[int(len(Iprofile)/2-n)-20:int(len(Iprofile)/2+n)-20])  #plotting central fringe
...
def ydsInterference(x,u,I,a,b,z,lam,dlam,delta,dx=0, showPlot=False):
    ...
    if showPlot:
        ...
        plt.plot(x*1e3,eF/np.max(eF), label="envelope", color=colours[1])   #eF*np.max(Ix)
        plt.plot(x*1e3,(eF/np.max(Ix))*(1-(np.max(Ix)-np.max(eF))), color=colours[1], linestyle='--')#, label="envelope")
...
def testYDSfitAuto():
    ...
    for n in range(0,numRuns):
        ...
        ranges = [range(fileStart + sum(aqus[0:i]), fileStart+sum(aqus[0:i+1])) for i in range(0,len(aqus))]
        fileStart += sum(aqus)
...
#            print(ran[n])
    ...
    dark_fields = [tiffPath + 'darkFields/M17186_13' + str(i) + '.tif' for i in range(602,702)] 
    ...
    Ip1 = prepYDSTiffs(Itiffs = tiff_f,            # array of intensity tiff paths
                      ...
                      path = None)                 # Save path for data (summed tiff files)
    ...
    p0 = [V,                                        # u-degree of coherence
            ...
            0.5e-4]                                 # dx- horizontal drift (+: left, -: right)
    
    # IF PARAMETER IS KNOWN EXACTLY PUT 1
    exact = [0,                                     # u-degree of coherence
            ...
            0]                                      # dx- horizontal drift
    

    v1, c1 = fitYDSInterference(Ip1,
                                detRange=dR, 
                                iG = p0,
                                known = exact,
                                savePath = None)# path + 'plots/' ) # need to manually create the 'plots' folder or else put None
...
def testI():
    p = 11e-6
    Xi = 0.008*p #0.008
    res = 11.0e-6 #5e-6/2 #1e-6*2
    Xrange = 2048*res #0.08 #Xi #*25
