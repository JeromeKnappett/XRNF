...
def gratingEfficiencyDEL(m, beta, delta, d, k, b, theta=0):
    ...
    F_0 = (
        (1 / k * R_0)
        * np.exp(
            -1j * np.cos((beta * C_0) / R_0)
            + 1j * k * (Z_0 - C_0 * X_0) * (delta + 1j * beta)
        )
        * (
            np.exp(1j * X_1 * (k * C_0 * (delta + 1j * beta) + phi))
            - (np.exp(1j * X_0 * (k * C_0 * (delta + 1j * beta) + phi)))
        )
    )
    F_1 = (
        (1 / k * R_1)
        * np.exp(
            -1j * np.cos((beta * C_1) / R_1)
            + 1j * k * (Z_1 - C_1 * X_1) * (delta + 1j * beta)
        )
        * (
            np.exp(1j * X_2 * (k * C_1 * (delta + 1j * beta) + phi))
            - (np.exp(1j * X_1 * (k * C_1 * (delta + 1j * beta) + phi)))
        )
    )
    F_2 = (
        (1 / k * R_2)
        * np.exp(
            -1j * np.cos((beta * C_2) / R_2)
            + 1j * k * (Z_2 - C_2 * X_2) * (delta + 1j * beta)
        )
        * (
            np.exp(1j * X_3 * (k * C_2 * (delta + 1j * beta) + phi))
            - (np.exp(1j * X_2 * (k * C_2 * (delta + 1j * beta) + phi)))
        )
    )
    F_3 = (
        (1 / k * R_3)
        * np.exp(
            -1j * np.cos((beta * C_3) / R_3)
            + 1j * k * (Z_3 - C_3 * X_3) * (delta + 1j * beta)
        )
        * (
            np.exp(1j * X_4 * (k * C_3 * (delta + 1j * beta) + phi))
            - (np.exp(1j * X_3 * (k * C_3 * (delta + 1j * beta) + phi)))
        )
    )
...
def gratingEfficiencyHARV(m, b, d, G=0):
    ...
    if G == 0:
        ...
        if b != d / 2:
            print(
                "Error: Ratio of slit size to periodicity must be 1:2 for phase grating"
            )
            import sys
...
def plotEfficiency():
    ...
    for s in slit:
        ...
        # sEff_0.append(E0)
        E1 = gratingEfficiencyHARV(1, 1e-9 * s, 200e-9, G=0)
        sEff_1.append(E1)
        E2 = gratingEfficiencyHARV(2, 1e-9 * s, 200e-9, G=0)
        sEff_2.append(E2)
        E3 = gratingEfficiencyHARV(3, 1e-9 * s, 200e-9, G=0)
        sEff_3.append(E3)
        E4 = gratingEfficiencyHARV(4, 1e-9 * s, 200e-9, G=0)
        sEff_4.append(E4)
...
def getEfficiency(
    ...
    if g == 1:
        intV = int(
            s2[0] / ((2 * m) + 1)
        )  # 500 # Number of pixels for segmentation interval
    elif g == 2:
        intV = 1150  # int(s2[0]/((4*m)+1))
    ...
    if g == 1:
        ...
        plt.clf()
        plt.close()
        plt.plot(A_0, label="m=0", color="b")
        plt.plot(A_1, label="m=+1", color="g")
        #        plt.plot(A_n1, label='m=-1', color='g')
        try:
        ...
        if m == 1:
            return E0, E1, En1, rE0, rE1, rEn1
        elif m == 2:
            return E0, E1, En1, E2, En2, rE0, rE1, rEn1, rE2, rEn2
        elif m == 3:
            return E0, E1, En1, E2, En2, E3, En3, rE0, rE1, rEn1, rE2, rEn2, rE3, rEn3
        elif m >= 4:
            return (
                E0,
                E1,
    ...
    if g == 2:
        ...
        if m >= 2:
            print(
                "Error: Efficiency calculations for higher orders from binary grating mask is not yet possible"
            )
            import sys
        ...
        plt.clf()
        plt.close()
        plt.plot(A_AE, label="m=+-1")
        plt.plot(A_0r, label="m=0 (right grating)")
        plt.plot(A_0l, label="m=0 (left grating)")
        plt.plot(A_1, label="m=+1 (right grating)")
        plt.plot(A_n1, label="m=-1 (left grating)")
        ...
        return EAE, E0r, E0l, E1, En1, rEAE, rE0r, rE0l, rE1, rEn1
...
def test():
    ...
    I2 = getImageData(
        "/data/maskTest/thickness/intensityPR_TH500.tif"
    )  # getImageData('/hom
...
def testReshape():
    I = getImageData(
        "/home/jerome/Documents/MASTERS/data/wavefields/Efficiency/intensityPR_1-4.tif"
    )
    plt.imshow(I)
    plt.title("Intensity")
