...
def makeYDS(nx, ny, xdim, ydim, sSep, sWidth, sLen, rotation, path=None):
    ...
    if rotation == 0:
        plt.imshow(
            mesh[sLocX[0] - pad : sLocX[1] + pad, sLocY[0] - pad : sLocY[1] + pad]
        )
    else:
        plt.imshow(
            scipy.ndimage.rotate(mesh, 360 - rotation)[
                sLocX[0] - pad : sLocX[1] + pad, sLocY[0] - pad : sLocY[1] + pad
            ]
        )
    ...
    sigma = (
        2 / 3
    )  # rX/(2*np.sqrt(8*np.log(2))) #(1/6)*((nx*3)+1) #rX/(2*np.sqrt(8*np.log(2)))
    ...
    plt.title("YDS")
    plt.xticks(
        [0, int(nx / 4), int(nx / 2), int(3 * nx / 4), nx],
        [-xdim / 2, -xdim / 4, 0, xdim / 4, xdim / 2],
    )
    plt.yticks(
        [0, ny / 4, ny / 2, 3 * ny / 4, ny],
        [-ydim / 2, -ydim / 4, 0, ydim / 4, ydim / 2],
    )
    plt.show()
...
def testYDS():
    ...
    numRots = 3
    numSeps = 9
    theta = np.linspace(0, 90, numRots)
    sep = np.linspace(25e-6, 225e-6, numSeps)

    path = "/opt/xl/xl/experiments/masks/testYDS"
    ...
    for n, s in zip(range(numSeps), sep):
        ...
        for i, t in zip(range(numRots), theta):
            ...
            p = str(
                path + "s" + str(int(np.rint(s * 1e6))) + "t" + str(int(t)) + ".tif"
            )  # save path
