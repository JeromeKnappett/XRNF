...
dirPath = "/home/jerome/dev/data/MonochromatorEfficiency/"
#'/home/jerome/dev/experiments/beamCoherence2/data/'

trials = ["beforeMono/m5/", "afterMono/m5_n1/", "afterMono/m5_n3/", "afterMono/m5_n5/"]
# ['beforeMono/m1/','afterMono/m1_n1/']
# ['beforeMono/m3/','afterMono/m3_n1/','afterMono/m3_n3/']
mePR = "res_int_pr_me.dat"
sePR = "res_int_pr_se.dat"
...
for t in trials:
    ...
    #    numC = int(str(np.loadtxt(dirPath+t+sePR, dtype=str, comments=None, skiprows=10, max_rows=1, usecols=(0)))[1:])#[1:3]
    ...
    I = np.reshape(
        np.loadtxt(dirPath + t + sePR, skiprows=10), (1, int(ny), int(nx))
    )  # Propagated multi-electron intensity
    Iflat = I.flatten()

    Isum = np.sum(I)

    print(" ")
    print("Intensity Sum: {}".format(Isum))
    #    print(Isum)
    ...
    try:
        ...
        I0 = np.reshape(
            np.loadtxt(dirPath + t + "res_int_se.dat", skiprows=10), (int(ix), int(iy))
        )  # Initial intensity
    except OSError:
        print("No initial intensity file found...")
    except ValueError:
        print(
            "Unexpected initial dimensions - Initial intensity file could not be reshaped"
        )
        pass
    # try:
    ...
    try:
        cX = np.reshape(
            np.loadtxt(dirPath + t + "res_int_pr_me_dcx.dat", skiprows=11),
            (int(nx), int(nx)),
        )  # Horizontal coherence
        cY = np.reshape(
            np.loadtxt(dirPath + t + "res_int_pr_me_dcy.dat", skiprows=11),
            (int(ny), int(ny)),
        )  # Vertical coherence
    ...
    try:
        miX = np.reshape(
            np.loadtxt(dirPath + t + "res_int_pr_me_mix.dat", skiprows=11),
            (int(nx), int(nx), 2),
        )  # Horizontal mutual intensity
        miY = np.reshape(
            np.loadtxt(dirPath + t + "res_int_pr_me_miy.dat", skiprows=11),
            (int(ny), int(ny), 2),
        )  # Vertical mutual intensity
    except OSError:
        print("No mutual intensity files found...")
    except ValueError:
        print(
            "Unexpected initial dimensions - Mutual intensity files could not be reshaped"
        )
        pass
    ...
    try:
        ...
        plt.plot(
            cX[:, int(int(nx) / 2)], label="vertical"
        )  # , '.', markersize = 1, label="vertical")
        plt.plot(
            cX[int(int(nx) / 2), :], label="horizontal"
        )  # , '.', markersize = 1, label="horizontal")
        plt.plot(
            np.diagonal(np.squeeze(cX)), label="diagonal"
        )  # , '.', markersize = 1, label="diagonal")
        plt.plot(
            (I[0, int(int(ny) / 2), :] / np.max(I[0, int(int(ny) / 2), :])),
            label="intensity",
        )  # , '.', markersize = 1, label="intensity")
        ...
        plt.plot(
            cY[:, int(int(ny) / 2)], label="vertical"
        )  # , '.', markersize = 1, label="vertical")
        plt.plot(
            cY[int(int(ny) / 2), :], label="horizontal"
        )  # , '.', markersize = 1, label="horizontal")
        plt.plot(
            np.diagonal(np.squeeze(cY)), label="diagonal"
        )  # , '.', markersize = 1, label="diagonal")
        plt.plot(
            (I[0, :, int(int(nx) / 2)] / np.max(I[0, :, int(int(nx) / 2)])),
            label="intensity",
        )  # , '.', markersize = 1, label="intensity")
