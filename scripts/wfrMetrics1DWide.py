...
class wfInterferencePatternProperties1DWide(action.Action):
    """
            ...
            for prop, W in zip(["intensity"], [I]):
                ...
                for axis, index in zip(["y"], [1]):
                    try:
                        ...
                        IOD, IODM, H, binEdges, binCenters = gm.integralOpticalDensity(
                            prof
                        )
                        Cf, Am, Fr, peakFr = gm.gratingContrastFourier(
                            prof, axisVals, show=True
                        )
