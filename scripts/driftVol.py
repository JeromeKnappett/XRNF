...
from beamlineExt import oeResample
from wpg.optical_elements import Use_PP, Empty


'''
Below import or define wrapper functions for post-processing wavefield after propagation to each z-step
'''
from wfrMetrics import wfrPropertiesInterference


def postProcFunc(fun, *args ):
...
class driftVol():
    ...
    def __init__(self, 
        ...
        param: postProcFunction is a list of function that will be passed the wavefield propagated to each distance in z-range.
        param: region of interest specified by corners ((x0,y0),(x1,y1)) passed as parameter to functions listed in postProcFunctions
        """     
        ...
        if outputPath is not None:
            ...
            try: 
                self.pathMetrics = os.path.join(self.outPath,'metrics')
                os.makedirs(self.pathMetrics)
        ...
        self.fun = postProcessFunction
        if self.fun is not None:
            print('Functions to be called to process wavefield at each plane:')
            for f in self.fun:
                print(f.__name__)
    ...
    def saveMetrics(self):
        filename = os.path.join(self.pathMetrics, 'summary.pickle')
        fileObject = open(filename, 'wb')
        pkl.dump(self.metrics, fileObject)
        fileObject.close()
    ...
    def propagateToZ(self,wf0,z,index,propagateCopy):
       ...
       self.drift(z, wf)
       self.resample(wf)
       dataSavePath = self.save(wf, index, z)
       self.processWF(wf,index,z,dataSavePath)
       if copy==True: 
           del wf
    ...
    def processWF(self,wf,index,z,dataPath):

       args = [self.pathMetrics,  wf, index, z, dataPath]
       if self.fun is not None: # added by JK
           for f in self.fun:
               #print('Calling function {}, with {} args in list.'.format(f.__name__.np.size(args)))
               self.metrics.append(postProcFunc(f,*args))
...
def testBeamDivergence():
   ...
   # initialWavefront='/opt/wpg/wpg/extensions/in/50Slices63dLTZP_simulationData/wf_noCS.h5'
    initialWavefront='/user/home/opt/xl/xl/experiments/focusedIL/data/maskExitUF/wf_final.hdf'
    outPath='/user/home/opt/xl/xl/experiments/focusedIL/data/maskExitUF/driftVol'
