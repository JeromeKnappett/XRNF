   ...
   # create grating object
    ...
    for x,L,clx in zip(inB,range(numCorrLengths),c_length):
        for height in Rh:
            ...
            fileStem = savePath +  '{height*1e9:.5f}_{clx*1e9:.5f}_{cly*1e9:.5f}'  #    str(round(((height/2)*1e9)-1)) + x 
            ...
            line = "{L},{maskHeight},{height},{sigma},{rmsRough},{clx},{cly}," +fileStem+"_cell.tif," + savePath + '_rough.tif' + "\n"
...
def testMultiGrating(savePath='./', display=True):
    ...
    # specify gratings geometry
    #p = 500e-9 #[10e-9, 14e-9, 18e-9, 22e-9, 26e-9, 30e-9, 34e-9, 38e-9]
    #Nx = 1000
    #Ny = 1000
    #xDim = 0.5e-6
    #yDim = 10e-6
    ...
    #rotG = 0.0
    #rotM = 90.0
    #r =[20e-6, 30e-6, 40e-6] # np.linspace(25e-6, 75e-6, numRads) #11.875e-6
    
    # specify number of gratings and define geometry
    numGratings = 8
    #pitches = np.linspace(20e-9, 40e-9, numGratings)
    Nx = 500
    Ny = 500
    xDim = 10e-6
    yDim = 10e-6
    ...
    pitches = [40e-6]#[10e-9, 14e-9, 18e-9, 22e-9, 26e-9, 30e-9, 34e-9, 38e-9] #np.linspace(10e-9, 36e-9, numGratings)
    ...
    for i, a in enumerate(pitches):
        ...
        print("Pitch [nm]: {}".format(a*1e9))
        print("...")
        import time
        start = time.time() 
        # create grating object
        gr = grating (nx = Nx, ny = Ny,
                     xdim = xDim, ydim = yDim,
                     height=72e-9/1000000,  #GVR HACK TO MAKE mask and roughness map scales similar
                     blockHeight=255,
                     symmetry = sym,
        ...
        if savePath is not None:
            start2 = time.time()
            path = '/opt/xl/xl/experiments/masks/'
            name = 'mask_p' + str(a*1e9)[0:len(str(a*1e9))-2] + '.tif'
            gr.writeMask(path + name)
            end2 = time.time()
