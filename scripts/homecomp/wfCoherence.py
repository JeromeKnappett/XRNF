...
from useful import round_sig, sampleField, fromPickle
...
def mutualIntensity(cfrT,cfrH,cfrV, Fx, Fy, dx=1, dy=1, pathJP=None, pathJ=None, verbose=False, show=False):
    ...
    A_T = sampleField(cfrT, Fx=Fx, Fy=Fy, limit = 100,verbose=verbose,show=show)
    A_H = sampleField(cfrH, Fx=Fx, Fy=Fy, limit = 100)
    A_V = sampleField(cfrV, Fx=Fx, Fy=Fy, limit = 100)
    ...
    # Testing new methods for calculating mutual intensity arrays
    
    # Mutual intensity with respect to central pixel of electric field array
    # J_test = np.array(A_T.conjugate()*A_T[int(np.shape(A_T)[0]/2),int(np.shape(A_T)[1]/2)])
    
    # Normalised spatial coherence array in a single step
    # gamma = np.array([((A_T.conjugate()*a)/(np.sqrt((abs(A_T)**2)*(abs(a)**2)))) for a in A_T.flatten()]) 
    gamma = np.array([((A_T.conjugate()*a)/(i)) for a,i in zip(A_T.flatten(),I.flatten())]) 
        ...
        for b in a:
            for c in A_T:
                for d in c:
                    ...
                    test = (b.conjugate()*d)/(np.sqrt((abs(b)**2)*(abs(d)**2)))
                    
                    J_test.append(test)
    
    J_test = np.reshape(J,(np.shape(A_T)[0]*np.shape(A_T)[1], np.shape(A_T)[0], np.shape(A_T)[1]))
    ...
    if verbose:
        ...
        print("J (test): {}".format(np.shape(J_test)))
        ...
        print("gamma: {}".format(np.shape(gamma)))
        
        print(f"Shape of Intensity array: {np.shape(I)}")
    
    """ Averaging each Mutual Intensity Array (& Normalising J)"""
    # THIS STEP NEEDS MORE THOUGHT (Normalising by the entire I array leads to a strange result for j, 
    #                               normalising by the central intensity value leads to a sensible result for j) 
    j = abs(J.mean(0))    #[int(np.shape(I)[0]/2),int(np.shape(I)[1]/2)]) # This line needs fixing
    jxx = abs(Jxx.mean(0))
    jxy = abs(Jxy.mean(0))
    jyx = abs(Jyx.mean(0))
    jyy = abs(Jyy.mean(0))
    ...
    jnorm = np.divide(j,I) #array([a/i for a in [b for b in j] for i in [z for z in I]])
    # print(np.shape(jnorm))
    # jNorm = np.reshape(jnorm, (np.shape(j)[0], np.shape(j)[1]))
    
    jtest = abs(J_test.mean(0))
    ...
    if show:
        ...
        plotTwoD(I, dx=dx, dy=dy, title='Intensity - I',
                 xLabel="x position [$\mu$m]",
                 yLabel="y position [$\mu$m]",
                 savePath=pathJ)
        
        plotTwoD(abs(jnorm), dx=dx, dy=dy, title='Mutual Intensity - J',
                 xLabel="Point Separation x-x' [$\mu$m]",
                 yLabel="Point Separation y-y' [$\mu$m]",
                 savePath=pathJ)
        
        plotTwoD(abs(g), dx=dx, dy=dy, title='Spatial Coherence - $\gamma$',
                 xLabel="Point Separation x-x' [$\mu$m]",
                 yLabel="Point Separation y-y' [$\mu$m]",
                 savePath=pathJ)
        
        plotTwoD(jtest, dx=dx, dy=dy, title='Spatial Coherence - $J - test$',
                 xLabel="Point Separation x-x' [$\mu$m]",
                 yLabel="Point Separation y-y' [$\mu$m]",
                 savePath=pathJ)
        
        # plt.imshow(abs(J_test))#, vmin=0, vmax=1)
        # plt.colorbar()
        # plt.show()
...
def test():
    ...
    cw = getComplex(wf0)
    cwH = getComplex(wf0,'horizontal')
    cwV = getComplex(wf0,'vertical')
    I = wf0.get_intensity()
...
def testCoherenceProfile():
    ...
    gy = 10e-6
    # path = 'wavefield_1.pkl'
    path = '/home/jerome/dev/data/coherence/efields/correctedWBS/'
    #slit sizes]
    sX = [150,250]
    sY = [250,250]
    ...
    for ce, ch, cv, s, dX, dY in zip(cE,Eh,Ev,sX, dx, dy): 
        ...
        J,Jxx,Jxy,Jyx,Jyy = mutualIntensity(ce,ch,cv,Fx=0.01,Fy=0.01,dx=dX,dy=dY,verbose=False,show=plot)
        
        s1,s2,s3,s4 = stokesFromJ(Jxx, Jxy, Jyx, Jyy,verbose=False,show=plot)
        ...
        # print(np.shape(ex))
        cH, cV, jH, jV, jHh, jVh, fx, fy = profileMI(ex,ey,dx=dX,dy=dY,verbose=False,show=plot)

        Cx = getCoherenceFromProfile(ex, verbose=False, show=plot)
        ...
        clx_1 = getCoherenceLength(0.8,cH = np.array(Cx),dx=dX, verbose=False, show=plot)*dX
        cly_1 = getCoherenceLength(0.8,cV = np.array(Cy),dy=dY)*dY
        
        # coherence length from MI function
        clx_2 = getCoherenceLength(0.8,cH = np.array(cH),dx=dX, verbose=False, show=plot)*dX
