...
#    
    ...
    return meanNILS,  meanLW, NILS, LW, rmsNILS, rmsLW, rmsdLW, p1
...
#    m = np.squeeze(_m)
#    print(argrelextrema(np.abs(s), np.greater, order=10))
#    print(np.squeeze(argrelextrema(np.abs(s), np.greater, order=10)))
#    print('DEBUG: m ', np.shape(m))
#    print(np.array(m).ndim)
#    print(m)
...
#    C = (np.sqrt(2))*((2*(abs(ymax)**2)) - abs(fmax)**2)/(abs(fmax)) 
...
def integralOpticalDensity(I,bins=256):
    ...
    '''
    bins = len(set(I))
#    print('Number of gray levels: {}'.format(bins))
...
def test():
    ...
    m1 = 67045 #21159 #15919
    m2 = 70348 #16377
    
    n = 1
    
    # profile = np.load('profile01.npy')
    Ipath = '/user/home/opt/xl/xl/experiments/fullbeamPolarisation2/data/'
    files =  ['TM/TMintensity.tif'] 
#    ['hor/horintensity.tif', 'TM/TMintensity.tif']# ['4000e_bx/4000e_bxintensity.tif']#, 'fullRes_2000eintensity.tif', 'fullRes_3000eintensity.tif', 'fullRes_4000eintensity.tif', 'fullRes_5000eintensity.tif', 'fullRes_10000eintensity.tif']
    ...
    files1 = ['TE/TEintensity.tif']
#    ['ver/verintensity.tif', 'TE/TEintensity.tif'] #['4000e_by/4000e_byintensity.tif']#, 'fullRes_1000e_TM.tif']
    ...
    res1 = 4.77529829067939e-10 #2.1488261831535813e-9 
    res2 = 6.46327159071368e-10 #1.9651701737063605e-9 # (3.924772866734718e-05 + 4.546782781154186e-05)/39424
...
#    tiffsV = [tifffile.imread(Ipath + f) for f in filesV]
    ...
    plt.imshow(tiffs[0][mid1-n:mid1+n,:], aspect='auto')
    plt.colorbar()
    plt.show()
...
#    labelsV = ['4000e']
    labels = ['TM']# ['100e', '2000e', '3000e', '4000e', '5000e', '10000e']
    #['TM']
    #['100e', '2000e', '3000e', '4000e', '5000e', '10000e']
    labels1 = ['TE'] # ['1000e', '1000e - TM']
    #['TE'] 
    #['1000e', '1000e - TM']
    ...
    profileXs = res1# 1.9651759267701173e-09 #1.9651759267701173e-09 #1.0e-8 # pixel scaling factor/
    ...
    profileX1s = res2 #(3.924772866734718e-05 + 4.546782781154186e-05)/39424
...
#    xV =  np.linspace(-profileVs*int(profileV/2),profileVs*int(profileV/2),profileV) 
    ...
    rho=4*1/xstep
    ''' generate interference intensity'''
    I = interferenceIntensity(x,k,theta,A=A, polarisationModes=('TM','TM'))
    I1 = interferenceIntensity(x1,k,theta,A=A, polarisationModes=('TE','TE'))
...
#    I = gaussian_filter(I, sigma=sigma)
    ...
    plt.plot(profileX1,aerialImages1[0], label=labels1[0])
    plt.legend()
...
#        plt.plot(profileV1*1e6, b, label=labelsV[i])
    ...
    nils1 = [NILS(a,profileX, d/4, show=True) for a in aerialImages] 
    nils2 = [NILS(b,profileX1, d/4, show=True) for b in aerialImages1] 
    ...
    # # Testing contrast metrics    # fourierC = [gratingContrastFourier(a,profileX*1e6, show=True) for a in aerialImages] #Cf,  Am, Fr, peakFr
...
#    fidel.insert(1,fidel1[1])
    ...
    ax2.plot(labels, [n[0] for n in nils1], ':', label='mean NILS')
    ax2.plot(labels, [n[4] for n in nils1], ':', label=' rms NILS')
...
def testTETMContrast():
    ...
    k = 2*np.pi/wl
    m = 1 # order of diffracted beams from each grating
    pitch = [14e-9,24e-9,40e-9] #[round_sig(n,sig=5) for n in [10e-9,12.5e-9,15e-9,20e-9,30e-9,50e-9,100e-9]] #np.linspace(10e-9,100e-9,10)]#24e-9 #24e-9 #100e-9 # grating spacing
    print(pitch)
...
#    plt.plot(I_TM[0])
    ...
    for p, ite, itm in zip(pitch, np.array(I_TE), np.array(I_TM)):
        ...
        axs[1].plot(x,ite, label='$p_G$ = ' + str(int(np.round(1e9*p))) + ' nm')
    ...
    axs[1].legend(loc='lower center', #title='Grating Pitch [nm]',
                  bbox_to_anchor=(0.5, -0.8),fancybox=True, shadow=True,ncol=4)
    # plt.legend()        
    if save:
...
def testNoise():
    ...
    # Amplitude of both beams (assumed equal)
    A = 1/2 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensitalsey
    k = 2*np.pi/wl
    m = 1 # order of diffracted beams from each grating
    pitch = 100e-9 #[round_sig(n,sig=5) for n in np.linspace(10e-9,100e-9,10)]#24e-9 #24e-9 #100e-9 # grating spacing
    
    # angle between the beams from each grating
    theta = np.arcsin( m *wl/pitch) #[np.arcsin( m *wl/p) for p in pitch]
    ...
    ''' generate interference intensity'''
    I_TM = interferenceIntensity(x,k,theta,A=A, polarisationModes=('TM','TM'))
    I_TE = interferenceIntensity(x,k,theta,A=A, polarisationModes=('TE','TE'))
    
    # noise parameters
    noiseDensity = np.linspace(0,150,2000)
    freQ = 1/(2*pitch)
    rho=[nD/(np.sqrt(freQ)) for nD in noiseDensity]#xstep
    sr = pitch/(2*xstep) #n  # (as period = n/sr)
    
    ''' generate noise '''
    noise = [whiteNoise(r, sr, n+1, mu=0) for r in rho]
    InoisyTM = I_TM + noise[0]
    InoisyTE = [I_TE + nZ for nZ in noise]
    
    plt.plot(x*1e9,I_TE,label='\u03C1 = 0 $Hz^{-1}$')
    # plt.plot(x,I_TM,label='TM')
    # plt.plot(x,InoisyTM,label='TM+noise')
    plt.plot(x*1e9,InoisyTE[int(len(noiseDensity)/8)], ':',label='\u03C1 = '+str(round_sig(noiseDensity[int(len(noiseDensity)/8)]))+' $Hz^{-1}$')
    plt.plot(x*1e9,InoisyTE[int(len(noiseDensity)/4)], ':', color=colours[3], label='\u03C1 = '+str(round_sig(noiseDensity[int(len(noiseDensity)/4)]))+' $Hz^{-1}$')
    plt.plot(x*1e9,InoisyTE[int(len(noiseDensity)/2)], ':', color=colours[4], label='\u03C1 = '+str(round_sig(noiseDensity[int(len(noiseDensity)/2)]))+' $Hz^{-1}$')
    plt.plot(x*1e9,InoisyTE[len(noiseDensity)-1], ':', color=colours[5], label='\u03C1 = '+str(round_sig(noiseDensity[len(noiseDensity)-1]))+' $Hz^{-1}$')
    plt.xlim(-50,50)
    plt.ylim(-0.4,1.4)
    ...
    plt.legend()
    if save:
        plt.savefig(savePath + 'Noise_AerialImage.pdf')
        plt.savefig(savePath + 'Noise_AerialImage.png', dpi=2000)
    else:
        pass
    ...
    # getting contrast
    michelsonC = [gratingContrastMichelson(a) for a in InoisyTE]
    rmsC = [gratingContrastRMS(a) for a in InoisyTE]
    compositeC = [meanDynamicRange(a, show=False) for a in InoisyTE] #, mdrC, imbalanceC 
    nils1 = [NILS(a,x, pitch/4, show=False) for a in InoisyTE] 
    fourierC = [gratingContrastFourier(a,x, show=False) for a in InoisyTE] #Cf,  Am, Fr, peakFr - Still unsure but seems good
#    fidel = [fidelity(a,b) for a, b in zip(InoisyTE, I_TE)]   # fidelity based on comparison to model

    fig, ax1 = plt.subplots()
    # ax2 = ax1.twinx()
    
    ax1.plot(noiseDensity, michelsonC, label='Michelson')
    ax1.plot(noiseDensity, rmsC, label='RMS')
    ax1.plot(noiseDensity, [c[0] for c in compositeC], label='Composite')
    ax1.plot(noiseDensity, [c[1] for c in compositeC], label='MDR')
    ax1.plot(noiseDensity, [c[2] for c in compositeC], label='Imbalance')
    ax1.plot(noiseDensity, [f[0] for f in fourierC], label='Fourier')
    # ax2.plot(noiseDensity, [n[0] for n in nils1], '', label='mean NILS') # meanNILS,  meanLW, NILS, LW, rmsNILS, rmsLW, rmsdLW
    # # ax2.plot(noiseDensity, [n[2] for n in nils1], ':', label='NILS')
    # ax2.plot(noiseDensity, [n[4] for n in nils1], ':.', label='rms NILS')
    # print("here")
    # print(nils1[0])
    # print("here")
    # print(nils1[4])
#    ax1.plot(noiseDensity, fidel, '--', label='Fidelity')
    ax1.legend(loc='upper left')
    # ax2.legend(loc='center left')
    # ax2.set_ylabel("Fidelity")
    # ax2.set_ylabel("NILS")
    ax1.set_ylabel("Aerial Image Contrast (TE-TE)")
    ax1.set_xlabel("Spectral Noise Density (\u03C1) [$Hz^{-1}$]")
    # ax1.set_xticklabels([1e9*np.max(pitch)*a for a in [0,1/5,2/5,3/5,4/5,1]])
    if save:
        plt.savefig(savePath + 'Noise_Contrast.pdf')
        plt.savefig(savePath + 'Noise_Contrast.png', dpi=2000)
    else:
        pass
    plt.show()
    
    plt.plot(noiseDensity, [n[0] for n in nils1], 'x', label='mean NILS')
    plt.plot(noiseDensity, [n[4] for n in nils1], 'x', label='rms NILS')
    plt.show()
    
    
    fig, axs = plt.subplots(2,3)
    
    axs[0,0].plot(noiseDensity, michelsonC)#, label='TM')
    # axs[0,0].plot(noiseDensity, michelsonCte, label='TE')
    # axs[0,0].legend()
    axs[0,0].set_title("Michelson")
    axs[0,0].set_ylabel("Contrast")
    axs[0,1].plot(noiseDensity, rmsC)#, label='TM')
    # axs[0,1].plot(noiseDensity, rmsCte, label='TE')
    # axs[0,1].legend()
    axs[0,1].set_title("RMS")
    axs[0,2].plot(noiseDensity, [c[0] for c in compositeC])#, label='TM')
    # axs[0,2].plot(noiseDensity, [c[0] for c in compositeCte], label='TE')
    # axs[0,2].legend()
    axs[0,2].set_title("Composite")
    axs[1,0].plot(noiseDensity, [f[0] for f in  fourierC])#, label='TM')
    # axs[1,0].plot(noiseDensity, [f[0] for f in  fourierCte], label='TE')
    # axs[1,0].legend()
    axs[1,0].set_title("Fourier")
    axs[1,0].set_ylabel("Contrast")
    axs[1,0].set_xlabel("\u03C1 [$Hz^{-1}$]")
    axs[1,1].plot(noiseDensity, [c[2] for c in compositeC])#, label='TM')
    # axs[1,1].plot(noiseDensity, [c[2] for c in compositeCte], label='TE')
    # axs[1,1].legend()
    axs[1,1].set_title("Imbalance")
    axs[1,1].set_xlabel("\u03C1 [$Hz^{-1}$]")
    axs[1,2].plot(noiseDensity, [n[0] for n in nils1])#, label='TM')
    # axs[1,2].plot(noiseDensity, [n[0] for n in nils1te], label='TE')
    # axs[1,2].legend()
    axs[1,2].set_title("NILS")
    axs[1,2].set_xlabel("\u03C1 [$Hz^{-1}$]")
    axs[1,2].set_ylabel("NILS")
    # for ax in axs[:,:]:
    # axs[0,0].set_xticklabels([np.max(noiseDensity)*a for a in [0,1/4,1/2,3/4,1]])
    # axs[0,1].set_xticklabels([np.max(noiseDensity)*a for a in [0,1/4,1/2,3/4,1]])
    # axs[0,2].set_xticklabels([np.max(noiseDensity)*a for a in [0,1/4,1/2,3/4,1]])
    # axs[1,0].set_xticklabels([np.max(noiseDensity)*a for a in [0,1/4,1/2,3/4,1]])
    # axs[1,1].set_xticklabels([np.max(noiseDensity)*a for a in [0,1/4,1/2,3/4,1]])
    # axs[1,2].set_xticklabels([np.max(noiseDensity)*a for a in [0,1/4,1/2,3/4,1]])
    fig.tight_layout()
    if save:
        plt.savefig(savePath + 'Noise_Contrast_Sep.pdf')
        plt.savefig(savePath + 'Noise_Contrast_Sep.png', dpi=2000)
    else:
        pass
    plt.show()
