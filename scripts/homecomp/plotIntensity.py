...
files = ['correctedWBS/' + str(o) + 'intensity.tif' for o in order]
...
res =  [(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),
        (2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),
        (2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),
        (2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),
        (2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07),(2.5182637683019347e-09, 1.113721350329607e-07)]
...
mid =   [(196, 12257),(196, 12257),(196, 12257),(196, 12257),(196, 12257),
         (196, 12257),(196, 12257),(196, 12257),(196, 12257),(196, 12257),
         (196, 12257),(196, 12257),(196, 12257),(196, 12257),(196, 12257),
         (196, 12257),(196, 12257),(196, 12257),(196, 12257),(196, 12257),
         (196, 12257),(196, 12257),(196, 12257),(196, 12257),(196, 12257)]
#40x40
...
if show:
    if len(tiffs) == 1:
        ...
        plt.yticks([int(np.shape(tiffs[0])[0]*(b/8)) for b in range(0,9)],
                   [round_sig(np.shape(tiffs[0])[0]*res[0][1]*(a/8)*1e6) for a in range(-4,5)],fontsize=15 )
        plt.xticks([int(np.shape(tiffs[0])[1]*(b/8)) for b in range(0,9)],
                   [round_sig(np.shape(tiffs[0])[1]*res[0][0]*(a/8)*1e6) for a in range(-4,5)],fontsize=15 )
        plt.colorbar()
        plt.show()
        
        plt.imshow(images[0], aspect = 'auto')
        plt.xticks([int(2*N*(a/6)-1) for a in range(0,7)],[round_sig(np.max(xP[0])*x*1e6/3) for x in np.linspace(-3,3,7)]) #[-1, -2/3,-1/3,0,1/3,2/3,1]])
        plt.yticks([int(2*n*(b/4)-1) for b in range(0,5)],[round_sig(np.max(yP[0])*y*1e6/2) for y in np.linspace(-2,2,5)])
        ...
        for i in range(len(tiffs)):
            Imax = np.max(images)
            Imin = np.min(images)
            im = axs[i].imshow(images[i], aspect=2)# ,vmin=Imin, vmax=Imax)# aspect=15) #cmap='gray',
            ...
            axs[i].set_yticklabels([round_sig(np.max(yP[i])*y*1e6) for y in [-1,0,1]])
            if i < len(tiffs)-1:
                axs[i].set_xticklabels([])
            else:
                axs[i].set_xticklabels([round_sig(np.max(xP[i])*x*1e6) for x in [-1, -2/3,-1/3,0,1/3,2/3,1]])
...
if subtractAperture:
    ...
    envelope = [a*profiles[0]/np.max(profiles[0]) + profiles[2]/np.max(profiles[2]) for a in [1]]#,1.5,2,2.5]]#,4,5,6,7]]
...
if plotTogether:
    if normalise:
        for i,p in enumerate(profiles):
            ...
            if i == 0:
                ...
                plt.plot(xP[i]*1e9,p/np.mean(p),  label=labels[i], color=colours[i+1])
            else:
                pass
        ...
        plt.ylim(bottom=0)
        plt.xlabel('Position [nm]')
        leg1 = plt.legend(title=None,loc='upper left')# bbox_to_anchor=(0.89,-0.12), ncol=3) 
                   #loc='upper left')
        # if save:
    ...
    if plotFromEq:
        if subtractAperture:
            profileSubtract = [profiles[1]/np.max(profiles[1]) - e for e in envelope] #- profiles[0]/np.max(profiles[0])  - profiles[2]/np.max(profiles[2])
            profileSubtract = [abs(1-(abs(p)/np.max(abs(p)))) for p in profileSubtract]
            EQplus = [IP/np.max(IP) + e for e in envelope]#+ profiles[2]/np.max(profiles[2]) + profiles[0]/np.max(profiles[0])
            ...
            for e, p in enumerate(profileSubtract):
                ...
                plt.plot(xP[0]*1e9,EQplus[e], ':', label = 'Eq.2.3.45 - added envelope #' + str(e))
            plt.xlim(-int(plotRange/2), int(plotRange/2))
            plt.ylabel('Intensity [a.u]')
            plt.ylim(bottom=0)
            plt.xlabel('Position [nm]')
...
if contrast:
    ...
    if twoD:
        ...
        fidel2d = [interferenceGratingModelsJK.fidelity(i,images[0]) for i in images[1::]]   # fidelity based on comparison to model
        
        if split:
            LWRvalues = [1.8419804271566513e-10, 
            ...
            axs[1,1].plot(sigma[0:5], LWRvalues[0:5], 'x:')
            axs[1,1].plot(sigma[5:10], LWRvalues[5:10], 'x:')
            axs[1,1].plot(sigma[10:15], LWRvalues[10:15], 'x:')
            axs[1,1].plot(sigma[15:19], LWRvalues[15:19], 'x:')
            axs[1,1].plot([sigma[20], sigma[21], sigma[22], sigma[24]], [LWRvalues[20], LWRvalues[21], LWRvalues[22], LWRvalues[24]], 'x:')
            # axs[0,2].set_title("NILS - RMS")
            axs[1,1].set_ylabel("$LWR$")
            # axs[2,0].axis('off')
            # axs[2,2].axis('off')
            # axs[0,0].legend(loc='lower left')
...
if findCorrelation:
    ...
    if twoD:
        ...
        dataStructure = [sigma,cY,michelsonC,rmsC,[c[0] for c in compositeC],
                      [f[0] for f in fourierC],[n[0] for n in nilsC],fidel]
    ...
    sns.heatmap(correlations,cmap='vlag',vmin=-1,vmax=1,annot=True)#,labelsize=10)#,fontSize=2)
    if save:
