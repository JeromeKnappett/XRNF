...
def mutualIntensity(cfr, Fx, Fy, pathJP=None, pathJ=None):
    ...
    j = abs(J)/np.squeeze(I)
    jxx = abs(Jxx)/np.squeeze(I)
    jxy = abs(Jxy)/np.squeeze(I)
    jyx = abs(Jyx)/np.squeeze(I)
    jyy = abs(Jyy)/np.squeeze(I)
...
def test():
    ...
    waveName = path + 'wavefieldME_TEST_500um_.pkl'
