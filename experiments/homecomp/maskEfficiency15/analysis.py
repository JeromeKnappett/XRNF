...
dirPath = '/data/experiments/maskEfficiency15/data/'

batchOutputFile = dirPath + '1280/results.pickle'

trialData = [dirPath + str(i) + '/' for i in range(1240, 1280)]
pickles = ['mt' + str(i) + '.pkl' for i in range(1240, 1280)]
...
Xprofiles = []
Yprofiles = []
maskThickness = list(range(1240, 1280))
...
pathE1close = dirPath + 'efficiencyPlotM1close.png'
...
for profile, mask in zip(Xprofiles, maskThickness):
        ...
        print(mask)
        E0, E1, En1, rE0, rE1, rEn1 = utilMask.getEfficiency(incidentI, exitI, profile, m=1, resIN = resolutionXIN, resEX = resolutionXEX, resPR = resolutionX[0], pathI=pathI+str(mask)+'.png', pathM=pathM+str(mask)+'.png')
        e0.append(E0)
        e1.append(E1)
        en1.append(En1)
        re0.append(rE0)
