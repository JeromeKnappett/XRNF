...
dirPath = '/home/jerome/dev/experiments/beamPolarisation13/data/'
batchOutputFile = dirPath + '200LV/results.pickle'

ran = range(0,3,2)

trialData = np.array([[dirPath + str(i) + '_1LH/', dirPath + str(i) + '_1LV/'] for i in ran]).flatten()
pickles = np.array([['p' + str(i) + '-1lh.pkl','p' + str(i) + '-1lv.pkl'] for i in ran]).flatten()
...
for t, pk in zip(trialData, pickles):
    ...
    plt.xlim(int(nx/2) - aerialImagePix, int(nx/2) + aerialImagePix)
    plt.xticks([int(nx/2)-aerialImagePix, int(nx/2)-int(aerialImagePix/2), int(nx/2), int(nx/2)+int(aerialImagePix/2), int(nx/2)+aerialImagePix],[round_sig(-aerialImagePix*dx*1e6), round_sig(-int(aerialImagePix/2)*dx*1e6), 0, round_sig(int(aerialImagePix/2)*dx*1e6), round_sig(aerialImagePix*dx*1e6)] )
    ...
    plt.plot(IxH, '.')
    plt.plot(IxH, ':')
    plt.title("Aerial Image" + pk[0:len(str(pk))-3])
    plt.xlim(int(nx/2) - aerialImagePix, int(nx/2) + aerialImagePix)
    plt.xticks([int(nx/2)-aerialImagePix, int(nx/2)-int(aerialImagePix/2), int(nx/2), int(nx/2)+int(aerialImagePix/2), int(nx/2)+aerialImagePix],[round_sig(-aerialImagePix*dx*1e6), round_sig(-int(aerialImagePix/2)*dx*1e6), 0, round_sig(int(aerialImagePix/2)*dx*1e6), round_sig(aerialImagePix*dx*1e6)] )
    plt.xlabel("position [\u03bcm]")
