...
dirPath = '/home/jerome/dev/experiments/maskEfficiency12/'#'/data/experiments/maskEfficiency8/data/'
batchOutputFile = dirPath + '200LV/results.pickle'

trialData = [dirPath, dirPath, dirPath, dirPath]# + '20LH/', dirPath + '40LH/', dirPath + '50LH/', dirPath + '60LH/', dirPath + '80LH/', dirPath + '100LH/', dirPath + '200LH/',dirPath + '20LV/', dirPath + '40LV/', dirPath + '50LV/', dirPath + '60LV/', dirPath + '80LV/', dirPath + '100LV/', dirPath + '200LV/' ]
#[dirPath + '01/', dirPath + '05/', dirPath + '60/', dirPath + '70/', dirPath + '80/', dirPath + '90/', dirPath + '100/']
files = ['stokes0.pkl', 'stokes1.pkl', 'stokes2.pkl']
pickles = ['p20-lh.pkl', 'p40-lh.pkl', 'p50-lh.pkl', 'p60-lh.pkl']#, 'p80-lh.pkl', 'p100-lh.pkl', 'p200-lh.pkl', 'p20-lv.pkl', 'p40-lv.pkl', 'p50-lv.pkl', 'p60-lv.pkl', 'p80-lv.pkl', 'p100-lv.pkl', 'p200-lv.pkl']
...
pitch = [20, 40, 50, 60, 80, 100, 200]#, 20, 40, 50, 60, 80, 100, 200]
...
for t, pk in zip(trialData, pickles):
    ...
    rY.append(ry)
    nY.append(ny)
    print("Dimensions [pixels]: {}".format((nx,ny)))
    
    Ix = r['results']['Intensity/Total/X/profile']
    Iy = r['results']['Intensity/Total/Y/profile']
    vIx = r['results']['Intensity/Vertical/X/profile']
    vIy = r['results']['Intensity/Vertical/Y/profile']
    hIx = r['results']['Intensity/Horizontal/X/profile']
    hIy = r['results']['Intensity/Horizontal/Y/profile']
    ...
    plt.plot(Ix, ':')
    plt.title("Aerial Image" + pk[0:len(str(pk))-3])
    plt.xlim(int(nx/2) - 20, int(nx/2) + 20)
    plt.xticks([int(nx/2)-20, int(nx/2)-10, int(nx/2), int(nx/2)+10, int(nx/2)+20],[round_sig(-20*dx*1e6), round_sig(-10*dx*1e6), 0, round_sig(10*dx*1e6), round_sig(20*dx*1e6)] )
    plt.xlabel("position [\u03bcm]")
    # print("Saving Aerial Image plot to: {}".format(dirPath + "plots/" + "aerialImage" + pk[0:len(str(pk))-4] + ".png"))
    # plt.savefig(dirPath + "plots/" + "aerialImage" + pk[0:len(str(pk))-4] + ".png")
    plt.show()
    ...
    plt.clf()
    plt.close()
    plt.plot(hIx, label= 'horizontal cut - hor pol')
    plt.plot(hIy, label= 'vertical cut - hor pol')
    plt.plot(vIx, label= 'horizontal cut - ver pol')
    plt.plot(vIy, label= 'vertical cut - ver pol')
    plt.title("Intensity_HorPol_" + pk[0:len(str(pk))-3])
    plt.legend()
    plt.xticks(np.arange(0,nx+1,nx/4),tickAx)
    plt.xlabel("position [m]")
    print("SavingPolarisation Intensity plot to: {}".format(dirPath + "plots/" + "IntensityPol" + pk[0:len(str(pk))-4] + ".png"))
    plt.savefig(dirPath + "plots/" + "IntensityPol" + pk[0:len(str(pk))-4] + ".png")
    plt.show()
    plt.clf()
    plt.close()
    
    plt.plot(vIx, '.')
    plt.plot(vIx, ':', label='vertical pol')
    plt.plot(hIx, '.')
    plt.plot(hIx, ':', label='horizontal pol')
    plt.title("Aerial Image_Pol_" + pk[0:len(str(pk))-3])
    plt.legend()
    plt.xlim(int(nx/2) - 1500, int(nx/2) + 1500)
    # plt.xticks([int(nx/2)-50, int(nx/2)-25, int(nx/2), int(nx/2)+25, int(nx/2)+50],[round_sig(-50*dx*1e6), round_sig(-25*dx*1e6), 0, round_sig(25*dx*1e6), round_sig(50*dx*1e6)] )
    plt.xlabel("position [\u03bcm]")
    print("Saving Polarisation Aerial Image plot to: {}".format(dirPath + "plots/" + "aerialImagePol" + pk[0:len(str(pk))-4] + ".png"))
    plt.savefig(dirPath + "plots/" + "aerialImagePol" + pk[0:len(str(pk))-4] + ".png")
    plt.show()    
