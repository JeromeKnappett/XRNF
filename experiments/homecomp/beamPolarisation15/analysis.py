...
colours =  plt.rcParams["axes.prop_cycle"].by_key()["color"][:1]+plt.rcParams["axes.prop_cycle"].by_key()["color"][3:] #['red','blue'] #
# %%
...
dirPath = '/home/jerome/dev/experiments/beamPolarisation15/data/'
batchOutputFile = dirPath + '1LV/results.pickle'

ran1 = range(0,2)
ran2 = [24,40]

trialData = np.array([[dirPath + str(i) + 'LH/', dirPath + str(i) + 'LV/'] for i in ran1]).flatten()
pickles = np.array([['p' + str(i) + '-lh.pkl','p' + str(i) + '-lv.pkl'] for i in ran2]).flatten()
...
for t, pk in zip(trialData, pickles):
    ...
    # print(np.shape(np.squeeze(Ix)))
    # print(Ix[:][0])
    # print(Ix[:][1])
    IX.append(np.squeeze(Ix))
    IY.append(np.squeeze(Iy))
    IXh.append(np.squeeze(IxH))
    IXv.append(np.squeeze(IxV))
    ...
    # plt.title("Intensity" + pk[0:len(str(pk))-3])
    # plt.legend()
    ...
    plt.close()
    # # Pickling results
    # with open(dirPath + pk[0:len(str(pk))-3] + 'IxV.pkl', 'wb') as p:
    #     pickle.dump(IxV, p)
    # with open(dirPath + pk[0:len(str(pk))-3] + 'IxH.pkl', 'wb') as q:
    #     pickle.dump(IxH, q)
    # with open(dirPath + pk[0:len(str(pk))-3] + 'Ix.pkl', 'wb') as r:
    #     pickle.dump(Ix, r)
...
print('shape of IX: ', np.shape(IX[:][:][:]))

aerialImagePix = 100

# for x in enumerate(IX):
#     print(np.shape(x[0]))
#     plt.plot(x)
#     plt.show()
length = len(IX[0])
print(int(length/2)+aerialImagePix)
print('length: ', len(IX[int(length/2)+aerialImagePix:int(length)-aerialImagePix]))
fig, axs = plt.subplots(2,1)
print('dx', dX[0])
axs[0].plot(IX[1][int(length/2-aerialImagePix):int(length/2+aerialImagePix)],'--',label='TE-TE')
axs[0].plot(IX[0][int(length/2-aerialImagePix):int(length/2+aerialImagePix)],':',label='TM-TM')
axs[1].plot(IX[3][int(length/2-aerialImagePix):int(length/2+aerialImagePix)],'--',label='TE-TE')
axs[1].plot(IX[2][int(length/2-aerialImagePix):int(length/2+aerialImagePix)],':',label='TM-TM')
for ax in fig.axes:
    ax.set_xticks([2*aerialImagePix*a for a in [0,1/4,2/4,3/4,1]])
    ax.set_xticklabels([round_sig(1e9*dX[0]*2*aerialImagePix*a) for a in [-1/2,-1/4,0,1/4,1/2]])
axs[1].set_xlabel('x-position [nm]')
axs[0].set_ylabel("Intensity [ph/s/.1%bw/mm$^2$]")
# axs[1].set_ylabel("Intensity [ph/s/.1%bw/mm$^2$]")
axs[0].legend()
axs[1].legend()
axs[0].yaxis.set_label_coords(-0.08,-0.15)
fig.tight_layout()
plt.show()
...
ax0.plot(IXv[1][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], '--', color=colours[1])
ax1.plot(IXh[1][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], '--', color=colours[0])
ax2.plot(IXv[0][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], '--', color=colours[1])
ax3.plot(IXh[0][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], '--', color=colours[0])
ax0.legend(loc='upper left', title='TE-TE')
# ax1.legend()
ax2.legend(loc='upper left', title='TM-TM')
# ax3.legend()
...
ax1.set_xticks([2*aerialImagePix*a for a in [0,1/4,2/4,3/4,1]])
ax1.set_xticklabels([round_sig(1e9*dX[0]*2*aerialImagePix*a) for a in [-1/2,-1/4,0,1/4,1/2]])
ax3.set_xticks([2*aerialImagePix*a for a in [0,1/4,2/4,3/4,1]])
ax3.set_xticklabels([round_sig(1e9*dX[0]*2*aerialImagePix*a) for a in [-1/2,-1/4,0,1/4,1/2]])
ax3.set_xlabel('x-position [nm]')
ax2.set_xlabel('x-position [nm]')
ax0.set_ylabel("Intensity (vertical) [ph/s/.1%bw/mm$^2$]")
ax1.set_ylabel("Intensity (horizontal) [ph/s/.1%bw/mm$^2$]")
ax0.yaxis.set_label_coords(-0.08,-0.15)
ax1.yaxis.set_label_coords(1.15,-0.15)
# ax2.set_ylabel("Intensity [ph/s/.1%bw/mm$^2$]")
# ax3.set_ylabel("Intensity [ph/s/.1%bw/mm$^2$]")
fig.tight_layout()
plt.show()
...
ax1.plot(IXh[3][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], color=colours[1])
ax0.plot(IXv[3][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], color=colours[0])
ax3.plot(IXh[2][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], color=colours[1])
ax2.plot(IXv[2][int(length/2-aerialImagePix):int(length/2+aerialImagePix)], color=colours[0])
ax0.legend(loc='upper left', title='TE-TE')
# ax1.legend()
ax2.legend(loc='upper left', title='TM-TM')
# ax3.legend()
...
ax0.set_ylabel("Intensity (vertical) [ph/s/.1%bw/mm$^2$]")
ax0.yaxis.set_label_coords(-0.08,-0.15)
ax1.yaxis.set_label_coords(1.08,-0.15)
fig.tight_layout()
plt.show()
