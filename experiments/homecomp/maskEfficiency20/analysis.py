...
dirPath = '/data/experiments/maskEfficiency20/data/'

batchOutputFile = dirPath + '1500/results.pickle'

ran= range(1000, 1480,5)
...
for t, pk, xp in zip(trialData, pickles, expickles):
    ...
    if propWave == True:
        ...
        plt.clf()
        plt.close()    
        plt.plot(Ix, label='x-cut (close up)')
        plt.xlim(int(nx/2)-1250, int(nx/2)+1250)
        plt.title("Horizontal Intensity - close up "+ pk[0:len(str(pk))-4])
        print("Saving intensity plot to: " + dirPath + 'plots/IntensityCloseUp' + pk[0:len(str(pk))-4] + '.png')
        plt.savefig(dirPath + 'plots/IntensityCloseUp' + pk[0:len(str(pk))-4] + '.png')
