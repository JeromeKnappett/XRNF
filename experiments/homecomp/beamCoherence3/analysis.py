...
dirPath = '/data/experiments/beamCoherence3/data/'
batchOutputFile = dirPath + 'CL/results.pickle'

trialData = [dirPath + '30/', dirPath + '40/', dirPath + '50/', dirPath + '60/', dirPath + '70/', dirPath + '80/', dirPath + '90/']
files = ['stokes0.pkl', 'stokes1.pkl', 'stokes2.pkl']
pickles = ['eh30.pkl', 'eh40.pkl', 'eh50.pkl', 'eh60.pkl', 'eh70.pkl', 'eh80.pkl', 'eh90.pkl']
...
emittance = [30, 40, 50, 60, 70, 80, 90]
...
for t, pk, e in zip(trialData, pickles, emittance):
    print("")
...
#    dCy = np.diagonal(np.squeeze(cY))
    lx = len(cX[0])
    ly = len(cY[0])
    dCx = np.array([cX[lx-1-i][i] for i in range(lx-1,-1,-1)])
    dCy = np.array([cY[ly-1-i][i] for i in range(ly-1,-1,-1)])
    ...
    plt.plot(cX[:,int(ny/2)], label="vertical")
    plt.plot(cX[int(nx/2),:], label="horizontal")
    plt.plot(dCx, label = "op diagonal")
    plt.plot(np.diagonal(np.squeeze(cX)), label="diagonal")   
    plt.plot((I[int(ny/2),:]/np.max(I[int(ny/2),:])), label="intensity")
#    plt.xticks(np.arange(0,nx+1,nx/4),tickAx)
#    plt.xlabel("position [m]") # [\u03bcm]")
...
#    plt.xticks(np.arange(0,nx+1,nx/4),tickAx)
#    plt.xlabel("position [m]") # [\u03bcm]")
    plt.title("Horizontal Coherence (cuts), close-up, e =" + str(e))
    ...
    plt.legend()
    plt.savefig(dirPath + "plots/" + "coherenceHorCuts_closeup" + str(e) + ".png")
    print("Saving horizontal coherence (close up) cuts plot to: {}".format("plots/" + "coherenceHorCuts_closeup" + str(e) + ".png"))
    plt.show()
    plt.clf()
    plt.close()
    ...
    plt.plot(cY[:,int(ny/2)], label="vertical")
    plt.plot(cY[int(nx/2),:], label="horizontal")
    plt.plot(dCy, label = "op diagonal")
    plt.plot(np.diagonal(np.squeeze(cY)), label="diagonal")   
    plt.plot((I[:,int(nx/2)]/np.max(I[:,int(nx/2)])), label="intensity")
...
plt.title("Coherence Length (horizontal) vs E-beam emittance")
plt.legend()
plt.xlabel("Horizontal emittance [nm]")
plt.ylabel("Coherence Length [m]")
plt.savefig(dirPath + "plots/" + "coherenceLengthHOR.png")
print("Saving vertical coherence cuts plot to: {}".format(dirPath + "plots/" + "coherenceLengthHOR.png"))
plt.show()
plt.clf()
plt.close()

plt.plot(emittance, cly)#, label="Vertical")
plt.title("Coherence Length (vertical) vs E-beam emittance")
plt.legend()
plt.xlabel("Horizontal emittance [nm]")
plt.ylabel("Coherence Length [m]")
plt.savefig(dirPath + "plots/" + "coherenceLengthVER.png")
print("Saving vertical coherence cuts plot to: {}".format(dirPath + "plots/" + "coherenceLengthVER.png"))
plt.show()
plt.clf()
plt.close()
