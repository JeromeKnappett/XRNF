...
dirPath = '/home/jerome/dev/experiments/maskEfficiency16/newdata/'
...
resultsIN = pickle.load( open(dirPath + 'incident/incident.pkl', 'rb')  )
...
IR_1 = incidentP[midInc-420:midInc+420]
plt.plot(IR_1)#[midInc-int(midInc/3):midInc+int(midInc/3)])
plt.ylim(0,0.00002)#1.204e12, 1.20425e12)
plt.ylabel('Phase [rad]')#'Intensity [ph/s/.1%bw/mm$^2$]')
plt.xlabel("Position [\u03bcm]")
plt.xticks([int(len(IR_1)*(b/8)) for b in [0,2,4,6,8]],[utilMask.round_sig(len(IR_1)*resolutionXIN*a*1e6) for a in [-1/2,-2/8,0,2/8,4/8]])
# plt.set_xticklabels([utilMask.round_sig(len(IR_1)*resolutionXIN*a*1e6) for a in [-1/2,-2/8,0,2/8,4/8]])
plt.legend()
plt.show()
...
print("Shape of incident, exit, prop: ", np.shape(incidentI), np.shape(exitIx[0]), np.shape(Xprofiles[0]))

print("Resolution of incident, exit, prop: ", resolutionXIN, resolutionXEX[0], resolutionX[0])
      
for profile, mask, exitI in zip(Xprofiles, maskThickness, exitIx):
        
        print(mask)
        E0, E1, En1, rE0, rE1, rEn1 = utilMask.getEfficiency(incidentI, exitI, profile, m=1, g=1, resIN = resolutionXIN, 
                                                             resEX = resolutionXEX[0], resPR = resolutionX[0], orderSqueeze=0.5, 
                                                             pathI=pathI+str(mask)+'.png', pathM=pathM+str(mask)+'.png')
        e0.append(E0)
        e1.append(E1)
        en1.append(En1)
        re0.append(rE0)
