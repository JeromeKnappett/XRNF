...
def propWavefront():
    ...
    from usefulWavefield import wavefrontFromPickle
    #from extensions.beamlineExt import bl
    
    path = '/user/home/opt/xl/xl/experiments/diamondFZP/data/FZP_test/'
    #'/user/home/opt/xl/xl/experiments/telePtychoTesting/data/p200/ap4/atAp/'
   # initialWavefront='/opt/wpg/wpg/extensions/in/50Slices63dLTZP_simulationData/wf_noCS.h5'
    initialWavefront= path + 'wf_final.hdf' #'wavefrontSE.pkl' #'wf_final.hdf'
    ...
    # wf = wavefrontFromPickle(initialWavefront,wavefront=True)
    ...
    plotPropWave(outPath + 'intensity/',
                 zRange,zPlanes,res=dx,axis='hor',log=True, lMin=1e9)#, savePath=savePath)
#    printWFDictUgly(wf)

def plotPropWave(path,zRange,zPlanes,res,axis,startPlane=0,log=False,lMin=1,savePath=False):
    import os
    import tifffile
