...
dirPath ='/user/home/opt/xl/xl/experiments/diamondFZP/data/' 
order = ['BDAincident_ssa' + str(s) + 'bda' + str(b) for s,b in zip(ssa,bda)]
# ['prop' + str(z) + 'cm' for z in Z]
# ['FZP_test']
...
plt.plot(ssa,IC,'x:')
plt.xlabel('SSA width')
plt.ylabel('I over FZP [ph/s/cm$^2$]')
plt.show()
plt.plot(ssa,FC,'x:')
plt.xlabel('SSA width')
plt.ylabel('Flux over FZP [ph/s]')
plt.show()
...
print(IC)
print('Flux: ', FC)
plt.plot(ssa,FWx,'x:',label='x')
plt.plot(ssa,FWy,'x:',label='y')
plt.xlabel('SSA width')
plt.ylabel('FWHM [mm]')
plt.legend()
plt.show()
