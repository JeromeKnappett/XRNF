...
order = [0,1,2,3,4,5,6,7,8] #range(19,20)
cY = [10.0,10.0,10.0,10.0,10.0,10.0,10,10,10,10,10]
sigma = [0.0,0.5,1.0,1.5,2.0,2.5,3.0,3.5,4.0] #[0.5,1.0,1.5,2.0,2.5]
...
dims = (2,3)
labels = ['\u03C3 = 0.0 nm ','\u03C3 = 0.5 nm ','\u03C3 = 1.0 nm ','\u03C3 = 1.5 nm ','\u03C3 = 2.0 nm ','\u03C3 = 2.5 nm ','\u03C3 = 3.0 nm ','\u03C3 = 3.5 nm','\u03C3 = 4.0 nm ']
...
N = 3900                           # number of pixels to take for line profile  - 1200 for roughness aerial images
n = 75                                 # number of pixels to average over for line profile - 15 for roughness aerial images
...
#    fourierC = [interferenceGratingModelsJK.gratingContrastFourier(p,x*1e6, show=False) for p, x in zip(profiles,xP)] #Cf,  Am, Fr, peakFr - Still unsure but seems good
    fidel = [interferenceGratingModelsJK.fidelity(p,ite) for p, ite in zip(profiles,IP)]   # fidelity based on comparison to model
    ...
    Nrms.append([n[9] for n in nilsC])
    F.append(fidel)
...
#            # ax.set_xticklabels(labels, rotation=45, ha='right')
        ...
        if toPickle:
            with open(dirPath + 'contrastMetrics_maskLER_retry.pkl', "wb") as f:
...
#        plt.show()
        ...
        if toPickle:
            with open(dirPath + 'contrastMetrics_maskLER_retry_2d.pkl', "wb") as f:
