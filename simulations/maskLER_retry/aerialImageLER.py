...
plt.show()
plt.plot(images[0][:,100])
plt.vlines(np.shape(images[0])[0]//2,0,5e9)
plt.show()
...
for i,f in enumerate(FR):
    plt.plot([_f/1.0e9 for _f in f], FP[i],'.')#, label='biased',alpha=0.2)
    ax = plt.gca()
    ...
    ler = np.sqrt(integrate.simpson(FP[i], f))
...
plt.plot(order,[l*1e9 for l in LER],label='std')
plt.plot(order,[l*1e9 for l in _LER],label='psd')
