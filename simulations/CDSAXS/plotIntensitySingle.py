...
order = [0,1,7] #range(19,20)
cY = [10.0] #[10,10,10,10,10]
sigma = [0,0.5,3.5] #[0.5,1.0,1.5,2.0,2.5]
...
labels = ['\u03C3 = 0.0 nm ','\u03C3 = 0.5 nm ','\u03C3 = 3.5 nm']
...
n = 360                                 # number of pixels to average over for line profile - 15 for roughness aerial images
...
if Pickle:
    ...
    for i, p in enumerate(profiles):
        with open(dirPath + str(order[i]) + '_cy10.pkl', "wb") as f:
...
if plotTogether:
    ...
    if plotFromEq:
        if subtractAperture:
            ...
            plt.plot(xP[0]*1e9,(IP/np.max(IP))*np.max(profiles[0]), ':', label='Eq.2.3.45')
            # plt.plot(xP[0]*1e9,idealProfile, label='\u03C3 = 0 nm')
    else:
        pass
    plt.xlim(-int(plotRange/2), int(plotRange/2))
    plt.ylim(bottom=0)
...
if contrast:
    ...
    if twoD:
        ...
        for e, i in enumerate(images):
            ...
            print('NILS std: ', NILS2D[:,9])
            print("shape of NILS: ", np.shape(NILS))
            print("shape of NILS std ", np.shape(NILS2D[:,9]))
#            NILSstd   = [a[0:np.min(NNILS)-1] for a in NILS2D[:,9]]
            ...
            if Pickle:
                with open(dirPath + str(order[e]) + 'NILS.pkl', "wb") as f:
                    pickle.dump(NILS, f)
            ...
            (Xcln,Ycln,freq,FourierPow) = LER.LER_analysis(i)   
        ...
        LW = [interferenceGratingModelsJK.LWR(i,xP[e], pitch/4, debug=False) for e,i in enumerate(images)]
        #disable LWR for speed - testing Fourier
        #LW = [[[0], 0, 0, 0] for i in images]
        
        
        (Xcln,Ycln,freq,FourierPow) = LER.LER_analysis()        
import cv2 #(OpenCV3)
from matplotlib import pyplot as plt
import matplotlib.ticker as ticker


image = cv2.imread("Spec_test.bmp")#("binary_test.jpg")#("Spec_test.bmp")
...
LER = edge_roughness()
(Xcln, Ycln, freq, FourierPow) = LER.LER_analysis(image, 50e-6, 3) #image, image width (m), threshold for outliers
...
#             axs[1,2].set_title("NILS - mean")
#             axs[1,2].set_ylabel("NILS")
#             axs[2,0].plot(sigma, nilsSTD, 'x:')
#             axs[2,0].set_title("NILS - std")
#             axs[2,0].set_ylabel("RMS")
