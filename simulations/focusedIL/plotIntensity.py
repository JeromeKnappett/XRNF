...
dirPath = '/user/home/opt/xl/xl/experiments/focusedIL/data/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = ['aerialImage_wap'] #range(180,225,5)

files = [str(o) + '/' + str(o) + '.pkl' for o in order]

labels =  ['focused beam']#[str(o) + 'um' for o in order]
...
mid = np.full((np.shape(files)[0],2), (1157,20475+3534))  #np.full((np.shape(files)[0],2), (360, 12250+127))
...
A = 1.0e4 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
k = 2*np.pi/wl
m = 1 # order of diffracted beams from each grating
d1 = pitch #24e-9 #100e-9 # grating spacing
d2 = 1.0188*pitch
# angle between the beams from each grating
thetaUF = np.arcsin(m *wl/d1)
...
IPf = interferenceGratingModelsJK.interferenceIntensity(xP[0],k,thetaF,A=A,polarisationModes=('TE','TE'))
IPte = interferenceGratingModelsJK.interferenceIntensity(xP[0],k,thetaUF,A=A,polarisationModes=('TE','TE'))
IPtm = interferenceGratingModelsJK.interferenceIntensity(xP[0],k,thetaUF,A=A,polarisationModes=('TM','TM'))
...
#            plt.plot(xP[0]*1e9,IPuf/np.max(IPuf), ':', label='Eq.2.3.45 - pitch = 100nm')
            plt.plot(xP[0]*1e9,IPf/np.max(IPf), ':', label=f'Eq.2.3.45 - pitch = {round_sig(d2*1e9,4)} nm')
            ...
            plt.plot(xP[0]*1e9,IPuf, ':', label='Eq.2.3.45 - pitch = 100nm')
            plt.plot(xP[0]*1e9,IPf, ':', label='Eq.2.3.45 - pitch = 200nm')
...
if contrast:
    ...
    nilsC = [interferenceGratingModelsJK.NILS(p,x, d2/4, show=False) for p, x in zip(profiles, xP)]
#    fourierC = [interferenceGratingModelsJK.gratingContrastFourier(p,x*1e6, show=False) for p, x in zip(profiles,xP)] #Cf,  Am, Fr, peakFr - Still unsure but seems good
    fidel = [interferenceGratingModelsJK.fidelity(p,IPf) for p in profiles]   # fidelity based on comparison to model
    ...
    if split:
        ...
        fig, axs = plt.subplots(2,3)
        axs[0,0].plot(order, michelsonC, 'x:')
        axs[0,0].set_title("Michelson")
        axs[0,0].set_ylabel("Contrast")
        axs[0,1].plot(order, rmsC, 'x:')
        axs[0,1].set_title("RMS")
        axs[0,2].plot(order, [c[0] for c in compositeC], 'x:')
        axs[0,2].set_title("Composite")
        axs[1,0].plot(order, [n[5] for n in  nilsC], 'x:')
        axs[1,0].set_title("NILS - rms")
        axs[1,0].set_ylabel("Contrast")
        axs[1,1].set_title("NILS")
        axs[1,1].plot(order, fidel, 'x:')
        axs[1,1].set_title("Fidelity")
        axs[1,2].plot(order, [n[0] for n in nilsC], 'x:')
        axs[1,2].set_title("NILS - mean")
        axs[1,2].set_ylabel("NILS")
#        for ax in fig.axes:
