...
#    print('Time taken to convert to complex wavefield (s): {}'.format(end1 - start1))
    ...
    A_T = cfrT #sampleField(cfrT, Fx=Fx, Fy=Fy, Limit = 1000000)
    A_H = cfrH #sampleField(cfrH, Fx=Fx, Fy=Fy, Limit = 1000000)
    A_V = cfrV #sampleField(cfrV, Fx=Fx, Fy=Fy, Limit = 1000000)
    
    I = cfrT.conjugate()*cfrT #sampleField(cfrT.conjugate()*cfrT, Fx=Fx,Fy=Fy,Limit=1000000) #sampleField(cfr.get_intensity(), Fx=Fx, Fy=Fy, Limit=200)
...
def stokesFromJ(Jxx,Jxy,Jyx,Jyy, pathSJ=None, pathP=None):
    ...
    return S0, S1, S2, S3
# %%
def stokesFromE(ex,ey,et):

    print("-----Calculating Stokes arrays-----")
    start2 = time.time()
    S0 = np.array([ex.conjugate()*a for a in ex.flatten()])
...
# %%
def sampleField(A,Fx,Fy,Limit = 750000):
    """
    Parameters
    ----------
    A : 2D array
...
#    names =['atM1']
    ...
    names = ['20nmTMincident','20nmTMexit','20nmTEincident','20nmTEexit']
    ...
    for ch, cv, r, n in zip(Ex,Ey,res,N): 
        ...
        plotStokes(sn,dx=r[0], dy=r[1], extra=False, savePath=None)
