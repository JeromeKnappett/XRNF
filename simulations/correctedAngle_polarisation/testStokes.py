...
#    names =['atM1']
    ...
    #'/user/home/opt/xl/xl/experiments/lowEnergyBeam/data/'
    names = ['8nmTMEfield','8nmTEEfield']
    #['atM1']#['8nmTMincident','8nmTEincident']#['20nmTMincident','20nmTMexit','20nmTEincident','20nmTEexit','24nmTMincident','24nmTMexit','24nmTEincident','24nmTEexit']
...
#    Iyrat = [Iytot[1]/Iytot[0], Iytot[3]/Iytot[2]]
    ...
    plotNames = ['8nm']#,'8nmTE']#,'24nmTM','24nmTE']
    ...
    for e in range(0,int(len(Itot)/2)):
        ...
        sRAT = np.sum(STOKES[e2p1][0])/np.sum(STOKES[e2][0])
        ...
        ax[0].plot(plotNames[e],iRAT, 'x',label='total flux')
        ax[0].plot(plotNames[e],sRAT, 'x',label='total S0')
    ...
    Idiff = Intensity[0]/Intensity[1]
    
    plt.imshow(Idiff,aspect='auto')
    plt.colorbar()
    plt.show()
#    
#    plt.plot(['TM','TE'],Irat)
