...
dirPath ='/user/home/srw2/SRW/experiments/coherentModeTest/data/' 
order = ['test_ch' + str(n) for n in [6]]##,4000,20000]] 
# ['WBSi_ne' + str(n) for n in [100,1000,2000,4000,10000,20000,30000]] 
...
if Efile == None:
    ...
    #    :param int _width_pixels: the width of the final plot in pixels  
            ...
            # plt.imshow(abs(np.squeeze(I))**2, aspect='auto')
            plt.imshow(abs(np.squeeze(I)), aspect='auto')
            plt.title("Multi-electron intensity: " + name)
    ...
    #            plt.savefig(dirPath + "plots/" + "Propintensity" + name + ".png")
            ...
            # plt.imshow(np.squeeze(I)**2,aspect='auto')
            # plt.title('Multi-e again')
            # plt.colorbar()
            # plt.show()
    ...
    #            imageio.imwrite(dirPath+'tiffs/'+t[0:len(t)-1]+'intensity.tif',np.float32(I[0,:,:]))
            ...
            eff = 0.1#0.04451154279639219 # cff = 2, grating E = 10% - fixed angle
            ...
            G, C, S = IoverGrating(I, GA=Ga, dx=dy, dy=dy, show=False)
            _G, _C, _S = IoverGrating(I, GA=1.0e-3, dx=dy, dy=dy, show=False)
