...
path = '/user/home/opt/xl/xl/experiments/spectralTuningBEUV/data/'
...
fileName = ['at' + str(plane) + '_' + str(SE_ME) + '_m' + str(m) + '_' + str(tuning) + '/' for m in M]

if SE_ME == 'SE':
#    I = [tifffile.imread(path + 'm' + str(m) + '_atMask_SE/intensity.tif')
    wPath = [path + f + '/wf_final.hdf' for f in fileName]
    I = []
    dx,dy = [], []
    for p in wPath:
        wf = Wavefront()
...
#plt.xlim([1000,3800])
plt.title('horizontal profiles at ' + str(plane) + ' (' + str(tuning) + ')')
...
#plt.xlim([2000,2800])
plt.title('vertical profiles at ' + str(plane) + ' (' + str(tuning) + ')')
plt.xlabel('position [mm]')
plt.ylabel('intensity')
plt.legend()
plt.show()
