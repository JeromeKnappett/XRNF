...
def propWavefront():
    ...
    path = '/user/home/opt/xl/xl/experiments/focusedOffAxis6/data/FZPexit_20umAngled'
#    path = ['/user/home/opt/xl/xl/experiments/focusedOffAxis4/data/exitAng_' + str(i) for i in values] 
...
#            dVol.propagate(wf)
        ...
        outPath= path + '/driftVolBiggerArea/'
        savePath = path + '/propagationPlotBiggerArea.png'
        
        zRange = 20e-3 #0.0196 #0.0002 # 0.0032664 #400e-6  #0.0196
        zPlanes= 50
...
#        distances=(0.0,zRange,zPlanes)
#        dVol=driftVol(distances,
#                      [0, 0, 1.0, 0, 0, 4.0, 1.0, 1.0, 0.125, 0, 0, 0],# Use_PP(),#(semi_analytical_treatment=1.0,zoom=1, sampling=1),
#                      outputPath = outPath, 
#                      zoomXOutput = 1.0,     # reduce size of output, keep all significant features
#                      zoomYOutput = 1.0,     # reduce size of output, keep all significant features
#                      resampleOutput=1.0,     # reduce size of output, proportional loss of resolution
...
#        
        plotPropWave(outPath + 'intensity/',
                     zRange,zPlanes,res=dx,axis='hor',log=True, lMin=1e11, savePath=savePath,plotProfiles=True)
