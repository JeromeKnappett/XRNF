...
# %%
def profileMI(cpX, cpY, Ix, Iy, pathCP=None, pickled=False):
    """
...
#    plt.plot(jVd/np.max(jVd), label='diagonal cut')#label="Vertical Mutual Intensity")
    ...
    if pathCP and pickled:
        import pickle
        with open(pathCP[0:len(str(pathCP))-4]+'HorProf.pkl', "wb") as f:
            pickle.dump(abs(j_H), f)
    else:
        pass
...
def testCoherenceProfile():
    ...
    if fromPickle:
        ...
        res = [(3.3419308755228203e-07, 6.079526002983963e-06),
               (3.3452615124385716e-07, 6.074644954894064e-06),
               (3.350840753972245e-07, 6.074714037379948e-06)]
    
##           (3.342511331206482e-07, 6.073634233862372e-06),
##           (3.343292278054577e-07, 6.0762943979168535e-06),
#           (3.3452615124385716e-07, 6.074644954894064e-06),
##           (3.3477438462533694e-07, 6.073654529757706e-06),
#           (3.350840753972245e-07, 6.074714037379948e-06),
#           (3.354515464962258e-07, 6.075388651610439e-06),
#           (3.3686215795505543e-07, 6.073797395069954e-06)
#           ]
...
#    dx = 2.4633703639094306e-06, 2.509768682470555e-06,2.704305789676536e-06
    ...
    # _I = [np.sqrt((abs(ex)**2) + (abs(ey)**2)) for ex,ey in zip(Eh,Ev)]
    ...
    for ce, ch, cv, s, r in zip(cE,Eh,Ev,sX, res):#,_I): 
        # I = (np.sqrt(ce.real**2 + ce.imag**2))**2
        I = abs(ch.conjugate()*ch)
        # I = (abs(ch)**2) + (abs(cv)**2)
        # print(np.shape(I))
        # I = abs(np.sqrt(ce.conjugate()*ce))
        ...
        eff = 0.05113671435543832 # cff = 3, grating E = 10% - fixed angles 
        ...
        #E field units: sqrt(Photons/s/0.1%bw/mm^2)
        
        # ph/s/0.1%bw/mm^2 to ph/s/mm^2
        I = I * bw/0.1 * eff
        ...
        #old method
        # Imtot_w = ((I) * (1/2) * _c * epsilon) / 10000 #((epsilon)/2) # W/m^2  # changed from I*(_c/2*epsilon) - Jk 09/10/23
        # ITOT = (Imtot_w * ev ) / 184.76 #(1/1.602e-19) * (1/184.76) # ph/s
        # # ITOT = ITOT*
        
        # new method
        Im = (I) / (1.0e-6)    #ph/s/m^2
        ITOT = Im * (r[0] * r[1]) #ph/s
        
        Nx,Ny = np.shape(ITOT)[1], np.shape(ITOT)[0]
        
        Imtot = (np.sum(ITOT) / (r[0]*r[1]*Nx*Ny*10000) ) #ph/s/0.1%bw/cm^2
        
        print(np.shape(ITOT))
        
        plt.imshow(ITOT,aspect='auto')
        plt.colorbar()
        plt.title(str(s))
        plt.show()
        ...
        G,C,S = IoverGrating(ITOT, GA=Ga, dx=r[0], dy=r[1],show=True)
        _g,C,_s = IoverGrating(ITOT,GA=0.5e-3, dx=r[0], dy=r[1], show=True)
        
#        J,Jxx,Jxy,Jyx,Jyy,S0,S1,S2,S3 = mutualIntensity(ce,ch,cv,Fx=0.1,Fy=0.1,dx=r[0],dy=r[1])
    
        # cH, cV, jH, jV, jHh, jVh = profileMI(ex,ey,Ix,Iy,pathCP='/user/home/opt/xl/xl/experiments/correctedAngle_coherence/coherence.png',pickled=True)
...
#        axs[1].plot(jVd/np.max(jVd), label='normalised intensity (x=0) - from MI')
        ...
        if toPickle:
            pathCP='/user/home/opt/xl/xl/experiments/correctedAngle_coherence/pickles/'
            import pickle
            with open(pathCP +'xProf' + str(s) + '.pkl', "wb") as f:
                pickle.dump(Cx, f)
            with open(pathCP +'NormalisedIx' + str(s) + '.pkl', "wb") as f:
                pickle.dump(Ix/np.max(Ix), f)
            with open(pathCP +'yProf' + str(s) + '.pkl', "wb") as f:
                pickle.dump(Cy, f)
            with open(pathCP +'combined' + str(s) + '.pkl', "wb") as f:
                pickle.dump([Ix/np.max(Ix),Cx,r], f)
        else:
            pass
    ...
    #    
        ...
        cly_1 = getCoherenceLength(0.8,cV = np.array(Cy),dy=r[1])*r[1]
        ...
        IC.append(C/((0.5e-3 * 0.5e-3 * 10000)))
...
#            imageio.imwrite(path + str(s) + 's3.tif',np.float32(S3))
    ...
    plt.plot(sX,IG, ':x')
    plt.xlabel('Slit Width (x) [microns]')
    plt.ylabel('Intensity over Grating [ph/s/cm^2]')
    plt.legend()
    plt.show()
    
    plt.plot(sX,IC, ':x')
    plt.xlabel('Slit Width (x) [microns]')
    plt.ylabel('Intensity over beam center [ph/s/cm^2]')
    plt.legend()
    plt.show()
    ...
    if toPickle:
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence/pickles/cXlength.pkl', "wb") as f:
            pickle.dump(cLx, f)
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence/pickles/totI.pkl', "wb") as g:
            pickle.dump(ImTOT, g)
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence/pickles/totI_G.pkl', "wb") as g:
            pickle.dump(IG, g)
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence/pickles/totI_C.pkl', "wb") as g:
            pickle.dump(IC, g)
    else:
        pass
#    plt.plot(cL)
