...
dirPath = '/user/home/opt/xl/xl/experiments/focusedOffAxis/data/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = ['aerialImage_100x100_100h'] #range(180,225,5)
...
N = 2300                             # number of pixels to take for line profile  - 1200 for roughness aerial images
n = 1                                 # number of pixels to average over for line profile - 15 for roughness aerial images
plotRange = 12000                       # range of aerial image plot in nm

# middle pixels of images
mid = np.full((np.shape(files)[0],2), (900, 18375-312)) #np.full((np.shape(files)[0],2), (360, 12250+126))
...
#    fourierC = [interferenceGratingModelsJK.gratingContrastFourier(p,x*1e6, show=False) for p, x in zip(profiles,xP)] #Cf,  Am, Fr, peakFr - Still unsure but seems good
    fidel = [interferenceGratingModelsJK.fidelity(p,IPte) for p in profiles]   # fidelity based on comparison to model
