...
dirPath = '/user/home/opt/xl/xl/experiments/EUVpolarisation/data/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = ['24nmTE','24nmTM']#['24nmTE']#,'24nmTM']
mag = [1,1,1,1,1]
...
_N = range(10000,10001,1000)  #range(1200,1201) #                            # number of pixels to take for line profile  - 1200 for roughness aerial images
...
#
#_N = range(6000,6001,1000)  #range(1200,1201) #                            # number of pixels to take for line profile  - 1200 for roughness aerial images
...
#np.full((np.shape(files)[0],2), (150, 131000))  #1250 for original
...
for N in _N:
    ...
    A = np.max(profiles[0])*1.5e-6 #45.0e5 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
    A2 = 18.0e5
    k = 2*np.pi/wl
