...
dirPath = '/user/home/opt/xl/xl/experiments/focusedIL3/data/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = ['aerialImage190um'] #range(180,225,5)
...
plotRange = 3000                       # range of aerial image plot in nm

# middle pixels of images
mid = np.full((np.shape(files)[0],2), (360, 12250+126)) # np.full((np.shape(files)[0],2), (900,18454)) #np.full((np.shape(files)[0],2), (360, 12250+126))
...
d2 = round_sig(1.05*pitch,5)
# angle between the beams from each grating
thetaUF = np.arcsin(m *wl/d)
...
if contrast:
    ...
    nilsC = [interferenceGratingModelsJK.NILS(p,x, d2/4, show=True) for p, x in zip(profiles, xP)]
