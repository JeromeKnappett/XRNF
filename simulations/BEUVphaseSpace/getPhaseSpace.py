...
def electronPhaseSpace(file,bins,plot=True):
    ...
    if plot:
        scatter_with_histograms([a*1e6 for a in x],[b*1e6 for b in _x], bins=bins,labels=["x [$\mu$m]","x' [$\mu$rad]"])
        scatter_with_histograms([a*1e6 for a in y],[b*1e6 for b in _y], bins=bins,labels=["y [$\mu$m]","y' [$\mu$rad]"])
    else:
        pass
...
# eFiles = ['/user/home/opt/xl/xl/experiments/BEUVphaseSpace/data/WBSi_n' + str(n) + '/res_int_pr_me.datelectron_phase_space.csv' for n in [1,2,3,4]]
...
# for ef in eFiles:
#     data = electronPhaseSpace(ef,bins=40,plot=False)
...
#     # ebeamData.append(data)
#     # print(np.shape(data))
#     # print(np.shape(ebeamData))
...
# dirPath = '/user/home/opt/xl/xl/experiments/BEUVphaseSpace/data/WBSi_ne' + str(ne) + '/'
...
# ySize = [x for xs in ySize for x in xs]
# yDiv = [x for xs in yDiv for x in xs]
# eSpread = [x for xs in eSpread for x in xs]

# # Lines added by JK - 05/06/24
# import pandas as pd
...
# df.to_csv(dirPath + "electron_phase_space_total.csv", index=False)

# print(xSize)

# totalData = electronPhaseSpace(dirPath + "electron_phase_space_total.csv", 60)


# plt.scatter(xSize,xDiv)
# plt.show()
...
path = '/user/home/opt/xl/xl/experiments/BEUVphaseSpace/data/'
#slit sizes]
wl = 6.7e-9
...
#           'SSA_i', 'SSA_e',
#           'mask_i']
planes = ['WBSi_n' + str(n) for n in [1,2,3,4,5]]
