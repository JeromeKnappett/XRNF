...
def fit_psd(frequencies, psd_values, model_function, bounds=None, p0=None, plot_fit=False, savePath=None):
    """
    ...
    if plot_fit:
        ...
        plt.plot(frequencies[1::]/1e6, [np.cbrt(_p)*1e9 for _p in psd_values[1::]], 'b.', label='Unbiased PSD Data')
        # plt.plot(frequencies[1::], guess[1::], ':', label='Initial Guess')
        plt.plot(frequencies[1::]/1e6, [np.cbrt(_p)*1e9 for _p in model_function(frequencies, *popt)[1::]], 'r--', label='Fitted Model')
...
order = [0,3,4,5,6]#
files = [str(o) + 'nm/' + str(o) + 'nm.pkl' for o in order]
labels =  ['3 nm']
...
numlines = 2 #(int(Y/hp) - 1) // 2  #99 #int(Y//hp-2)# 125
shiftY = 10
extraY = hp / 3
...
for i,a in enumerate(images):
    ...
    plt.plot([_f/1.0e6 for _f in f[1::]], [np.cbrt(_p)*1e9 for _p in p[1::]],'.', label='biased',alpha=0.2)
    ax = plt.gca()
    ax.xaxis.set_major_formatter(ticker.FormatStrFormatter('%0.0e'))
    plt.yscale('log')
    plt.xscale('log')
    plt.xlabel('Spatial Frequency (\u03bcm$^{-1}$)')
    plt.ylabel('PSD [nm$^3$]')
    ...
    plt.plot([_f/1.0e6 for _f in f[1::]], [np.cbrt(_p)*1e9 for _p in p_ub[1::]],'.', label='unbiased',alpha=0.2)
    plt.plot([_f/1e6 for _f in f[1:len(PSD)]],[np.cbrt(_p)*1e9 for _p in PSD[1::]],'black', label='analytic')
    plt.legend()
    plt.savefig(dirPath + 'PSD_biased_' + str(i) + '.svg', bbox_inches='tight')
    plt.show()
    ...
    p_ub = np.array(p_ub)
    popt, pcov = fit_psd(f,p_ub,analyticalPSD_unbiased,p0=[PSD0,H,c],bounds = B,plot_fit=True, savePath=dirPath + 'PSD_' + str(i) + '.svg')
...
# if save2pick:
#     with open(dirPath + 'LER__maskLER_retry.pkl', "wb") as f:
