...
order = [50,75,100,125,150,175,200]#,300]#
files =  [str(o) + 'um/' + str(o) + 'um.pkl' for o in order]
 # [str(o) + 'um/' + str(o) + 'um.pkl' for o in order]
# [str(o) + 'um_50SSA/' + str(o) + 'um_50SSA.pkl' for o in order]
labels = [str(o) for o in order] # ['3 nm']
...
numlines = (int(Y/hp) - 1) // 2 - 1  #99 #int(Y//hp-2)# 125
shiftY = 10
extraY = hp / 2

peaks, properties = find_peaks(profiles[0],distance=pitch-5)
troughs, properties = find_peaks(1 - profiles[0]/np.max(profiles[0]),distance=pitch-5)
...
for i,a in enumerate(images):
    ...
    peaks, properties = find_peaks(profiles[i],distance=hp)#pitch-5)
    troughs, properties = find_peaks(1 - profiles[i]/np.max(profiles[i]),distance=hp)#pitch-5)
    ...
    for n in range(numlines):
        ...
        # if n+1 < numlines:
        if peaks[n]-extraY < 0: # making sure the first point in ROI is above 0
            lineImage1 = a[int(peaks[n]-peaks[n]):int(troughs[n]+extraY),:].copy()
        else:
        ...
        Ycln1 = [min(abs(x), pitch_m/2) for x in Ycln1] #[Ycln1 > pitch/2] = pitch/2
        Ycln2 = [min(abs(x), pitch_m/2) for x in Ycln2] #Ycln2[Ycln2 > pitch/2] = pitch/2
    ...
    H = 1.0 #1.5
    c = 50.0e-9 #C #102.0e-9#5.68e-8#4.0e-9 #3.5e-9
...
if save2pick:
    with open(dirPath + 'LER__BEUVcoherence_350SSA.pkl', "wb") as f:
