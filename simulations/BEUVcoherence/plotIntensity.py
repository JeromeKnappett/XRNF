...
order = ['50um','75um','100um','125um','150um','175um','200um']
# ['50um_50SSA','75um_50SSA','100um_50SSA','125um_50SSA','150um_50SSA','175um_50SSA','200um_50SSA','300um_50SSA']
# ['50um','75um','100um','125um','150um','175um','200um']
#['50um_50SSA','75um_50SSA','100um_50SSA','125um_50SSA','150um_50SSA','175um_50SSA','200um_50SSA','300um_50SSA']
#['100um','200um','100um_50SSA','200um_50SSA','300um_50SSA']

cY = [2,2,2,2,2,4,4,4,4,4,6,6,6,6,6,8,8,8,8,8,10,10,10,10,10]
sigma = np.zeros_like(order)#[0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5]
files = [str(o) + '/' + str(o) + '.pkl' for o in order]
mag = np.full((np.shape(files)[0]), 1)


labels =  ['50 um', '75 um', '100 um', '125 um', '150 um','175 um', '200 um']#, '300 um']
#['100 um', '200 um','100um - 50um SSA','200um - 50um SSA','300um - 50um SSA']#, '300 um SSA']
...
N = 700                           # number of pixels to take for line profile  - 1200 for roughness aerial images
...
mid = np.full((np.shape(files)[0],2), (336,21746))
#     [(336,21747),
#       (336,21747),
#       (336,21747),
#       (336,21747)]
...
#       (148,126000+103),
#       (150,131000+72),
#       (148,126000+103)]#np.full((np.shape(files)[0],2), (150, 130000))
...
A = 4.0e5 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
A2 = 1.0e5
k = 2*np.pi/wl
...
if toPickle:
    for i, p in enumerate(profiles):
        with open(dirPath + str(order[i]) + 'Profile.pkl', "wb") as f:
...
if show:
    if len(tiffs) == 1:
        ...
        # fig.tight_layout()
        plt.savefig(dirPath + 'allAerialImagesFar_350um.svg', bbox_inches='tight')
        plt.show()
...
#            axs[i].set_yticklabels([round_sig(np.max(yP[i])*y*1e9) for y in [0,-1, -2/5,2/5,1]])
        ...
        axs[len(tiffs)-1].set_xticklabels([round_sig(2*N*res[i][0]*(a/(8))*1e9) for a in range(-int((8)/2),int((8/2 + 1)))])
        axs[len(tiffs)-1].set_xlabel('x-position [nm]')
        axs[int(len(tiffs)/2)].set_ylabel('y-position [nm]')
        plt.savefig(dirPath + 'allAerialImagesClose_350um.svg', bbox_inches='tight')
        if save:
        ...
        # fig.tight_layout()
        plt.savefig(dirPath + 'allAerialImagesProfile_350um.svg', bbox_inches='tight')
        plt.show()
if subtractAperture:
...
#            plt.plot(idealXP*1e9,idealProfile/np.max(idealProfile), '-', label='non-focused beam')
            plt.plot(xP[0]*1e9,IPte[0]/np.max(IPte[0]), ':', label='Eq.2.3.45 - TE')
...
#            plt.plot(xP[0]*1e9,IPf, ':', label='Eq.2.3.45 - pitch = {}nm'.format(d2*1e9))
            plt.plot(xP[0]*1e9,IPte[0], ':', label='Eq.2.3.45 - TE')
...
if contrast:
    if split:
        ...
        fig, axs = plt.subplots(2,3)
        try:
            axs[0,0].errorbar(labels, conM, yerr=eM, fmt='x:', capsize=2.0)
            axs[0,0].set_title("Michelson")
            axs[0,0].set_ylabel("Contrast")
            axs[0,1].errorbar(labels, conRMS, yerr=eRMS, fmt='x:', capsize=2.0)
            axs[0,1].set_title("RMS")
            axs[0,2].errorbar(labels, conC, yerr=eC, fmt='x:', capsize=2.0)
            axs[0,2].set_title("Composite")
            axs[1,0].errorbar(labels, conNRMS, yerr=eNrms, fmt='x:', capsize=2.0)
            axs[1,0].set_title("NILS - rms")
            axs[1,0].set_ylabel("Contrast")
            axs[1,1].set_title("NILS")
            axs[1,1].errorbar(labels, FID, yerr=eF, fmt='x:', capsize=2.0)
            axs[1,1].set_title("Fidelity")
            axs[1,2].errorbar(labels, conN, yerr=eN, fmt='x:', capsize=2.0)
            axs[1,2].set_title("NILS - mean")
            axs[1,2].set_ylabel("NILS")
        except NameError:
            print(mC)
            axs[0,0].plot(labels, np.squeeze(mC),'x:')
            axs[0,0].set_title("Michelson")
            axs[0,0].set_ylabel("Contrast")
            axs[0,1].plot(labels, np.squeeze(rC),'x:')
            axs[0,1].set_title("RMS")
            axs[0,2].plot(labels, np.squeeze(cC),'x:')
            axs[0,2].set_title("Composite")
            axs[1,0].plot(labels, np.squeeze(Nrms),'x:')
            axs[1,0].set_title("NILS - rms")
            axs[1,0].set_ylabel("Contrast")
            axs[1,1].set_title("NILS")
            axs[1,1].plot(labels, np.squeeze(F),'x:')
            axs[1,1].set_title("Fidelity")
            axs[1,2].plot(labels, np.squeeze(NI),'x:')
            axs[1,2].set_title("NILS - mean")
            axs[1,2].set_ylabel("NILS")
    #        for ax in fig.axes:
    #            ax.set_xticklabels(order, rotation=45, ha='right')
        ...
        if toPickle:
            with open(dirPath + 'contrastMetrics.pkl', "wb") as f:
                pickle.dump([labels,np.squeeze(mC),np.squeeze(rC),np.squeeze(cC),np.squeeze(Nrms),np.squeeze(F),np.squeeze(NI)], f)
...
with open(dirPath + 'LWR.pkl', "wb") as f:
    pickle.dump(LWR, f)
with open(dirPath + 'dataStructure_350um.pkl', "wb") as g:
