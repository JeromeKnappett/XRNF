...
def round_sig(x, sig=2):
    ...
    # %%
dirPath = '/user/home/opt/xl/xl/experiments/'
files = ['maskLER1/dataStructureLER1NEW.pkl','maskLER2/dataStructuremaskLER2NEW.pkl']#,'contrastMetrics350_2d.pkl']
labelX = '$LER$ [um]'#'$\sigma [nm]$'
labelY = ['$C_M$','$C_{RMS}$', '$C_c$','Fidelity','NILS', 'NILS$_{\sigma_n}$', 'LWR [nm]', '$\eta$' ,'LER [nm]']
...
LERpick = pickle.load(open(dirPath + 'maskLER1/data/LER__maskLER1.pkl', 'rb'))
pick1 = pickle.load(open(dirPath + files[1], 'rb'))
# pick2 = pickle.load(open(dirPath + files[1], 'rb'))

print(LERpick[0])

A1 = [p for p in pick1[2::]]
...
X1 = [3.15,5.5,7.8,10.0,12.5,14.5,16.0,17.0] #[float(p[4:-4]) for p in pick1[0]]
...
ax[0,1].plot(X1,[a / 0.6 for a in A1[2]],'x:',label='50 um')
