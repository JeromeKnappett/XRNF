...
#    
    ...
    return meanNILS,  meanLW, NILS, LW, rmsNILS, rmsLW, rmsdLW
...
def test():
    ...
    m1 = 67045 - shift #21159 #15919
    m2 = 70348 - int(shift*(1/1.353480180144755)) #16377
...
#    profileV1 = np.linspace(-0.5*profileV*profileVs, 0.5*profileV*profileVs,profileV)
    ...
    # Amplitude of both beams (assumed equal)
    A = 1.e5 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
...
#    plt.plot(x1*1e6,I1, ':', label='Model - TE')
    for i, a in enumerate(aerialImages): #[1::]
         ...
         plt.plot(profileX*1e6, a, ':o', label=labels[i])
    for i, b in enumerate(aerialImages1):
         plt.plot(profileX1*1e6, b, ':o', label=labels1[i])
...
#    fidel.insert(1,fidel1[1])
    ...
    avIn = [np.mean(aerialImages), np.mean(aerialImages1)]

    printContrast = True
    
    if printContrast:
        for i in range(0,len(michelsonC)):
            print(" ")
...
#            print(labels[i])
            ...
            print("Average intensity                {}".format(avIn[i]))
...
def testTETMContrast():
    ...
    k = 2*np.pi/wl
    m = 1 # order of diffracted beams from each grating
    pitch = np.linspace(10e-9,50e-9,20)#24e-9 #24e-9 #100e-9 # grating spacing
...
#    plt.plot(I_TM[0])
    ...
    for p, ite, itm in zip(pitch, np.array(I_TE), np.array(I_TM)):
        axs[0].plot(itm, label='p=' + str(p))
        axs[1].plot(ite, label='p=' + str(p))
    axs[0].legend(title='TM')
    axs[1].legend(title='TE')
#    plt.legend()
    plt.show()
