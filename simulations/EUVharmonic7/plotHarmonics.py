...
def round_sig(x, sig=2):
    if x != 0:
        return round(x, sig-int(floor(log10(abs(x))))-1)
            ...
            return x
#%%
path = '/user/home/opt/xl/xl/experiments/EUVbeamProfile/data/'

M = [1,3,5,7,9]
numPoints = 1000

dI = [0.05445441159673731, 0.003055435933184331, 0.0003164088236775894, 3.066014143408092e-05, 2.8895631847948516e-06]
dI = [d/np.max(dI) for d in dI]
print(dI)
...
#    I = [tifffile.imread(path + 'm' + str(m) + '_atMask_SE/intensity.tif')
    wPath = [path + 'm' + str(m) + '_atWBS_SE/wf_final.hdf' for m in M]
    I = []
    dx,dy = [], []
    for p in wPath:
        wf = Wavefront()
...
#    m1 = pickle.load(open(path + 'm1_atMask_ME/m1_atMask_ME.pkl', 'rb'))
#    m3 = pickle.load(open(path + 'm3_atMask_ME/m3_atMask_ME.pkl', 'rb'))
#    m5 = pickle.load(open(path + 'm5_atMask_ME/m5_atMask_ME.pkl', 'rb'))

    I = [p[0]*di for p,di in zip(picks,dI)]
...
#print(Nx)
midX = [nx[0]/2 for nx in Nx]
midY = [ny[0]/2 for ny in Ny]

for i,p in enumerate(xProfs):
    plt.plot(p,label='m = '+str(2*i+1))
    plt.xticks([int(Nx[0][0]*(a/8)) for a in range(0,9)],
                [round_sig(Nx[0][0]*dx[i]*(b/8)) for b in range(-4,5)])

#plt.xlim([int(midX[0]-numPoints/2),int(midX[0] + numPoints/2)])
...
plt.xlim([int(midY[0]-numPoints/2),int(midY[0] + numPoints/2)])
...
plt.plot(xProfs[0] + xProfs[1] + xProfs[2],label='Sum')
plt.xticks([int(Nx[0][0]*(a/8)) for a in range(0,9)],
                [round_sig(Nx[0][0]*dx[i]*(b/8)) for b in range(-4,5)])
plt.xlim([int(midX[0]-numPoints/2),int(midX[0] + numPoints/2)])
plt.title('Total Horizontal Profile')
plt.xlabel('Position [mm]')
plt.ylabel('Intensity [$ph/s/cm^2$]')
plt.legend()
plt.show()

plt.plot(yProfs[0] + yProfs[1] + yProfs[2],label='Sum')
plt.xticks([int(Ny[0][0]*(a/10)) for a in range(0,11)],
            [round_sig(Ny[0][0]*dy[i]*(b/10)) for b in range(-5,6)])
plt.xlim([int(midX[0]-numPoints/2),int(midX[0] + numPoints/2)])
plt.title('Total Vertical Profile')
plt.xlabel('Position [mm]')
plt.ylabel('Intensity [$ph/s/cm^2$]')
plt.legend()
plt.show()
...
for i,p in enumerate(I):
    fwhmx,fwhmy = getFWatValue(p,dx[i]*1e-3,dy[i]*1e-3,show=False)
    FWHMx.append(fwhmx)
    FWHMy.append(fwhmy)
...
fwhmX,fwhmY = getFWatValue(I[0] + I[1] + I[2],dx[0]*1e-3,dy[0]*1e-3,show=True)
