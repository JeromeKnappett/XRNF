...
def testCoherenceProfile():
    ...
    sX = [25,50,75,100,125,150,175,200,225,250,275,300,325,350,375]
    sY = np.full_like(sX,100)
    files = [path + 'sx' + str(sx) + 'sy' + str(sy) + '/sx' + str(sx) + 'sy' + str(sy) + 'Efields.pkl' for sx,sy in zip(sX,sY)]
...
#    dx = 2.4633703639094306e-06, 2.509768682470555e-06,2.704305789676536e-06
    ...
    for ce, ch, cv, s, r in zip(cE,Eh,Ev,sX, res): 
        ...
        Imtot = np.sum(ITOT*r[0]*r[1])
        
#        J,Jxx,Jxy,Jyx,Jyy,S0,S1,S2,S3 = mutualIntensity(ce,ch,cv,Fx=0.1,Fy=0.1,dx=r[0],dy=r[1])
    
        cH, cV, jH, jV, jHh, jVh = profileMI(ex,ey,Ix,Iy,pathCP='/user/home/opt/xl/xl/experiments/correctedAngle_coherence3/coherence.png',pickled=True)
...
#        axs[1].plot(jVd/np.max(jVd), label='normalised intensity (x=0) - from MI')
        ...
        if toPickle:
            pathCP='/user/home/opt/xl/xl/experiments/correctedAngle_coherence3/pickles/'
            import pickle
...
#            imageio.imwrite(path + str(s) + 's3.tif',np.float32(S3))
    ...
    if toPickle:
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence3/pickles/cXlength.pkl', "wb") as f:
            pickle.dump(cLx, f)
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence3/pickles/totI.pkl', "wb") as g:
            pickle.dump(ImTOT, g)
    else:
        pass
#    plt.plot(cL)
