...
dirPath = '/user/home/opt/xl/xl/experiments/maskLER_retry/data/' #correctedAngle_roughness/data/'
savePath = None #'/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = [0,1,2,3,4,7] #range(19,20)
cY = [10.0,10.0,10.0,10.0,10.0,10.0] #[10,10,10,10,10]
sigma = [0.0,0.5,1.0,1.5,2.0,3.5] #[0.5,1.0,1.5,2.0,2.5]
...
dims = (2,3)
labels = ['\u03C3 = 0.0 nm ','\u03C3 = 0.5 nm ','\u03C3 = 1.0 nm ','\u03C3 = 1.5 nm ','\u03C3 = 2.0 nm ','\u03C3 = 3.5 nm']
...
N = 4000#1200                               # number of pixels to take for line profile  - 1200 for roughness aerial images
n = 300                                 # number of pixels to average over for line profile - 15 for roughness aerial images
plotRange = 1000                       # range of aerial image plot in nm

# res and middle pixels of images
# res = np.full((np.shape(files)[0],2), [2.506451563950617e-10,5.015047928163975e-10])
mid = np.full((np.shape(files)[0],2), (2772, 25576))
pitch = 100e-9
...
# idealPick = pickle.load(open(dirPath + '0/0.pkl'))
# idealTif = idealPick[0] #tifffile.imread('/user/home/opt/xl/xl/experiments/correctedAngle_roughness/data/ideal/idealintensity.tif')
...
#            # ax.set_xticklabels(labels, rotation=45, ha='right')
    ...
    if twoD:
        ...
        for e, i in enumerate(images):
            numlines = int((N*res[e][0]) / (pitch/4))
            hp = (pitch/res[e][0])/2
            ...
            for n in range(0,numlines):
                ...
                print('Number of pixels per line:   ',int(hp//2 + n*hp + extraY) - int(n*hp))
            
    # lineImage = gratingImage[int(shiftY + n*hp):int(shiftY + hp//2 + extraY + n*hp),:]
                # lineImage = np.rot90((i/np.max(i))*255)[int(shift + n*hp):int(hp//2 + n*hp + extraY),:]
                lineImage = ((i/np.max(i))*255)[:,int(shift + n*hp):int(hp//2 + n*hp + extraY)]
                plt.imshow(lineImage,aspect='auto')
                plt.title('Line Image 0')
                plt.colorbar()
                plt.show()
                
                lineImage[lineImage<1] == 0
                if lineImage[0,0] != 0:
                    lineImage = abs(255 - lineImage)
                lineImage[lineImage<1] == 0
                
                plt.imshow(lineImage,aspect='auto')
                plt.title('Line Image 1')
                plt.colorbar()
                plt.show()
                # print(X)
                Lbox = X*res[e][1]
                ...
                if n == 0 or n == numlines-1:
                    (Xcln, Ycln, freq, FourierPow) = _LER.LER_analysis(lineImage, [res[e][0],res[e][1]], 10,show=True) #image, image width (m), threshold for outliers
                else:
                    (Xcln, Ycln, freq, FourierPow) = _LER.LER_analysis(lineImage, [res[e][0],res[e][1]], 10,show=False) #image, image width (m), threshold for outliers
      ...
      # %%
        if split:
            ...
            if efficiency:
                ...
                for s,mic,rms,com,fid,nil,nilrms,lwr,ler in zip(_sigma,_michelsonC2d,_rmsC2d,
                                                                _compositeC2d,_fidel,_nils2d,
                                                                _nils2dRMS,_LWR,LER):
                    axs[0,0].plot(s,mic,'x:')
                    axs[0,1].plot(s,rms,'x:')
