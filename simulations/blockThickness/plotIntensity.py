...
def getTransmission(inc,ex):
    inx, iny = np.shape(inc)[1], np.shape(inc)[0]
    exnx, exny = np.shape(ex)[1], np.shape(ex)[0]
    
    Ii = inc[int(iny/2),int(inx/2)]
    Iex = ex[int(exny/2),int(exnx/2)]
    
    plt.plot(inc[int(iny/2),:],label='incident')
    plt.plot(ex[int(exny/2),:],label='exit')
    plt.legend()
    plt.show()
...
dirPath = '/user/home/opt/xl/xl/experiments/blockThickness/data/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = ['100nm','150nm','250nm','300nm']
mag = np.full((np.shape(order)), 1)

#[1.00352485696006,1.00704989017032,1.01057527591621,1.01410119055371,1.01762781054459,1.02115531249182,1.0246838731749,1.02821366958539,1.03174487896247,1.0352776788286]

files = [str(o) + '/' + str(o) + '.pkl' for o in order]

exitFiles = [tifffile.imread(dirPath + str(o) + 'Exit/intensity.tif') for o in order]
exitFiles.append(tifffile.imread(dirPath + '200nmExit/intensity.tif'))
incidentFile = tifffile.imread(dirPath + 'incident/intensity.tif')
T = [getTransmission(incidentFile,e) for e in exitFiles]
...
mid = np.full((np.shape(files)[0],2), (196, 25025-712))#np.full((np.shape(files)[0],2), (150, 130000))
...
resIdeal = [2.5011882651601634e-09, 2.426754806976689e-07]
midIdeal = (196, 12188)
xIdeal = np.linspace(-N*resIdeal[0], N*resIdeal[0],2*N)

idealTif = tifffile.imread(dirPath + 'ideal/idealintensity.tif')
idealImage = idealTif[midIdeal[0]-n:midIdeal[0]+n,midIdeal[1]-N:midIdeal[1]+N]
...
A = 2.0e5 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
A2 = 1.0e5
k = 2*np.pi/wl
...
if plotTogether:
    if normalise:
        ...
        plt.plot(xIdeal*1e9,idealProfile, label='200 nm')
        plt.ylabel('Intensity [ph/s/.1%bw/mm$^2$]')
    if plotFromEq:
        if normalise:
...
if contrast:
    ...
    nilsC = [interferenceGratingModelsJK.NILS(p,x, _d/4, show=True) for p, x, _d in zip(profiles, xP, d)]
#    fourierC = [interferenceGratingModelsJK.gratingContrastFourier(p,x*1e6, show=False) for p, x in zip(profiles,xP)] #Cf,  Am, Fr, peakFr - Still unsure but seems good
    ...
    mCideal = interferenceGratingModelsJK.gratingContrastMichelson(idealProfile)
    rCideal = interferenceGratingModelsJK.gratingContrastRMS(idealProfile)
    cCideal = interferenceGratingModelsJK.meanDynamicRange(idealProfile)
    Nideal = interferenceGratingModelsJK.NILS(idealProfile,xIdeal, d[0]/4, show=False)
    Fideal = interferenceGratingModelsJK.fidelity(idealProfile,IPte[0])
    ...
    mC[0].append(mCideal)
    rC[0].append(rCideal)
    cC[0].append(cCideal[0])
    Nrms[0].append(Nideal[5])
    F[0].append(Fideal)
    NI[0].append(Nideal[0])
...
print(np.shape(np.squeeze(mC)))
newOrder = [0,1,4,2,3]
mC[0] = [mC[0][o] for o in newOrder]
rC[0] = [rC[0][o] for o in newOrder]
cC[0] = [cC[0][o] for o in newOrder]
Nrms[0] = [Nrms[0][o] for o in newOrder]
F[0] = [F[0][o] for o in newOrder]
NI[0] = [NI[0][o] for o in newOrder]
order = [order[o] for o in newOrder]
T = [T[o] for o in newOrder]
#for a in [mC[0],rC[0],cC[0],Nrms[0],F[0],NI[0],order,T]:
#    print(np.shape(a))
#    a = [a[0],a[1],a[4],a[2],a[3]]
#    print(a)
...
if contrast:
    if split:
        ...
        try:
            ...
            axs[0,0].plot(order, np.squeeze(mC), 'x:')
            axs[0,0].set_title("Michelson")
            axs[0,0].set_ylabel("Contrast")
            axs[0,1].plot(order, np.squeeze(rC), 'x:')
            axs[0,1].set_title("RMS")
            axs[0,2].plot(order, np.squeeze(cC), 'x:')
            axs[0,2].set_title("Composite")
            axs[1,0].plot(order, np.squeeze(Nrms), 'x:')
            axs[1,0].set_title("NILS - rms")
            axs[1,0].set_ylabel("Contrast")
            axs[1,1].set_title("NILS")
            axs[1,1].plot(order, T, 'x:')
            axs[1,1].set_title("Transmission")
            axs[1,2].plot(order, np.squeeze(NI), 'x:')
            axs[1,2].set_title("NILS - mean")
            axs[1,2].set_ylabel("NILS")
    ...
    #            ax.set_xticklabels(order, rotation=45, ha='right')
        ...
        ax2 = ax1.twinx()
        ax1.plot(order, np.squeeze(NI), 'x:',color='red')
        ax2.plot(order, T, 'x:',color='blue')
        ax1.set_ylabel("NILS")
...
#        fig, axs = plt.subplots(2,3)
#        
#        axs[0,0].plot(T, np.squeeze(mC), 'x:')
#        axs[0,0].set_title("Michelson")
#        axs[0,0].set_ylabel("Contrast")
#        axs[0,1].plot(T, np.squeeze(rC), 'x:')
#        axs[0,1].set_title("RMS")
#        axs[0,2].plot(T, np.squeeze(cC), 'x:')
#        axs[0,2].set_title("Composite")
#        axs[1,0].plot(T, np.squeeze(Nrms), 'x:')
#        axs[1,0].set_title("NILS - rms")
#        axs[1,0].set_ylabel("Contrast")
#        axs[1,1].set_title("NILS")
#        axs[1,1].plot(T, np.squeeze(F), 'x:')
#        axs[1,1].set_title("Fidelity")
#        axs[1,2].plot(T, np.squeeze(NI), 'x:')
#        axs[1,2].set_title("NILS - mean")
#        axs[1,2].set_ylabel("NILS")
#        fig.tight_layout()
#        plt.show()
