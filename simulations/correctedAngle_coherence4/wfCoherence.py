...
def testCoherenceProfile():
    ...
    # path = 'wavefield_1.pkl'
    path = '/user/home/opt/xl/xl/experiments/correctedAngle_coherence4/data/'
    #slit sizes]
    
    sY = [25,50,75,100,125,150,175,200,225,250,275,300,325,350]
    sX = np.full_like(sY,100)
...
#        axs[1].plot(jVd/np.max(jVd), label='normalised intensity (x=0) - from MI')
    ...
    #    
        ...
        if toPickle:
            pathCP='/user/home/opt/xl/xl/experiments/correctedAngle_coherence4/pickles/'
            import pickle
...
#            imageio.imwrite(path + str(s) + 's3.tif',np.float32(S3))
    ...
    plt.legend()
    plt.show()
    plt.plot(sY,[c*1e3 for c in cLy], ':x', label='vertical - profile')
    plt.xlabel('Slit Width (x) [microns]')
    plt.ylabel('Coherence Length (x) [mm] ')
    plt.legend()
    plt.show()
    ...
    plt.legend()
    plt.show()
    plt.plot(sY,[c*1e3 for c in cLy2], ':x', label='vertical - MI #1')
    plt.xlabel('Slit Width (x) [microns]')
    plt.ylabel('Coherence Length (x) [mm] ')
    plt.legend()
    plt.show()
    ...
    if toPickle:
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence4/pickles/cYlength.pkl', "wb") as f:
            pickle.dump(cLy, f)
        with open('/user/home/opt/xl/xl/experiments/correctedAngle_coherence4/pickles/totI.pkl', "wb") as g:
            pickle.dump(ImTOT, g)
    else:
        pass
#    plt.plot(cL)
