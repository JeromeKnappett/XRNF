...
dirPath ='/user/home/opt/xl/xl/experiments/correctedWBS_beamCharacterisation/data/'  #'/user/home/opt/xl/xl/experiments/fullbeamPolarisation/'#'/user/home/opt/xl/xl/experiments/beamCoherence2/data/' 
order = ['aerialImageLarge','aerialImageEfield'] #0,24,4,20,1,23,9,15,2,22,14,10,3,21,19,5,18,8,16,6,17,13,11,7,12]
...
#            print(ev[n+i])
            ...
            Ey[i2p1] = Ev[nTot + i]
#    
#        Ecom = np.array([eh + ev for eh,ev in zip(Ex,Ey)]) #np.array(Ex) + np.array(Ey))
#        Cim = c2image.c2image(Ecom)
#        
#        plt.imshow(Cim)
#        plt.show()
...
#        fig.colorbar(im,cax=cbar_ax)
        ...
        EhC = [eR + eI*1j for eR,eI in zip(np.float32(I1),np.float32(I2))]
        EvC = [eR + eI*1j for eR,eI in zip(np.float32(I3),np.float32(I4))]
        # E = [np.sqrt((h**2) + (v**2)) for h,v in zip(Eh,Ev)]
        Ecomplex = [eh + ev for eh,ev in zip(Eh,Ev)]
        
        Cim = c2image.c2image(Ecomplex)
        
        plt.imshow(Cim)
        plt.show()
        
        if pick:
            with open(dirPath + t+t[0:len(t)-1]+ 'compWavefield.pkl', "wb") as f:
                pickle.dump([Cim, dx, dy], f)
        else:
            pass
