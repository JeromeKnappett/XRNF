...
dirPath ='/user/home/opt/xl/xl/experiments/beamCharacterisationVer/data/' #beamCharacterisationVer/data/'  #'/user/home/opt/xl/xl/experiments/fullbeamPolarisation/'#'/user/home/opt/xl/xl/experiments/beamCoherence2/data/' 

trials = ['afterProp/']#['atMask/', 'atMaskFlux/', 'atMaskStokes/'] #, 'beamCoherence2_op/data/coherence/']#,'atMask/', 'atMaskFlux/'] #['data/'] #['fullRes_5000e/', 'fullRes_100e/'] #['120_1000x1000/']#'200e/','10000e/', '120e_250ir/']
...
inten = imageio.imread(dirPath + 'afterBDA_125/afterBDA_125intensity.tif')
sIn = imageio.imread(dirPath +'afterBDA_125/intensity.tif')

plt.imshow(sIn, aspect='auto')
plt.title("multi-electron")
plt.colorbar()
plt.show()
...
#              round_sig(ry*sF/2)]
    ...
    try:
        ...
        plt.imshow(Ise[40000:120000,150:290], aspect='auto')
        plt.title("single electron intensity")
        plt.colorbar()
        plt.show()
        
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'singleEintensity.tif',np.float32(Ise[40000:120000,150:290]))
...
#        I4 = abs(I[3,:,:])
        ...
        #plt.clf()
        #plt.close()
        cMax,cMin = np.max(I), np.min(I)
        im = axs[0,0].imshow(I1,vmax=cMax,vmin=cMin, aspect='auto')
        ...
        #plt.xlabel("position [m]") # [\u03bcm]")
        axs[0,0].set_title("$S_0$") #Multi-Electron Intensity")
        axs[0,1].imshow(I2,vmax=cMax,vmin=cMin, aspect='auto')
        axs[0,1].set_title("$S_1$")
        axs[1,0].imshow(I3,vmax=cMax,vmin=cMin, aspect='auto')
        axs[1,0].set_title("$S_2$")
        axs[1,1].imshow(I4,vmax=cMax,vmin=cMin, aspect='auto')
        ...
        #plt.clf()
        #plt.close()
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'stokes0.tif',np.float32(I1))
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'stokes1.tif',np.float32(I2))
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'stokes2.tif',np.float32(I3))
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'stokes3.tif',np.float32(I4))
    else:
...
#            plt.savefig(dirPath + "plots/" + "Propintensity" + name + ".png")
    ...
    if hdf5:
        print(" ")
        print('Loading wavefront from hdf5 file')
        w = Wavefront()
        w.load_hdf5(dirPath + t + 'wf_final.hdf')
        phase = w.get_phase()
        ...
        plt.imshow(phase)
        plt.colorbar()
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'phase.tif',np.float32(phase))
    else:
    ...
    try:
        ...
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'coherenceHor.tif',np.float32(cX))
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'coherenceVer.tif',np.float32(cY))
    except OSError:
