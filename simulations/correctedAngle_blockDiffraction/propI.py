...
dirPath ='/user/home/opt/xl/xl/experiments/correctedAngle_blockDiffraction/data/'  #'/user/home/opt/xl/xl/experiments/fullbeamPolarisation/'#'/user/home/opt/xl/xl/experiments/beamCoherence2/data/' 
order = ['blockDiffraction','apertureDiffraction'] #0,24,4,20,1,23,9,15,2,22,14,10,3,21,19,5,18,8,16,6,17,13,11,7,12]
...
#    plt.show()
    ...
    if numC > 1:
        ...
        axs[1,0].imshow(I2, aspect='auto')
        axs[1,0].set_title("C #2")
        axs[0,1].imshow(I3, aspect='auto')
        axs[0,1].set_title("C #3")
        axs[1,1].imshow(I4, aspect='auto')
...
#        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'intensity_close.tif',np.float32(I[0,int(float(ny)/4):int(3*float(ny)/4),int(float(nx)/4):int(3*float(nx)/4)]))
