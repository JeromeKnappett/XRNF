...
order = [0.005,0.01,0.015,0.02,0.025,0.03]#,0.035,0.04,0.045,0.05] #range(180,225,5)
mag = [1.00198486079209,
       1.00408889540472, 
       1.00625875052425,
       1.00846892284805, 
       1.01070572004105,
...
labels =  [str(o) + ' rad' for o in order]
...
N = 3000                           # number of pixels to take for line profile  - 1200 for roughness aerial images
n = 100                                 # number of pixels to average over for line profile - 15 for roughness aerial images
plotRange = 500                       # range of aerial image plot in nm

# middle pixels of images
mid = np.full((np.shape(files)[0],2), (500, 15750-125))
...
A = 39.0e5 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
A2 = 18.0e5
k = 2*np.pi/wl
