...
dirPath = '/user/home/opt/xl/xl/experiments/divergence/data2/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = [0.005,0.01,0.015,0.02,0.025,0.03,0.035,0.04,0.045,0.05,0.055,0.06,0.065,0.07,0.075,0.08,0.085,0.09,0.095,0.1] #range(180,225,5)
mag = [1.00198486079209,1.00408889540472,1.00625875052425,1.00846892284805, 1.01070572004105,1.01296033962728,1.01522981766794,1.01750874456236,
       1.01979620345035,1.02209038719491,1.02439023666151,1.02669499295026,1.02900410749688,1.03131718268714,1.03363393155178,1.03595414975578,
       1.03827769572085,1.04060447625549,1.04293443599288,1.04526754951109]
#1.04760381537504
#1.04994325157638
#1.05228589200468
#1.05463178368905
...
files = [str(o) + 'rad_AE/' + str(o) + 'rad_AE.pkl' for o in order]

labels =  [str(o-0.015) + ' rad' for o in order]
...
_N = range(30,2031,1)  #range(1200,1201) #                            # number of pixels to take for line profile  - 1200 for roughness aerial images
...
mid = np.full((np.shape(files)[0],2), (500, 15750-125))  #1250 for original
...
for N in _N:
    ...
    A = 45.0e5 # 0.37e5 #0.3e5  # this may be scaled to  match simulated intensity
    A2 = 18.0e5
    k = 2*np.pi/wl
