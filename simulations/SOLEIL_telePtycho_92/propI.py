...
dirPath ='/user/home/opt/xl/xl/experiments/SOLEIL_telePtycho_92/data/13x15_20um_highT/'
# 6x6_twopins/'
# 15x5_4um/'
# test_scan_4um_longZ/' 
order = ['probe']
...
z_pos = [0]
x_pos = range(0,numX+1)
y_pos = range(0,numY+1)

order = []
for z in z_pos:
   for y in y_pos:
       for x in x_pos:
           name =  f"pos_z{z}_x{x}_y{y}/"
...
print([x for x in x_pos])
print([y for y in y_pos])
print(order)

trials =[ str(o) for o in order] 
mePR = 'IntensityDist_SE.dat'
# 'res_int_pr_me.dat_1.dat' 
# 'EField.dat' 
...
if Efile == None:
    ...
    #    plt.show()
        ...
        if SAVE or pick:
            try:
                I = np.loadtxt(dirPath+t+mePR,skiprows=10)
                print(len(I))
                # print(numC*nx*ny)
                # plt.plot(I)
                # plt.title('ME intensity')
                # plt.show()
                I = np.reshape(I, (numC,int(ny),int(nx)))   #CHANGED BY JK      # Propagated multi-electron intensity
            except:
                # pass
                I = np.loadtxt(dirPath+t+mePR,skiprows=11)
                I = np.reshape(I, (numC,int(ny),int(nx)))   #CHANGED BY JK      # Propagated multi-electron intensity
            Iflat = I.flatten()**2
    ...
    #    :param int _width_pixels: the width of the final plot in pixels  
                ...
                I = I[0,:,:]
                I = (I / 1.0e-6) * (dx*dy) # convert from intensity to ph/s
                I[I < 1] = 0
                nx,ny = int(nx),int(ny)
                # plt.imshow(I[ny//2 - 1000:ny//2 + 1000,nx//2 - 200:nx//2 + 200], aspect='auto')
        #         plt.imshow(I, aspect='auto')
        ...
        #         plt.show()
                ...
                plt.title("Multi-electron intensity (log scale): ".format(name))
                plt.xticks(np.arange(0,int(nx)+1,int(nx)/4),tickAx)
                plt.yticks(np.arange(0,int(ny)+1,int(ny)/4),tickAy)
                plt.colorbar()
                plt.show()
                
                # plt.imshow(np.log(I)[0:100,0:100])
                # plt.colorbar()
                # plt.show()
                if np.max(np.abs(I)) > 2147483647:
                    I = I*(2147483647 / np.max(np.abs(I)))
                else:
                    pass
                if SAVE:
                    # I = I.astype(int)
        ...
        #            imageio.imwrite(dirPath+'tiffs/'+t[0:len(t)-1]+'intensity.tif',np.float32(I[0,:,:]))
                    imageio.imwrite(dirPath+t+t[0:len(t)-1]+'intensity.tif',np.int32(I))
                else:
                    pass
                ...
                if pick:
                    ...
                    with open(dirPath + t+t[0:len(t)-1]+ '.pkl', "wb") as f:
                        pickle.dump([np.float32(I), dx, dy], f)
                else:
                    pass
    ...
    for e,t in enumerate(trials):
        ...
        name = t[0:len(str(t))-1]
        print("Analysing trial: {}".format(name))
        wf = pickle.load(open(dirPath+t+Efile, 'rb')) #Electric Field Wavefront   
        print(pick)
        ...
        z = wf.mesh.zStart #Longitudinal Position [m] at which Electric Field has to be calculated, i.e. the position of the first optical element
        eStart = wf.mesh.eStart  #Initial Photon Energy [eV]
        eFin = wf.mesh.eFin #Final Photon Energy [eV]
        xMin = wf.mesh.xStart #Initial Horizontal Position [m]
        xMax = wf.mesh.xFin  #Final Horizontal Position [m]
        yMin = wf.mesh.yStart #Initial Vertical Position [m]
        yMax = wf.mesh.yFin #Final Vertical Position [m]
        Ex = wf.arEx 
        Ey = wf.arEy
        ne = wf.mesh.ne
        nx = wf.mesh.nx
        ...
        # I, P = getIPfromWavefield(wf)
        
        # fig, ax = plt.subplots(1,2)
        # Iplt = ax[0].imshow(I,aspect='auto')        
        # fig.colorbar(Iplt, ax=ax[0])
        # ax[0].set_title('Intensity from Efield pickle')
        # Pplt = ax[1].imshow(P,aspect='auto')        
        # fig.colorbar(Pplt, ax=ax[1])
        # ax[1].set_title('Phase from Efield pickle')
