...
dirPath = '/user/home/opt/xl/xl/experiments/SOLEIL_telePtycho_92/data/13x15_20um_highT/pos_z0_x0_y0/'
# test_scan_4um_longZ/pos_z0_x0_y0/'
# 5x5_twopins_z2/pos_z0_x0_y0/'
# test_scan_4um_z2/pos_z0_x0_y0/'
#test_scan_twopins/pos_z0_x0_y0/'
...
elements = [
            ...
            'Pinhole incident',     #9
            'Pinhole exit',     #10
            'Detector incident',     #11
            'resample'
            ]

# elements = [elements[i-1] for i in order]
...
for e,o in enumerate(order):
    ...
    if o == max(order)+1:
        ...
        if np.max(np.abs(I)) > 2147483647:
            ...
            pass
        if SAVE:
            tifffile.imwrite(dirPath + elements[o-1] + '.tif', np.int32(I))
        print('dx, dy: ', dx,dy)
        if checkSampling:
            from FWarbValue import getFWatValue
            fwhmx, fwhmy = getFWatValue(I, dx=dx, dy=dy,show=False,verbose=False)
...
# I = tifffile.imread(dirPath + 'intensity.tif')
P = tifffile.imread(dirPath + 'phase.tif')
# plt.imshow(I, aspect='auto')
# plt.title('Intensity')
# plt.colorbar()
# plt.show()

Ifile = dirPath + 'IntensityDist_SE.dat'
nx = str(np.loadtxt(Ifile, dtype=str, comments=None, skiprows=6, max_rows=1, usecols=(0)))[1:]
ny = str(np.loadtxt(Ifile, dtype=str, comments=None, skiprows=9, max_rows=1, usecols=(0)))[1:]
xMax = str(np.loadtxt(Ifile, dtype=str, comments=None, skiprows=5, max_rows=1, usecols=(0)))[1:]
xMin = str(np.loadtxt(Ifile, dtype=str, comments=None, skiprows=4, max_rows=1, usecols=(0)))[1:]
