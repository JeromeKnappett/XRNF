...
dirPath ='/user/home/opt/xl/xl/experiments/correctedWBS/data/'  #'/user/home/opt/xl/xl/experiments/fullbeamPolarisation/'#'/user/home/opt/xl/xl/experiments/beamCoherence2/data/' 
order = ['atExitSlits_corrected_20ev']
...
for t in trials:
    ...
    try:
        Ise = np.reshape(np.loadtxt(dirPath+t+sePR,skiprows=9), (int(ny),int(nx)))              # Propagated single-electron intensity
        plt.imshow(Ise[:,:], aspect='auto')
        plt.title("Single-electron intensity: ".format(name))
        plt.colorbar()
#        if SAVE == True:
...
#            plt.savefig(dirPath + "plots/" + "Propintensity" + name + ".png")
        ...
        imageio.imwrite(dirPath+t+t[0:len(t)-1]+'SEintensity.tif',np.float32(Ise[:,:]))
