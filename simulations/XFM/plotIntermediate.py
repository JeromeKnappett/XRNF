...
dirPath ='/user/home/opt/xl/xl/experiments/XFM/data/grating_nearfield/FZPprop_10mm/'
#FZP/FZPprop_1mm/'
#FZP_exit/'
...
elements = ['Drift2Aperture','Aperture 1','Drift2Sample',
            'Aperture 2',
            'Block','Mask','Aerial Image']#,'Drift2Pinhole','Pinhole','Drift2Detector']
            # ]
...
for e,o in enumerate(order):
    ...
    if e == 0:
        ...
        nx = str(np.loadtxt(dirPath + str('res_int_se.dat'), dtype=str, comments=None, skiprows=6, max_rows=1, usecols=(0)))[1:]#[1:3]
        ny = str(np.loadtxt(dirPath + str('res_int_se.dat'), dtype=str, comments=None, skiprows=9, max_rows=1, usecols=(0)))[1:]#[1:3]
        xMin = str(np.loadtxt(dirPath + str('res_int_se.dat'), dtype=str, comments=None, skiprows=4, max_rows=1, usecols=(0)))[1:]#[1:3]
        xMax = str(np.loadtxt(dirPath + str('res_int_se.dat'), dtype=str, comments=None, skiprows=5, max_rows=1, usecols=(0)))[1:]#[1:3]
        rx = float(xMax)-float(xMin)
        dx = np.divide(rx,float(nx))
        yMin = str(np.loadtxt(dirPath + str('res_int_se.dat'), dtype=str, comments=None, skiprows=7, max_rows=1, usecols=(0)))[1:]#[1:3]
        yMax = str(np.loadtxt(dirPath + str('res_int_se.dat'), dtype=str, comments=None, skiprows=8, max_rows=1, usecols=(0)))[1:]#[1:3]
        ry = float(yMax)-float(yMin)
        dy = np.divide(ry,float(ny))
        I = np.reshape(np.loadtxt(dirPath+str('res_int_se.dat'),skiprows=10), (int(ny),int(nx)))
    ...
    plt.title(f"{elements[e]} : {propDist[e]} m propagation, #{o}",fontsize=fSize)
    ...
    #     imageio.imwrite(dirPath + elements[e] + '.tif',I)
