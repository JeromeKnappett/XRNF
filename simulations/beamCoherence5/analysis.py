...
dirPath = '/data/experiments/beamCoherence5/data/'
batchOutputFile = dirPath + '001/results.pickle'

trialData =[dirPath + '10/']#, dirPath + '10/', dirPath + '001/'] 
#[dirPath + 'LV/', dirPath + 'LH/', dirPath + 'LD/', dirPath + 'CR/', dirPath + 'CL/']
#[dirPath + '01/', dirPath + '05/', dirPath + '60/', dirPath + '70/', dirPath + '80/', dirPath + '90/', dirPath + '100/']
files = ['stokes0.pkl', 'stokes1.pkl', 'stokes2.pkl']
pickles = ['eh10.pkl']#, 'e10.pkl', 'e001.pkl']
#['ch0.pkl', 'ch1.pkl', 'ch2.pkl', 'ch3.pkl', 'ch4.pkl', 'ch10.pkl', 'ch20.pkl', 'ch40.pkl']
...
#intensity profiles
for i, v in enumerate(listValues(results, 'Intensity/Total/Y/profile')):
    print("Shape of y-intensity: {}".format(np.shape(v)))
    print("Shape of y: {}".format(np.shape(y)))
    #print("Y = {}".format(y))
    plt.plot(y[0],v)
    plt.title('Intensity (x=0)')
    plt.savefig(dirPath + "plots/" + "Yintensity" + str(i) + ".png")
    print("Y-intensity plots saved to: " + dirPath + "plots/" + "Yintensity" + str(i) + ".png")
    plt.show()
    plt.clf()
    plt.close()

#intensity profiles
for i, v in enumerate(listValues(results, 'Intensity/Total/X/profile')):
    print("Shape of x-intensity: {}".format(np.shape(v)))
    print("Shape of x: {}".format(np.shape(y)))
    plt.plot(x[0],v)
    plt.title('Intensity (y=0)')
    plt.savefig(dirPath + "plots/" + "Xintensity" + str(i) + ".png")
    print("X-intensity plots saved to: " + dirPath + "plots/" + "Xintensity" + str(i) + ".png")
    plt.show()
    plt.clf()
    plt.close()
...
for t, pk in zip(trialData, pickles):
    print("")
    print("----- Analysing trial:" + pk[0:len(str(pk))-4] + " -----")
    ...
    plt.xlim(int(nx/2) - 100, int(nx/2) + 100)
    plt.legend()
    plt.savefig(dirPath + "plots/" + "intensitypickleCLOSE" + pk[0:len(str(pk))-4] + ".png")
    print("Intensity saved to: " + dirPath + "plots/" + "intensitypickleCLOSE" + pk[0:len(str(pk))-4] + ".png")
    plt.show()
    ...
    plt.plot(Iy, '.', label='Vertical cut')
    plt.legend()
    plt.savefig(dirPath + "plots/" + "intensitypickle" + pk[0:len(str(pk))-4] + ".png")
    print("Intensity saved to: " + dirPath + "plots/" + "intensitypickle" + pk[0:len(str(pk))-4] + ".png")
    plt.show()

    

    try:
        ix = str(np.loadtxt(t + 'res_int_se.dat', dtype=str, comments=None, skiprows=6, max_rows=1, usecols=(0)))[1:6]#[1:3]
        iy = str(np.loadtxt(t + 'res_int_se.dat', dtype=str, comments=None, skiprows=9, max_rows=1, usecols=(0)))[1:6]#[1:3]
        print("Initial resolution (x,y): {}".format((ix,iy)))
        I0 = np.reshape(np.loadtxt(t + 'res_int_se.dat',skiprows=10), (int(ix),int(iy)))         # Initial intensity     
    except OSError:
        ...
        pass
    try:
        nx = str(np.loadtxt(t + 'res_int_pr_me.dat', dtype=str, comments=None, skiprows=6, max_rows=1, usecols=(0)))[1:6]#[1:3]
        ny = str(np.loadtxt(t + 'res_int_pr_me.dat', dtype=str, comments=None, skiprows=9, max_rows=1, usecols=(0)))[1:6]#[1:3]
        nx = int(nx)
        ny = int(ny)
        print("Propagated resolution (x,y): {}".format((nx,ny)))
        I = np.reshape(np.loadtxt(t + 'res_int_pr_me.dat',skiprows=11), (nx,ny))          # Propagated intensity
    except OSError:
    ...
    try:
        ...
        #plt.xlabel("position [m]") # [\u03bcm]")
        plt.title("Initial intensity, :" + pk[0:len(str(pk))-4])
        plt.colorbar()
        plt.savefig(dirPath + "plots/" + "initialintensity" + pk[0:len(str(pk))-4] + ".png")
        print("Saving initial intensity plot to: {}".format("plots/" + "initialintensity" + pk[0:len(str(pk))-4] + ".png"))
        plt.show()
        plt.clf()
        plt.close()
    except NameError:
    ...
    try:
        ...
        #plt.xlabel("position [m]") # [\u03bcm]")
        #plt.xlabel("position [m]") # [\u03bcm]")
        plt.title("Intensity, e =" + pk[0:len(str(pk))-4])
        plt.xlim(475,525)
        plt.ylim(475,525)
        plt.colorbar()
        plt.savefig(dirPath + "plots/" + "intensity" + pk[0:len(str(pk))-4] + ".png")
        print("Saving intensity plot to: {}".format("plots/" + "intensity" + pk[0:len(str(pk))-4] + ".png"))
        plt.show()
        plt.clf()
        plt.close()
    except NameError:
    ...
    try:
        plt.imshow(cX)
        plt.title("Horizontal Coherence, e =" + pk[0:len(str(pk))-4])
        plt.colorbar()
        plt.savefig(dirPath + "plots/" + "coherenceHor" + pk[0:len(str(pk))-4] + ".png")
        print("Saving horizontal coherence plot to: {}".format("plots/" + "coherenceHor" + pk[0:len(str(pk))-4] + ".png"))
        plt.show()
        plt.clf()
        plt.close()
        ...
        #plt.xticks(np.arange(0,nx+1,nx/4),tickAx)
        plt.xlabel("position [m]") # [\u03bcm]")
        plt.title("Horizontal Coherence (cuts), e =" + pk[0:len(str(pk))-4])
        plt.legend()
        plt.savefig(dirPath + "plots/" + "coherenceHorCuts" + pk[0:len(str(pk))-4] + ".png")
        print("Saving horizontal coherence cuts plot to: {}".format("plots/" + "coherenceHorCuts" + pk[0:len(str(pk))-4] + ".png"))
        plt.show()
        plt.clf()
        plt.close()
        
        plt.imshow(cY)
        plt.title("Vertical Coherence, e =" + pk[0:len(str(pk))-4])
        plt.colorbar()
        plt.savefig(dirPath + "plots/" + "coherenceVer" + pk[0:len(str(pk))-4] + ".png")
        print("Saving vertical coherence plot to: {}".format("plots/" + "coherenceVer" + pk[0:len(str(pk))-4] + ".png"))
        plt.show()
        plt.clf()
        plt.close()
        ...
        #plt.xlabel("position [m]") # [\u03bcm]")
        plt.title("Vertical Coherence (cuts), e =" + pk[0:len(str(pk))-4])
        plt.legend()
        plt.savefig(dirPath + "plots/" + "coherenceVerCuts" + pk[0:len(str(pk))-4] + ".png")
        print("Saving vertical coherence cuts plot to: {}".format(dirPath + "plots/" + "coherenceHorCuts" + pk[0:len(str(pk))-4] + ".png"))
        plt.show()
        plt.clf()
        plt.close()
        ...
        plt.clf()
        plt.close()
        clX, clY = wfCoherence.getCoherenceLength(dCx, dCy, 0.7, dirPath + "plots/" + "coherenceLen" + pk[0:len(str(pk))-4] + ".png")
    ...
    try:
        ...
        print("Saving horizontal mutual intensity (J) plots to: {}".format(dirPath + "plots/" + "mutualIntensityHor" + pk[0:len(str(pk))-4] + ".png"))
        plt.savefig(dirPath + "plots/" + "mutualIntensityHor" + pk[0:len(str(pk))-4] + ".png")
        plt.show()
        plt.close()
        plt.clf()
        ...
        axs[1,2].set_title("J (mean) - Normalised to I")
        print("Saving vertical mutual intensity (J) plots to: {}".format(dirPath + "plots/" + "mutualIntensityVer" + pk[0:len(str(pk))-4] + ".png"))
        plt.savefig(dirPath + "plots/" + "mutualIntensityVer" + pk[0:len(str(pk))-4] + ".png")
        plt.show()
        plt.close()
        plt.clf()
    except NameError:
...
for t, pk in zip(trialData, pickles):
    ...
    #            print("shape of _s3: {}".format(np.shape(_s3)))
            ...
            plt.plot(stk.arS)
            plt.title(pk[0:len(str(pk))-4] + '_Stokes' + str(i))
            plt.savefig(dirPath + 'plots/' + pk[0:len(str(pk))-4] + '_arrayStokes' + str(i) + '.png')
            print("stk.arS plot saved to : {}".format(dirPath + 'plots/' + pk[0:len(str(pk))-4] + '_arrayStokes' + str(i) + '.png'))
            plt.show()
            plt.clf()
            plt.close()
            ...
            axs[0, 0].set_title(pk[0:len(str(pk))-4] + '_S0_' + str(i))
            axs[0, 1].imshow(S1, vmin=smin, vmax=smax)
            axs[0, 1].set_title(pk[0:len(str(pk))-4] + '_S1_' + str(i))
            axs[1, 0].imshow(S2, vmin=smin, vmax=smax)
            axs[1, 0].set_title(pk[0:len(str(pk))-4] + '_S2_' + str(i))
            axs[1, 1].imshow(S3, vmin=smin, vmax=smax)
            axs[1, 1].set_title(pk[0:len(str(pk))-4] + '_S3_' + str(i))
            ...
            plt.savefig(dirPath + 'plots/' + pk[0:len(str(pk))-4] + '_reshapedStokes' + str(i) + '.png')
            print("Reshaped stk.arS plots saved to : {}".format(dirPath + 'plots/' + pk[0:len(str(pk))-4] + '_reshapedStokes' + str(i) + '.png'))
            plt.show()
            plt.clf()
            plt.close()        
            ...
            try:
                ...
                plt.savefig(dirPath + 'plots/' + pk[0:len(str(pk))-4] + '_reshapedSecondStokes' + str(i) + '.png')
                print("Secondary reshaped stk.arS plots saved to : {}".format(dirPath + 'plots/' + pk[0:len(str(pk))-4] + '_reshapedSecondStokes' + str(i) + '.png'))
                plt.show()
                plt.clf()
                plt.close()
    ...
    #            plt.close()
        ...
        print("No stokes files for trial... {}".format(pk[0:len(str(pk))-4]))
