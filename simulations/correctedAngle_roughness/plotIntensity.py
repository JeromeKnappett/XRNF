...
order = range(0,25)
cY = [0,2,2,2,2,2,4,4,4,4,4,6,6,6,6,6,8,8,8,8,8,10,10,10,10,10]
sigma = [0.0,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5,0.5,1.0,1.5,2.0,2.5]
...
labels =  ['ideal','\u03C3 = 0.5 nm ','\u03C3 = 1.0 nm ','\u03C3 = 1.5 nm ','\u03C3 = 2.0 nm','\u03C3 = 2.5 nm',
...
res = np.full((np.shape(files)[0]+1,2), [2.5011882651601634e-09, 2.426754806976689e-07])
mid = np.full((np.shape(files)[0]+1,2), (196, 12188))
...
tiffs.append(idealTif) # added to ensure efficiency is taken for ideal aerial image
print(np.shape(images))
IMS = []
IMS.append(idealImage)
PS = []
PS.append(idealProfile)
for im,ps in zip(images,profiles):
    IMS.append(im)
    PS.append(ps)
...
if efficiency:
    ...
    E0,E1 = efficiency.getAerialImageEfficiency(inTiff,tiffs,inRes,res,mid,G=10.0e-6,D=27.5e-6,
                                                extra=(1500,10),#(1300,100),
...
#                      M2D.flatten(),rmsC2d.flatten(),C2D.flatten(),#michelsonC2d.flatten(),rmsC2d.flatten(),[c[0] for c in compositeC2d.flatten()],
#                      fidel2d.flatten(),
#                      nils2d.flatten(),
#                      nilsSTD.flatten(),
#                      LWR.flatten(),
...
import pickle
with open(dirPath + 'LWR_redone.pkl', "wb") as f:
    pickle.dump(LWR, f)
with open(dirPath + 'dataStructure_redone.pkl', "wb") as g:
...
with open('/user/home/opt/xl/xl/experiments/correctedAngle_roughness/LWR_1.pkl', "wb") as f:
    pickle.dump(LWR, f)
with open('/user/home/opt/xl/xl/experiments/correctedAngle_roughness/dataStructure_1.pkl', "wb") as g:
    pickle.dump(dataStructure, g)