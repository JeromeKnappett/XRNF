...
def propWavefront():
   ...
   # initialWavefront='/opt/wpg/wpg/extensions/in/50Slices63dLTZP_simulationData/wf_noCS.h5'
    path = '/user/home/opt/xl/xl/experiments/speckleFocused/data/maskExit11.28mm/'
    ...
    dVol.propagate(wf)
    # Plot propagation plot
    plotPropWave(outPath + 'intensity/',
                 zRange,zPlanes,res=dx,axis='hor')#,log=True,lMin=1e13)
#    printWFDictUgly(wf)

def plotPropWave(path,zRange,zPlanes,res,axis,startPlane=0,log=False,lMin=None):
    import os
    import tifffile
