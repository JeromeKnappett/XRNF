...
dirPath = '/user/home/opt/xl/xl/experiments/speckleFocused/data/'
savePath = '/home/jerome/Documents/MASTERS/Figures/plots/roughness/'
order = ['focusCheck' + str(r) + 'mm/intensity.tif' for r in range(2,14)]#['closeNbig','closeNbigNoMask']#range(0,25)
...
files = [o for o in order]#[str(o) + '/' + str(o) + 'intensity.tif' for o in order]
...
labels =  [str(r) + 'mm' for r in range(2,14)]
#['With Speckle Mask', 'Without Speckle Mask']
...
N = 2500                            # number of pixels to take for line profile  - 1200 for roughness aerial images
n = 400                             # number of pixels to average over for line profile - 15 for roughness aerial images
plotRange = 1e7                       # range of aerial image plot in nm

# res and middle pixels of images

#res = [(5.253179450836339e-09, 3.622995883515388e-08),(5.253219530026472e-09, 3.6314214553375166e-08)]#np.full((np.shape(files)[0],2), [2.5011882651601634e-09, 2.426754806976689e-07])
#res = [(3.317631116498579e-09,5.0412035231282055e-09),
res = [(4.3861701380034884e-07,6.663653540690268e-07),
...
mid = np.full((np.shape(files)[0],2), (2880, 2880))
#[(int(2160/2),int(16384/2)),(int(4320/2),int(32768/2))] #np.full((np.shape(files)[0],2), (196, 12188))
...
axs[1].imshow(images[1],aspect='auto')
...
with open('/user/home/opt/xl/xl/experiments/correctedAngle_roughness/LWR.pkl', "wb") as f:
    pickle.dump(LWR, f)
with open('/user/home/opt/xl/xl/experiments/correctedAngle_roughness/dataStructure.pkl', "wb") as g:
